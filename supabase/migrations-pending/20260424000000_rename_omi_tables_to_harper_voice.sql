...
--
-- Renames the two Omi-era tables to Harper Voice namespace. Kept in
...
--      backend-hono/src/routes/harper-voice.ts still reference `omi_sessions`
...
--        b. sed -i '' 's/from("omi_sessions")/from("harper_voice_sessions")/g;
--                      s/from("omi_pairings")/from("harper_voice_pairings")/g;
--                      s/"omi_uid"/"harper_voice_device_uid"/g'
--             backend-hono/src/routes/harper-voice.ts
--             backend-hono/src/services/harper-voice/*.ts
--        c. supabase db push
...
ALTER TABLE IF EXISTS public.omi_pairings RENAME TO harper_voice_pairings;
ALTER TABLE IF EXISTS public.harper_voice_pairings
  RENAME COLUMN omi_uid TO harper_voice_device_uid;

ALTER TABLE IF EXISTS public.omi_sessions RENAME TO harper_voice_sessions;

-- Rename the supporting indexes so they stay self-documenting.
ALTER INDEX IF EXISTS public.omi_pairings_omi_uid_idx
  RENAME TO harper_voice_pairings_device_uid_idx;
ALTER INDEX IF EXISTS public.omi_sessions_user_started_idx
  RENAME TO harper_voice_sessions_user_started_idx;
ALTER INDEX IF EXISTS public.omi_sessions_active_idx
  RENAME TO harper_voice_sessions_active_idx;
