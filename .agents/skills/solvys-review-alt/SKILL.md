...
description: TP-side junior PR review. Reads the latest victory-kind message for a brief, fetches the open PR via gh, reviews the diff, and posts review notes back to the junior's Codex via the coworking bridge. Use when a junior has passed work up the chain and TP is ready to review.
---
...
"Notes sent to peer `<id>`. Their Codex agent picks them up on the next poll cycle (up to 30 minutes) or when they refresh their thread. No further action from you until they post another victory."
