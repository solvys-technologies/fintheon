# Sprint Brief: S32-T7 — Harper Advisory Layer + Calendar Countdown Pill + Autopilot Guardian + Blindspots UI Wiring
...
- [ ] New table `backend-hono/migrations/036_watchouts.sql`:
  ```sql
...
- [ ] Changelog entry
- [ ] `// [claude-code 2026-04-23] S32-T7 advisory + calendar pill + autopilot guardian + blindspots wire` on modified files
...
4. Migration `036_watchouts.sql`; hand to TP.
...
```
[v5.23.0] feat: S32-T7 advisory layer + calendar countdown pill + autopilot guardian + blindspots UI wiring
```
