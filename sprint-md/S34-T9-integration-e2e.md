...
1. **Gate check.** `git fetch origin && git log --oneline origin/main -30`. Verify T1/T3/T5/T6/T7/T8 commits are on `main`. If anything is missing, stop and message orchestrator peer `ra3mt2`.
...
7. **Modal mount.** Grep for `<EconCountdownModal`; if not rendered anywhere, mount in Strategium root.
8. **Build + typecheck.**
...
- **If populator is still scraping all 7 countries × 4 categories** — the filter-guard in step 4 is still correct; it becomes a no-op once everything is active.
