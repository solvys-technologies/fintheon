...
- [ ] **Remove the rettiwt-gated no-op path** in [backend-hono/src/services/riskflow/feed-poller.ts:302-332](backend-hono/src/services/riskflow/feed-poller.ts#L302-L332):
  ...
  - Do NOT remove the `RETTIWT_REENABLE=true` flag or the rettiwt service itself — we keep the kill-switch path for future re-enable per the plan's "open items."
...
2. Edit `agent-reach.ts`: add `handles?` field, Nitter expansion, mirror fallback chain, tagged `source_domain`. Reuse the existing `fetchRss` + `scrapeUrl` from `agent-reach-service`.
