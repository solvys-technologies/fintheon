...
export const changelog: ChangelogEntry[] = [
  ...
  },
  {
    ...
    agent: "claude-code",
    summary:
      "v5.29.0 — S40 close-out: P9 Consul Browser end-to-end (Browserbase client/sse/session-manager + auth-gated /api/browserbase routes + ConsulBrowserContext SSE consumer + DualPaneShell primitive + ConsulBrowserPane iframe/webview + 32x32 Globe button in chat composer + consul_browser Strands tool added to AUTO_APPROVED_TOOLS in both harper-tools and vproxy/anthropic-client; t-pane-slide-in/t-pane-retract transitions in transitions.css). P4 closed: BEA/FRB/Census/EDGAR/Treasury pollers now ship alongside BLS via shared burst-runner.ts; agency-pollers/scheduler dispatches by agency. P6 closed: useTimeToPrint hook now exposes upcoming[] + cadence step-down (60s→30s→1s); TimeToPrintDockable renders the t-dropdown stack of up to 4 upcoming events; new StrategiumTimeToPrintSlot couples PsychAssist auto-float to TTP visibility. P7 closed: persona YAML frontmatter on oracle/feucht/consul/herald; spawnSectorDispatch added to boardroom-spawner; catalyst-promoter routes promoted items to dispatchBySector / triggerMegacapAnalyst. P3 closed: 8 Macro handles seeded via 20260425200000_s40_seed_macro_handles.sql + same handles in DEFAULT_SOURCE_ACCOUNTS; streaming-watcher prefers Browserbase CDP connect when credentialed, falls back to local Chromium. P8 closed: refreshMegacapEarnings now pulls market_cap_usd via FMP /v3/profile and persists it; new megacap-earnings-enrichment cron runs every 5 min, enriches actual_eps/beat_miss/surprise_percent and dispatches the megacap analyst on each landed actual. Version bump to v5.29.0 in package.json. Backend bun build clean; frontend tsc + clean vite build clean.",
    files: [
      "package.json",
      "supabase/migrations/20260425200000_s40_seed_macro_handles.sql",
      "backend-hono/src/services/browserbase/client.ts",
      "backend-hono/src/services/browserbase/sse.ts",
      "backend-hono/src/services/browserbase/session-manager.ts",
      "backend-hono/src/routes/browserbase/index.ts",
      "backend-hono/src/routes/browserbase/handlers.ts",
      "backend-hono/src/routes/index.ts",
      ...
      "backend-hono/src/services/time-to-print/agency-pollers/burst-runner.ts",
      "backend-hono/src/services/time-to-print/agency-pollers/bea.ts",
      "backend-hono/src/services/time-to-print/agency-pollers/frb.ts",
      "backend-hono/src/services/time-to-print/agency-pollers/census.ts",
      "backend-hono/src/services/time-to-print/agency-pollers/edgar.ts",
      "backend-hono/src/services/time-to-print/agency-pollers/treasury.ts",
      "backend-hono/src/services/time-to-print/agency-pollers/scheduler.ts",
      "backend-hono/src/services/cron/megacap-earnings-enrichment.ts",
      "backend-hono/src/services/earnings/megacap-fmp.ts",
      ...
      "frontend/contexts/ConsulBrowserContext.tsx",
      "frontend/components/layout/DualPaneShell.tsx",
      "frontend/components/strategium/ConsulBrowserPane.tsx",
      "frontend/components/strategium/StrategiumTimeToPrintSlot.tsx",
      "frontend/components/strategium/TimeToPrintDockable.tsx",
      "frontend/components/chat/FintheonComposer.tsx",
      "frontend/components/ChatInterface.tsx",
      "frontend/hooks/useTimeToPrint.ts",
  ...
  },
  {
    ...
    agent: "claude-code",
    summary:
      "S40 — Time-To-Print + News Realtime + Narrative Overhaul. Eight pillars on branch s40-ttp-realtime (worktree fintheon-s40-ttp-realtime, off v5.28.0). P1 voice TTS routes through services/voice-tts/ (Piper en_GB-cori-medium primary; ElevenLabs fallback; new GET /api/voice/sample). P2 news-worker stabilization: NEWS_WORKER_CONTRACT.md lock + boot-time assertion (BREAKING 60s→180s, STANDARD 5m→1h), watchdog with auto-restart + notifySuperadmins, soft-delete sweep + hard-delete + source auto-downweight crons. P3 Twitter overhaul: services/twitter/streaming-watcher (Playwright + GraphQL XHR intercept), rettiwt fallback with cookie pool, pipeline orchestrator with HEALTHY/DEGRADED/RECOVERING/DEAD state machine; agent-reach source retired. P4 agency burst pollers (BLS canonical with body-hash diff at 500ms in T-30s..T+60s window; BEA/FRB/Census/EDGAR/Treasury are descriptor stubs); polarity-aware beat-miss classifier; broadcastEconPrint on print. P5 narrative classifier: ai-singularity keyword rewrite (drop generic data center/infrastructure, add compound matches + entity tokens) + shouldExcludeFromSingularity guard for geopolitical-without-AI-token; merger weight 3→6 + new partnership_deal:7; NarrativeFlow gate hybrid (iv>=4.5 OR top-50/24h); softened junk filter; level-3 catalyst keywords; one-shot scripts/s40-narrative-backfill.ts. P6 Time-To-Print: SSE 'time-to-print' event (imminent/live/printed/cleared), eligibility join over economic_events+earnings_events country='US', useTimeToPrint hook, TimeToPrintDockable header+floating widget with pulse on countdown 00:00 + beat/miss chip + +N more, CountryFlag SVG primitive, Refinement Engine CountryToggle (US-only v1). MainLayout integration NOT touched (peers own per s35-unified safe-zones). P7 Sector-of-Risk: config/risk-sectors.ts + megacap-analyst with hardcoded 12-ticker NDX∩SPX>$300B watchlist. P8 megacap earnings: new earnings_events table + FMP weekly Sun 22:00 ET cron + /api/earnings/upcoming. Data layer 20260425194950_s40_data_layer.sql adds tier_weight, noise_score, country, headline_hash, archived_at, earnings_events, worker_health (push from main worktree per memory). Operator notes: Piper binary + ONNX weights install, X_AUTH_COOKIES/X_BURNER credentials, FMP_API_KEY required for earnings; backfill script is destructive (sets archived_at) — smoke on Supabase branch first.",
    files: [
      "supabase/migrations/20260425194950_s40_data_layer.sql",
      "backend-hono/src/services/voice-tts/index.ts",
      "backend-hono/src/services/voice-tts/piper.ts",
      "backend-hono/src/services/voice-tts/elevenlabs.ts",
      "backend-hono/src/services/voice-tts/types.ts",
      "backend-hono/src/services/voice-tts.ts",
      "backend-hono/src/routes/voice/handlers.ts",
      "backend-hono/src/routes/index.ts",
      "backend-hono/src/workers/news-worker/NEWS_WORKER_CONTRACT.md",
      "backend-hono/src/workers/news-worker/contract.ts",
      "backend-hono/src/workers/news-worker/boot.ts",
      "backend-hono/src/workers/news-worker/watchdog.ts",
      ...
      "backend-hono/src/services/twitter/types.ts",
      "backend-hono/src/services/twitter/streaming-watcher.ts",
      "backend-hono/src/services/twitter/rettiwt-fallback.ts",
      "backend-hono/src/services/twitter/pipeline.ts",
      ...
      "backend-hono/src/config/catalyst-levels.ts",
      ...
      "backend-hono/src/services/time-to-print/eligibility.ts",
      "backend-hono/src/services/time-to-print/scheduler.ts",
      "backend-hono/src/services/time-to-print/agency-pollers/types.ts",
      "backend-hono/src/services/time-to-print/agency-pollers/beat-miss.ts",
      "backend-hono/src/services/time-to-print/agency-pollers/bls.ts",
      "backend-hono/src/services/time-to-print/agency-pollers/scheduler.ts",
      "backend-hono/src/routes/time-to-print/index.ts",
      "backend-hono/src/routes/riskflow/index.ts",
      "backend-hono/src/routes/narrative/handlers.ts",
      "backend-hono/src/boot/services.ts",
      "backend-hono/scripts/s40-narrative-backfill.ts",
      "frontend/hooks/useTimeToPrint.ts",
      "frontend/components/strategium/TimeToPrintDockable.tsx",
      "frontend/components/primitives/CountryFlag.tsx",
      "frontend/components/refinement-engine/CountryToggle.tsx",
      "frontend/styles/transitions.css",
    ],
  },
  {
    date: "2026-04-25T21:00:00",
    agent: "claude-code",
    summary:
      "[v5.29.0] S42 Chat SOTA — 8-track parallel sprint unified into s42-chat-sota. T1 stream protocol (BridgeStreamEvent + UIEvent additive variants: thinking / tool_call / citation / artifact / complete; streamHarperChat threads sourceCount + provider model). T2 ComposerPrimitive + cmdk palette + MessageQueue wiring on web + mobile (Cmd+K, slash-persona override, @TICKER injection, queue-while-streaming + offline localStorage drain). T3 message render (BridgeStreamEvent fold helper, MessagePrimitive shim, StreamdownText + inline [N] CitationChip parsing, MessageFooter, AgentActivityRail with NothingFuse-style segmented progress). T4 dual-pane ArtifactPane (web resizable 25-70% + persisted split, t-panel-slide + rAF) + ArtifactSheet (mobile 3-snap framer-motion); fintheon:artifact CustomEvent across surfaces. T5 Browserbase agent-iframe plugin (services/browser/browserbase.ts + routes/browserbase.ts + browse_visible Strands tool + EmbeddedBrowserFrame mode='browserbase'); fallback playwright-screenshot stream when key absent. T6 Ask About This primitive on every output surface (Arbitrum, TradePlan, RegimeCard, Catalyst/Sanctum/MissionControl on web; ArbitrumVerdict/Catalyst/MiniRegime on mobile) + RiskFlow expanded card refactor (flat surface + accent top border + Source/Ask header). T7 chat mount-perf (mountTelemetry markers + HistorySkeletonList + idle-deferred skills fetch + isHydrating gate). T8 Nothing-design fuses + spinners + Doto display font (web NothingFuse 600ms ease-out, mobile VerticalFuseBar 220ms with 32ms stagger, mobile RadarSpinner WAAPI single-line sweep, SegmentedSpinner WAAPI step rotation, ai-loader horizontal indeterminate fuse, Doto on KPICard/AccountSummary/SessionStatusBar). All validations clean: frontend tsc + vite build, mobile tsc + vite build, backend bun build (16 frontend + 6 mobile pre-existing baseline tsc errors in shared/zod imports + voice-rim playwright spec — unchanged from v5.28.0). Local backend healthy. Unified PR opens against main; backend deploy to Fly + prod smoke pending TP authorization.",
    files: [
      ...
      "backend-hono/scripts/smoke-browserbase-route.mjs",
      "frontend/components/ChatInterface.tsx",
      "frontend/components/chat/FintheonComposer.tsx",
      ...
      "mobile/components/shared/NothingFuse.tsx",
