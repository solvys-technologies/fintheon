...
export const changelog: ChangelogEntry[] = [
  ...
  },
  {
    ...
    agent: 'claude-code',
    summary: 'Fix RiskFlow feed disappearance (updateFeedCache merge vs replace) + Harper feed health monitoring hook + /api/diagnostics/feed-health endpoint',
    files: ['backend-hono/src/services/riskflow/feed-service.ts', 'backend-hono/src/routes/diagnostics/index.ts', '.claude/hooks/harper-feed-health.sh', '.claude/settings.json']
  },
  {
  ...
  },
  {
    ...
    agent: 'claude-code',
    summary: 'Strands Agents SDK migration (Phases 1-5): Replace Vercel AI SDK with @strands-agents/sdk for Harper-Opus. VProxy provider at localhost:8317, 6 core tools + 9 solvys skills as Strands tools, UIMessageStream adapter, Graph-based PIC pipeline (Herald→Oracle→Consul→Feucht), 8 Claude Code hooks installed.',
    files: [
  ...
  },
  {
    ...
    agent: 'claude-code',
    summary: 'Harper Consilium real-time integration: autonomous loop output → boardroom messages with [HARPER-AUTO] prefix, AgentChattr autonomous message styling (Bot icon + task badge + accent border), HarperActivityFeed sidebar (w-80 collapsible), SSE stream for ops (full round-trip replacing 10s polling), NarrativeMap "Harper watching" overlay with gold flash, ConsiliumHub heartbeat indicator, AgentBadge autonomous marker, narrative-synthesis → timeline card links, latest-synthesis journal endpoint',
    files: [
  ...
  },
  {
    ...
    agent: 'claude-code',
    summary: 'Harper Autonomous Agent: Soul file, journal/ops tables, loop manager (Claude CLI subprocess), heartbeat scheduler (5min market/15min off), context builder, Harper Ops API + frontend panel (footer icon), event triggers (Level 4 items, VIX spikes), Consilium observer hook, codebase manifest. Gated by HARPER_AUTONOMOUS_ENABLED=true.',
    files: [
