...
export const changelog: ChangelogEntry[] = [
  ...
  },
  {
    ...
    files: [
      ...
      'frontend/components/auth/fintheonAppearance.ts',
      'backend-hono/src/index.ts',
  ...
  },
  {
    ...
    agent: 'claude-code',
    summary: 'T3 Narrative Map overhaul: Replaced NarrativeManageModal with NarrativeTimelineModal — scrollable vertical timeline with electric fintheon animation, editable titles, AI-sparkle description textarea, inline tag editing, date pickers, category selectors, and Add Narrative Event CTA. Added category field to CatalystCard type. Added timeline-fintheon CSS keyframes.',
    files: [
  ...
  },
  {
    ...
    agent: 'claude-code',
    summary: 'Fintheon rebrand T1: Rename all Fintheon references to Fintheon across codebase. Rename Clawnalyst Desk to Analyst Desk. Migrate all fintheon_ localStorage keys to fintheon: namespace with backward-compat migration. Update package names (fintheon-api → fintheon-api), API headers (Fintheon-AI-Gateway → Fintheon-AI-Gateway), and system prompts.',
    files: [
      ...
      'frontend/components/analysis/QuickFintheonModal.tsx',
      'frontend/components/chat/FintheonChatInput.tsx',
  ...
  },
  {
    ...
    agent: 'claude-code',
    summary: 'Fintheon rebrand: Fintheon → Fintheon, Roman terminology throughout, new tiers, IV labels, ER states, brief names',
  ...
  },
  {
    ...
    agent: 'claude-code',
    summary: 'Font theme switcher (Default/Solvys/Classic) in Appearance settings. Self-hosted WOFF2 fonts replace Google Fonts CDN. Compact TraderNametag sized to match VIX ticker. Nametag ER emotional fintheon (green=stable, red=tilt) with settings toggle.',
    files: [
      'frontend/fonts.css',
  ...
  },
  {
    ...
    agent: 'claude-code',
    summary: "Thinking toggle → Nous Hermes 4 deep reasoning. When thinkHarder=true and OPENROUTER_API_KEY set, route to OpenRouter nousresearch/hermes-4-70b with reasoning.enabled; stream reasoning + text. Default thinkHarder=true in FintheonFloatingChat, ChatInterface, AskHarpChatPanel, InterventionSidebar, ResearchDepartment.",
    files: ['backend-hono/src/routes/ai/handlers/chat.ts', 'frontend/components/chat/FintheonFloatingChat.tsx', 'frontend/components/ChatInterface.tsx', 'frontend/components/chat/AskHarpChatPanel.tsx', 'frontend/components/InterventionSidebar.tsx', 'frontend/components/executive/ResearchDepartment.tsx'],
  },
  {
    date: '2026-03-14T07:00:00',
    agent: 'claude-code',
    summary: 'Fintheon CLI: run shell commands from footer in Electron (e.g. cd backend-hono && npm run dev). Added / slash-command suggestions (backend, frontend, install, build, typecheck). IPC run-shell-command + cli-output streaming in main/preload.',
  ...
  },
  {
    ...
    agent: 'claude-code',
    summary: 'Remotion V4 rebuild: Inter/JetBrains Mono fonts (matching Fintheon frontend swap), all screenshots faded to near-black (0.04-0.06 opacity) as dark texture, overlay items significantly bigger and centered for presentation/keynote vibe. Same Apple/Perplexity editorial style (easeOut, crossfades, glass-morphism, hero-number-at-a-time). Recaptured dashboard + riskflow screenshots with new fonts.',
    files: ['remotion/src/theme.ts', 'remotion/src/scenes/IntroScene.tsx', 'remotion/src/scenes/DashboardScene.tsx', 'remotion/src/scenes/FeedScene.tsx', 'remotion/src/scenes/MissionControlScene.tsx', 'remotion/src/scenes/MetricsScene.tsx', 'remotion/src/scenes/OutroScene.tsx', 'remotion/public/fintheon-dashboard-clean.png', 'remotion/public/fintheon-riskflow.png'],
  },
  {
  ...
  },
  {
    ...
    agent: 'claude-code',
    summary: 'Hermes init: launches hermes gateway + Groq warm-up on backend startup. Gateway port default 7787→8080 (Fintheon backend). Updated .env.example with HERMES_* vars. Final OpenClaw remnant scrub.',
  ...
  },
  {
    ...
    files: ['remotion/src/theme.ts', 'remotion/src/Root.tsx', 'remotion/src/FintheonMockup.tsx', 'remotion/src/scenes/IntroScene.tsx', 'remotion/src/scenes/DashboardScene.tsx', 'remotion/src/scenes/FeedScene.tsx', 'remotion/src/scenes/MissionControlScene.tsx', 'remotion/src/scenes/MetricsScene.tsx', 'remotion/src/scenes/OutroScene.tsx'],
  },
  {
    date: '2026-03-13T23:45:00',
    agent: 'claude-code',
    summary: 'FULL OpenClaw → Hermes migration (backend). Created hermes-service.ts + hermes-handler.ts (Groq direct, no gateway middleman). Updated ai-types.ts (openclaw→hermes provider type), rewrote ai-config.ts (hermes-* model keys, HERMES_API_KEY, Groq model IDs without groq/ prefix), rewrote model-selector.ts. Updated all consumers: chat.ts, voice handlers, quick-fintheon, notion-poller, conversation-store, agent-instructions, boardroom-schedule. Deleted openclaw-service.ts + openclaw-handler.ts. Architecture: Fintheon → Groq API direct.',
    files: [
      ...
      'backend-hono/src/routes/ai/handlers/quick-fintheon.ts',
      'backend-hono/src/services/notion-poller.ts',
      'backend-hono/src/services/notion-service.ts',
      'backend-hono/src/services/ai/conversation-store.ts',
  ...
  },
  {
    ...
    agent: 'claude-code',
    summary: 'Remotion V2: Rebuilt mockup as hybrid narrative — real Playwright screenshots of live Fintheon (dashboard, RiskFlow, Mission Control, Blindspots, Performance) composited with animated overlays. 6-scene narrative arc: Hook (typewriter → logo), Signal (screenshot zoom + targeting brackets), Score (IV ring gauge + self-drawing VIX chart), Decision (ER waveform + strategy status + blindspot warnings), Edge (animated P&L counters + daily bar chart), Outro. ~26.5s @ 1920x1080 30fps.',
    files: ['remotion/src/Root.tsx', 'remotion/src/FintheonMockup.tsx', 'remotion/src/theme.ts', 'remotion/src/scenes/IntroScene.tsx', 'remotion/src/scenes/DashboardScene.tsx', 'remotion/src/scenes/FeedScene.tsx', 'remotion/src/scenes/MissionControlScene.tsx', 'remotion/src/scenes/MetricsScene.tsx', 'remotion/src/scenes/OutroScene.tsx', 'remotion/public/fintheon-dashboard-clean.png', 'remotion/public/fintheon-riskflow.png', 'remotion/public/fintheon-mission-control.png', 'remotion/public/fintheon-blindspots.png', 'remotion/public/fintheon-performance.png'],
  },
  {
    date: '2026-03-13T15:00:00',
    agent: 'claude-code',
    summary: 'Track 2: Voice engine fix — sentiment analysis pipeline dispatches psychassist:infraction events after voice interactions, recognition auto-restarts after TTS playback, enhanced waveform (7 bars + glow), global border fintheon (green=listening, gold=speaking)',
  ...
  },
  {
    ...
    agent: 'claude-code',
    summary: 'Created Remotion mockup project (remotion/) — 6-scene animated video showcasing Fintheon dashboard: intro logo reveal with gold particle convergence, three-panel dashboard assembly, live news feed with IV impact scores, Mission Control widgets (ER waveform, Account Tracker pendulum chart, Blindspots, Algo Status), performance metrics with animated counters and daily P&L bar chart, and Priced In Capital brand outro. 1920x1080 @ 30fps, ~20s total, TransitionSeries with fade/slide/wipe. Full Solvys Gold palette.',
    files: ['remotion/package.json', 'remotion/src/Root.tsx', 'remotion/src/FintheonMockup.tsx', 'remotion/src/theme.ts', 'remotion/src/scenes/IntroScene.tsx', 'remotion/src/scenes/DashboardScene.tsx', 'remotion/src/scenes/FeedScene.tsx', 'remotion/src/scenes/MissionControlScene.tsx', 'remotion/src/scenes/MetricsScene.tsx', 'remotion/src/scenes/OutroScene.tsx'],
  },
  {
    date: '2026-03-12T02:00:00',
    agent: 'claude-code',
    summary: 'Created Notion onboarding course: "How to Use Fintheon to Trade Narratives & Risk Events" — 7 modules under PIC covering foundation, news reading (second-level thinking, data cycles, distribution of expectations), RiskFlow pipeline, risk events & Playbook models (40/40 Club, Flush, Ripper, 22 VIX Fixer), divergent prepositioning & safe havens, Fintheon execution workflow, and risk management psychology (13 Commandments).',
    files: [],
  },
  {
  ...
  },
  {
    date: '2026-03-12T00:15:00',
    agent: 'claude-code',
    summary: 'Track 2 v7.7.7 — Frontend Chat UI fixes: 2a) clear skill badge after send in FintheonComposer. 2b) Image part renderer in user chat bubbles. 2c) CoT auto-open on stream, auto-close after 4s via useEffect. 2d) ResearchDepartment sidebar refactored to rounded bubble style + ReactMarkdown + FintheonChatInput (matches InterventionSidebar). 2e) Persistent thread toggle + thread ID input in Gateway settings tab, hook updated to check localStorage persistent thread.',
    files: [
      'frontend/components/chat/FintheonComposer.tsx',
  ...
  },
  {
    ...
    agent: 'claude-code',
    summary: 'Track 4 v7.7.7: Replace agent-plan.tsx with 21st.dev enhanced version — full framer-motion LayoutGroup animations, nested subtask expand/collapse, status toggling (completed/in-progress/pending/need-help/failed), priority badges, tool badges, progress bar header, dependency indicators, Fintheon theme throughout. Backwards-compatible AgentPlan/PlanTask exports preserved.',
    files: [
  ...
  },
  {
    ...
    agent: 'claude-code',
    summary: 'Track 3 v7.7.7: 3A — mcpServers field added to SkillDef interface and all SKILLS entries (exa, notion, fmp, playwright mappings). 3B — MCP auto-activation in FintheonComposer: merges skill mcpServers into localStorage fintheon_mcp_active_connectors before append. 3C — QuickFintheon prefix updated for auto-screenshot via Playwright; chat handler injects Playwright screenshot when QUICKFINTHEON skill detected and no image present. 3D — iFrame power switch guard: removed auto-enable from platform dropdown selection (power via dedicated button only). 3E — NarrativeFlow Coming Soon overlay.',
    files: [
  ...
  },
  {
    ...
    agent: 'claude-code',
    summary: 'Track 5 v7.7.7: Chat + toolbar — removed steer strip from FintheonComposer/PromptBox (always full input), added queue chips (max 2) with cancel, added /stop to FintheonSlashPicker, power button always visible in TopHeader (dim when inactive), RiskFlow drag-drop into chat (application/x-riskflow), created useVoiceMemory.ts hook (mic device persistence + transcript history), added mic device selector to SettingsPanel notifications tab',
    files: [
      'frontend/components/chat/FintheonComposer.tsx',
      'frontend/components/ui/chatgpt-prompt-input.tsx',
      'frontend/components/chat/FintheonSlashPicker.tsx',
      'frontend/lib/skills.ts',
      'frontend/components/layout/TopHeader.tsx',
      'frontend/components/RiskFlowPanel.tsx',
      'frontend/hooks/useVoiceMemory.ts',
  ...
  },
  {
    ...
    agent: 'claude-code',
    summary: 'Track 4: Chat Rebuild — PromptBox (unified input with vanish animation, image dialog, Think Harder sparkle toggle, mic button, compact mode), FintheonComposer rewired to PromptBox + API skills fetch, FintheonThread enhanced (hover action bar with Copy/Checkpoint, scroll-to-bottom IntersectionObserver, ChainOfThought gold-bordered display, fadeSlideIn animation), ChatInterface simplified, AskHarpChatPanel aligned to FintheonComposer, FintheonFloatingChat migrated from useChatSession to useOpenClawRuntime + AssistantRuntimeProvider + FintheonThread compact',
    files: [
      'frontend/components/ui/chatgpt-prompt-input.tsx',
      'frontend/components/chat/FintheonComposer.tsx',
      'frontend/components/chat/FintheonThread.tsx',
  ...
  },
  {
    date: '2026-03-10T14:00:00',
    agent: 'claude-code',
    summary: 'T4 QuickFintheon backend: screenshot-service.ts (Playwright inline-script screenshot, isPlaywrightReady check), quick-fintheon.ts handler (POST /api/ai/quick-fintheon — auto-screenshot fallback via Playwright, multimodal content → OpenClaw pma-1, JSON parse with fence-strip, auto-screenshot in response). Types: backend/src/types/quick-fintheon.ts + frontend/types/quick-fintheon.ts. Route registered in ai/index.ts. playwright added to backend package.json.',
    files: [
      'backend-hono/src/services/screenshot-service.ts',
      'backend-hono/src/routes/ai/handlers/quick-fintheon.ts',
      'backend-hono/src/types/quick-fintheon.ts',
      'frontend/types/quick-fintheon.ts',
      'backend-hono/src/routes/ai/index.ts',
      'backend-hono/package.json',
    ],
  },
  ...
  },
  {
    ...
    agent: 'claude-code',
    summary: 'Groq as OpenClaw primary model: registered Groq provider in openclaw.json, set Harper primary to groq/llama-3.3-70b-versatile (free tier, ~750 tok/s). ALL agent traffic now routes Fintheon → OpenClaw gateway → Groq. Removed direct Groq bypass — everything flows through gateway with ACP provenance. OpenRouter kept as fallback only. Zero API cost.',
    files: [
      '~/.openclaw/openclaw.json',
      'backend-hono/src/config/ai-config.ts',
      'backend-hono/src/services/ai/model-selector.ts',
  ...
  },
  {
    ...
    agent: 'claude-code',
    summary: 'ACP provenance headers (OpenClaw 3.8): gateway calls now carry X-ACP-Provenance, X-ACP-Origin-Channel, X-ACP-Origin-Session, X-ACP-Origin-Agent, and X-ACP-Trace-Id headers. Fintheon channel types defined (analysis, boardroom, intervention, voice). Default mode: meta+receipt. Prevents agents from blindly trusting unidentified inputs.',
    files: [
      'backend-hono/src/services/openclaw-service.ts',
      'backend-hono/src/services/openclaw-handler.ts',
      'backend-hono/.env',
  ...
  },
  {
    date: '2026-03-09T22:00:00',
    agent: 'claude-code',
    summary: 'Chat reliability hardening (Plan 1.3): (1) No blank assistant bubble — FintheonAssistantMessage gates on content presence, only renders once text-delta or reasoning arrives. FintheonThinkingIndicator handles visual feedback separately. (2) Per-surface session isolation — each chat surface (Analysis, AskHarp, Floating, Research) gets its own conversationId via surfaceId param, preventing context bleed across surfaces. (3) Error visibility in thread — lastError now renders as an AlertCircle banner inside FintheonThread (not just the input area), so errors are visible even when scrolled up. (4) Boardroom audit — confirmed read-only (Notion embed + intervention sidebar, no mutating tools).',
    files: [
      'frontend/components/chat/FintheonThread.tsx',
      'frontend/components/chat/useOpenClawRuntime.ts',
      'frontend/components/chat/hooks/useChatSession.ts',
  ...
  },
  {
    ...
    agent: 'claude-code',
    summary: 'Chat UX + Upload + Slash + Permissions: (1) Fixed image attachment pipeline — images now flow through ChatInputArea→FintheonComposer→useOpenClawChat→backend as multimodal content parts. (2) Replaced floating attach modal with slide-up panel anchored to composer, added drag-drop and client-side compression. (3) Slash-command launcher — typing / in textarea triggers filtered skill picker with keyboard nav (<=150ms). (4) Brain icon deep-research flow — Exa API integration with graceful fallback, thinkHarder flag wired end-to-end. (5) Permissions — feature flags config, GET /api/ai/features endpoint, server-side skill enforcement, disabled skills show reason in UI. (6) Tool-selection policy — Exa preferred before browser automation.',
    files: [
      'frontend/components/chat/ChatInputArea.tsx',
      'frontend/components/chat/FintheonComposer.tsx',
      'frontend/components/chat/FintheonChatInput.tsx',
  ...
  },
  {
    ...
    agent: 'openclaw',
    summary:
      '21st-style chat polish pass implemented directly in Fintheon (while preserving black/gold palette): upgraded Analysis + Ask Harp message cards and composer shell, refined action controls, and improved visual hierarchy/spacing for modern chat UX without changing OpenClaw transport behavior.',
    files: [
      'frontend/components/chat/FintheonChatInput.tsx',
      'frontend/components/ChatInterface.tsx',
      'frontend/components/chat/AskHarpChatPanel.tsx',
  ...
  },
  {
    ...
    agent: 'openclaw',
    summary:
      'Chat UI thinking pass (Analysis + Ask Harp): added radar fintheon indicator, expanded finance thinking phrase set, and kept thinking pane visible when reasoning is present so users can inspect model thought stream after response completes.',
    files: [
      'frontend/components/chat/FintheonThinkingIndicator.tsx',
      'frontend/components/ChatInterface.tsx',
      'frontend/components/chat/AskHarpChatPanel.tsx',
  ...
  },
  {
    ...
    agent: 'openclaw',
    summary:
      'Fixed protected API auth middleware matching so nested routes are actually authenticated (added wildcard middleware for /api/* groups, including /api/ai/* and /api/riskflow/*). This resolves Fintheon agent chat unauthorized failures where c.get("userId") was undefined in /api/ai/chat.',
  ...
  },
  {
    ...
    agent: 'claude-code',
    summary:
      'Fixed React #300 (fewer hooks): move early return after all hooks in FintheonFloatingChat. Gateway health check URL configurable via VITE_GATEWAY_URL.',
    files: ['frontend/components/chat/FintheonFloatingChat.tsx', 'frontend/contexts/GatewayContext.tsx'],
  },
  {
  ...
  },
  {
    date: '2026-03-16T22:00:00',
    agent: 'claude-code',
    summary: 'Persona selector pills in composer (FintheonComposer) — 5 agent pills with name/description/status dot, gold accent for active, replaces header-level agent dropdown. Greeting shimmer animation (ChatGreeting) — scroll-in + shimmer left-to-right + settle to 80% opacity, removed emoji/border/bg from greeting area. CSS keyframes in custom.css.',
    files: [
      'frontend/components/chat/FintheonComposer.tsx',
      'frontend/components/chat/ChatGreeting.tsx',
      'frontend/styles/custom.css',
