...
- [ ] Wire CHART toggle to page-level `chartOpen` state that renders a full-width TradingView iframe overlay, replacing all Aquarium content until toggled off
...
- `miroshark-service.ts` or any backend Aquarium service
...
2. Extract the new top container into a local subcomponent `AquariumBriefBand` within `Sanctum.tsx` (or colocate under `frontend/components/narrative/`). Match Dashboard pattern exactly.
...
6. Apply feels polish to all Aquarium cards: `bg-[var(--fintheon-surface)] border border-[var(--fintheon-accent)]/10 rounded-md px-4 py-3`, monospace for numeric values, Inter for labels at 72% opacity, no emoji, no gradient, no shadow.
...
- [ ] Aquarium loads with no top QQQ chart
...
- [ ] CHART toggle renders a full-width TradingView iframe; toggle-off restores Aquarium view
...
```
[v.04.17.1] feat: S23-T1 aquarium UI restructure + feels polish
```
