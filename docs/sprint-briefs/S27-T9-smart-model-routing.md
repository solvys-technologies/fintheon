...
- [`backend-hono/src/services/harper-handler.ts`](backend-hono/src/services/harper-handler.ts) — Harper routing (keeps `claude-opus-4-7` by default, but routing layer is where it lives now)
