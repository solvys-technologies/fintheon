...
const pLookback = input.int("Pattern Extremity Lookback", 20, {
  min: 1,
  key: "pLookback",
});
const pinSens = input.float("Pin Bar Sensitivity", 0.65, {
  min: 0.1,
  max: 0.9,
  key: "pinSens",
});
const showEngulfing = input.bool("Engulfing", true, { key: "showEng" });
const showHammer = input.bool("Hammer", true, {
  key: "showHam",
  sameLine: true,
});
const showStar = input.bool("Shooting Star", true, {
  key: "showStar",
  sameLine: true,
});
const showLabels = input.bool("Show Labels", true, { key: "showLbl" });
const bullEngCol = input.color("Bull Engulfing", "#089981", {
  key: "bullEngCol",
});
const bearEngCol = input.color("Bear Engulfing", "#f23645", {
  key: "bearEngCol",
  sameLine: true,
});
const bullPinCol = input.color("Hammer", "#089981", { key: "bullPinCol" });
const bearPinCol = input.color("Shooting Star", "#f23645", {
  key: "bearPinCol",
  sameLine: true,
});
...
const bearDivCol = input.color("Bearish Div", "#f23645", {
  key: "bearDivCol",
  sameLine: true,
});
...
function onBar(index) {
  ...
  // OB/OS lines
  plot("OB", 70, {
    color: color.transp(bearDivCol, 50),
    style: linestyle.dashed,
    width: 1,
  });
  plot("OS", 30, {
    color: color.transp(bullDivCol, 50),
    style: linestyle.dashed,
    width: 1,
  });
  ...
  if (showDiv) {
    ...
    if (!na(pivotRsiLow)) {
      ...
      if (!na(plPrice[0]) && currRsi > plRsi[0] && currPrice < plPrice[0]) {
        ...
        plotMarker("Bull Div Shape", currRsi, {
          color: bullDivCol,
          marker: shape.up,
          size: 8,
        });
    ...
    if (!na(pivotRsiHigh)) {
      ...
      if (!na(phPrice[0]) && currRsi < phRsi[0] && currPrice > phPrice[0]) {
        ...
        plotMarker("Bear Div Shape", currRsi, {
          color: bearDivCol,
          marker: shape.down,
          size: 8,
        });
  ...
  if (isNewHTF) {
    ...
    // Bullish Engulfing
    const isBullEng =
      showEngulfing && hC > hO && hPC < hPO && hC >= hPO && hO <= hPC;
    ...
    const isHammer =
      showHammer &&
      lowerWick > candleRange * pinSens &&
      math.abs(hC - hO) < candleRange * (1 - pinSens);
