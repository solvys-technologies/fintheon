...
const areaMode = input.select("Swing Area", 0, {
  selectables: ["Wick Extremity", "Full Range"],
  key: "areaLS",
});
const filterMode = input.select("Filter By", 0, {
  selectables: ["Count", "Volume"],
  key: "filterLS",
});
const filterValue = input.float("Filter Threshold", 0, {
  min: 0,
  key: "filterValLS",
});
const showTop = input.bool("Swing High", true, { key: "showTop" });
const topCss = input.color("High Color", "#f23645", {
  key: "topCss",
  sameLine: true,
});
const topAreaCss = input.color("High Area", "#f2364580", {
  key: "topAreaCss",
  sameLine: true,
});
const showBtm = input.bool("Swing Low", true, { key: "showBtm" });
const btmCss = input.color("Low Color", "#089981", {
  key: "btmCss",
  sameLine: true,
});
const btmAreaCss = input.color("Low Area", "#00808080", {
  key: "btmAreaCss",
  sameLine: true,
});
...
const colUpNeg = input.color("Up Neg Delta", "#f23645", {
  key: "colUpNeg",
  sameLine: true,
});
const colDnVD = input.color("Down", "#f23645", { key: "colDnVD" });
const colDnPos = input.color("Down Pos Delta", "#089981", {
  key: "colDnPos",
  sameLine: true,
});
const vdDisplay = input.select("Display", 1, {
  selectables: ["half bar", "full bar"],
  key: "vdDisplay",
});
const showDotVD = input.bool("Max Volume Price Point", false, {
  key: "showDotVD",
});
...
const showSignals = input.bool("Show Cross + Retest Signals", true, {
  key: "showSignals",
});
...
function onBar(index) {
  ...
  if (showVD) {
    ...
    if (vdDisplay === "half bar") {
      ...
      valueVD = posVD
        ? minOC + math.abs(normDelta) * absOC
        : maxOC - math.abs(normDelta) * absOC;
      baseVD = posVD ? minOC : maxOC;
    }

    // Delta fill candle
    const cssD =
      normDelta > 0 ? (c > o ? colUpVD : colDnPos) : c < o ? colDnVD : colUpNeg;
    const cssDTransp = color.transp(cssD, 50);

    plotCandle("Delta", baseVD, baseVD, valueVD, valueVD, {
      bodyColor: cssDTransp,
      wickColor: color.transparent,
      borderColor: color.transparent,
    });

    // Outline candle
    const cssOutline = c > o ? colUpVD : c < o ? colDnVD : color.white;
    plotCandle("Price", o, h, l, c, {
      bodyColor: color.transparent,
      wickColor: cssOutline,
      borderColor: cssOutline,
    });
  }
