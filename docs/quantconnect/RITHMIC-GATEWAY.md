...
2. **QC calls Fintheon** → `POST /api/autopilot/generate` with market context
3. **Fintheon agent pipeline** debates and produces a proposal
4. **Mode 1:** Trader approves in UI → `POST /api/autopilot/execute`
   **Mode 2:** Auto-execute if confidence ≥ threshold
5. **Fintheon** calls Rithmic Gateway → order placed
