...
export class ContextBankService {
  ...
  async getDeskReportHistory(
    desk: string,
    limit?: number,
  ): Promise<{ desk: string; reports: DeskReport[]; count: number }> {
...
export class MemoryService {
  ...
  async listShared(params?: {
    category?: string;
    search?: string;
  }): Promise<{ entries: SharedMemoryEntry[] }> {
  ...
  ): Promise<{ entry: SharedMemoryEntry }> {
    return this.client.put(
      `/api/memory/shared/${encodeURIComponent(key)}`,
      data,
    );
  }
  ...
  ): Promise<{ results: AnalysisThought[] }> {
  ...
  async getAgentHistory(
    agent: string,
    limit?: number,
  ): Promise<{ thoughts: AnalysisThought[] }> {
    const suffix = limit ? `?limit=${limit}` : "";
    return this.client.get(
      `/api/memory/analysis/agent/${encodeURIComponent(agent)}${suffix}`,
    );
  }
}
...
export class SkillsService {
  ...
  async generateTradePlan(data: {
    instrument: string;
    direction: string;
    context?: string;
  }): Promise<{ plan: any | null; reason?: string }> {
    return this.client.post("/api/skills/trade-plan", data);
  }

  async enrichProposal(
    proposalId: string,
  ): Promise<{ proposal: any | null; reason?: string }> {
...
export class EditorSidebarService {
  ...
  async executeSidebarAction(
    action: SidebarAction,
  ): Promise<{ action: SidebarAction }> {
