...
export class AIService {
  ...
  async chat(data: {
    ...
    try {
      ...
      const payload = data.messages
        ? { messages: data.messages, conversationId: data.conversationId }
        : {
  ...
  async listConversations(): Promise<any[]> {
    try {
      const response = await this.client.get<{ conversations: any[] }>(
  ...
  async generateMDBReport(): Promise<MDBReport> {
    return {
      report: {
        content:
          "MDB report generation is not yet implemented in the Hono backend.",
      },
    };
  }
}
...
export class PsychService {
  ...
  async getProfile(): Promise<PsychProfile> {
    const response = await this.client.get<{ profile: PsychProfile }>(
      "/api/psych/profile",
    );
  ...
  async updateProfile(data: {
    ...
    goal?: string | null;
    orientationComplete?: boolean;
    source?: "orientation" | "settings";
  }): Promise<PsychProfile> {
    const response = await this.client.put<{ profile: PsychProfile }>(
      "/api/psych/profile",
      data,
    );
    return response.profile;
  ...
  async updateScores(scores: Partial<PsychScores>): Promise<PsychProfile> {
    const response = await this.client.post<{ profile: PsychProfile }>(
      "/api/psych/scores",
      scores,
    );
    return response.profile;
...
export class AnalystService {
  ...
  async getReports(params?: {
    ...
    const response = await this.client.get<{ reports: AnalystReport[] }>(
      endpoint,
    );
  ...
  async runReports(data?: { instrument?: string }): Promise<AnalystReport[]> {
    const response = await this.client.post<{ reports: AnalystReport[] }>(
