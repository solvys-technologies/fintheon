...
const CRITICAL_CHECKS: Array<(t: string) => boolean> = [
  ...
  (t) =>
    t.includes("emergency") && (t.includes("fed") || t.includes("meeting")),
  (t) => t.includes("government shutdown"),
  (t) =>
    t.includes("debt ceiling") &&
    (t.includes("default") || t.includes("deadline")),
  // V3: credit/liquidity stress
  (t) =>
    t.includes("liquidity crisis") ||
    t.includes("liquidity crunch") ||
    t.includes("funding stress"),
  (t) =>
    t.includes("bank run") || (t.includes("bank") && t.includes("insolvency")),
...
const HIGH_CHECKS: Array<(t: string) => boolean> = [
  ...
  (t) =>
    ...
    (t.includes("actual") ||
      ...
      t.includes("report")),
  (t) =>
    wordMatch(t, "nfp") || (t.includes("non-farm") && t.includes("payroll")),
  (t) =>
    wordMatch(t, "gdp") &&
    (t.includes("actual") ||
      t.includes("print") ||
  ...
  (t) =>
    ...
    (t.includes("official") || t.includes("confirm") || t.includes("enter")),
  (t) =>
    wordMatch(t, "war") &&
    (t.includes("declare") ||
      t.includes("escalat") ||
  ...
  (t) =>
    ...
    (t.includes("new") || t.includes("impose") || t.includes("expand")),
  (t) =>
    t.includes("bankruptcy") &&
    (t.includes("bank") || t.includes("major") || t.includes("file")),
...
const MEDIUM_CHECKS: Array<(t: string) => boolean> = [
  ...
  (t) =>
    ...
    (t.includes("rise") ||
      ...
      t.includes("report")),
  (t) =>
    t.includes("retail sales") && (t.includes("actual") || t.includes("data")),
  (t) =>
    t.includes("unemployment") && (t.includes("rate") || t.includes("claims")),
  (t) => t.includes("jobless claims"),
  (t) =>
    wordMatch(t, "ipo") &&
    (t.includes("price") || t.includes("launch") || t.includes("debut")),
  (t) => t.includes("merger") || t.includes("acquisition"),
  (t) =>
    t.includes("opec") &&
    (t.includes("cut") || t.includes("output") || t.includes("production")),
  (t) =>
    t.includes("downgrade") &&
    (t.includes("credit") || t.includes("rating") || t.includes("analyst")),
  (t) =>
    t.includes("upgrade") &&
    (t.includes("credit") || t.includes("rating") || t.includes("analyst")),
  (t) =>
    wordMatch(t, "pce") &&
    (t.includes("actual") || t.includes("data") || t.includes("print")),
  (t) =>
    t.includes("housing") &&
    (t.includes("starts") || t.includes("data") || t.includes("sales")),
  (t) =>
    t.includes("consumer") &&
    t.includes("confidence") &&
    (t.includes("data") || t.includes("index")),
  (t) => t.includes("treasury") && t.includes("auction"),
  (t) =>
    t.includes("yield") &&
    (t.includes("surge") || t.includes("spike") || t.includes("invert")),
  // V3: credit spread and leverage signals
  (t) =>
    t.includes("credit spread") && (t.includes("widen") || t.includes("blow")),
  (t) =>
    t.includes("high yield") && (t.includes("stress") || t.includes("spread")),
  (t) =>
    t.includes("margin debt") && (t.includes("record") || t.includes("surge")),
  (t) =>
    t.includes("leverage") && (t.includes("warning") || t.includes("unwind")),
];

function classifySeverity(text: string): AlertSeverity {
  const lower = text.toLowerCase();
  for (const check of CRITICAL_CHECKS) {
    if (check(lower)) return "critical";
  }
  for (const check of HIGH_CHECKS) {
    if (check(lower)) return "high";
  }
  for (const check of MEDIUM_CHECKS) {
    if (check(lower)) return "medium";
  }
  return "low";
}
...
function inferCyclical(
  alert: RiskFlowAlert,
): "Cyclical" | "Counter-cyclical" | "Neutral" {
 ...
 */
export function downgradeNonFinancialBreaking(
  alerts: RiskFlowAlert[],
): RiskFlowAlert[] {
  for (const alert of alerts) {
    const hasBreaking =
      alert.isBreaking || /breaking|🚨|⚠️|🔴/i.test(alert.headline);
    if (!hasBreaking) continue;
    // Already low — skip
    if (alert.severity === "low") continue;
    const lower = (alert.headline + " " + (alert.summary ?? "")).toLowerCase();
    const hasFinancialTerm = FINANCIAL_TERMS.some(
      (term) => wordMatch(lower, term) || lower.includes(term),
    );
    const hasGeopoliticalTerm = GEOPOLITICAL_TERMS.some((term) =>
      lower.includes(term),
    );
