...
export function useHarperOps() {
  ...
  const fetchFeed = useCallback(async () => {
    try {
      ...
      if (!feedRes.ok || !statusRes.ok) throw new Error('Failed to fetch Harper Ops data')

      const feedData = await feedRes.json() as { entries: OpsEntry[]; total: number }
      const statusData = await statusRes.json() as { loop: LoopStatus; ops: OpsStatus }
  ...
  const triggerTask = useCallback(async (type: string, message?: string) => {
    await fetch(`${API_BASE}/api/harper-ops/trigger`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const approve = useCallback(async (id: string) => {
    await fetch(`${API_BASE}/api/harper-ops/approve/${id}`, { method: 'POST' })
    fetchFeed()
  }, [fetchFeed])
