...
interface MockMessage { id: string; role: "user" | "assistant"; content: string; createdAt: string; }
...
export function handleMockAgentRequest(request: MockAgentRequest) {
  ...
  if (request.path.startsWith("/api/ai/conversations/") && request.method === "GET") {
  ...
  const approvalMatch = request.path.match(/^\/api\/harper\/ui-actions\/([^/]+)\/answer$/);
  if (approvalMatch) {
...
export function createNarrativeFlowMockEventSource() {
  return function MockEventSource(url: string) {
    ...
    const requestId = new URL(url, window.location.href).searchParams.get("requestId");
    if (requestId) {
...
function handleMockHarperChat(body: Record<string, unknown>) {
  ...
  const workspaceTitle = stringValue(workspace?.title) ?? "NarrativeFlow workspace";
  ...
  conversations.set(conversationId, [
    ...
    { id: `${requestId}-u`, role: "user", content: message, createdAt: new Date().toISOString() },
    { id: `${requestId}-a`, role: "assistant", content: assistantText, createdAt: new Date().toISOString() },
  ]);
