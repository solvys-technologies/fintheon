// [claude-code 2026-03-15] Track 2: PsychAssist hardening — core ERProvider using useERVad hook
import React, { createContext, useContext, useState, useRef, useCallback, useEffect } from 'react';
import { useBackend } from '../../lib/backend';
import { useERVad } from './er-vad';
import type { ERState, ERContextValue, ERProviderProps, ERSnapshot, InterventionLevel } from './er-types';
import { computeInterventionLevel } from './er-types';
...
export function ERProvider({ children }: ERProviderProps) {
  ...
  const [overtradingStatus, setOvertradingStatus] = useState<{ isOvertrading: boolean; tradesInWindow: number; warning?: string } | null>(null);
  ...
  // VAD hook
  const { vadActive, lastSentiment, setLastSentiment, startVAD, stopVAD, resetSentimentTimestamp } = useERVad({
    backend,
    erScoreRef,
    updateScore,
    addInfraction,
  ...
  }, [backend, persistScore, startVAD, resetSentimentTimestamp, setLastSentiment]);
  ...
  // Cleanup infraction hold timeout
  useEffect(() => {
    return () => {
      if (infractionHoldTimeoutRef.current) {
        clearTimeout(infractionHoldTimeoutRef.current);
