...
export function BloombergChart({
  ...
  const { pnlMin, pnlMax, pnlRange, toX, toPnlY, gridLines } = useMemo(() => {
    ...
    const toX_ = (i: number) =>
      PAD.left + (i / (cumulative.length - 1)) * chartW;
    const toPnlY_ = (v: number) =>
      PAD.top + chartH - ((v - lo) / total) * chartH;
  ...
  return (
    <div className="relative bg-[var(--fintheon-surface)] border border-[var(--fintheon-accent)]/15 rounded-lg overflow-hidden">
      ...
      <div className="absolute top-2 right-2 z-10 flex items-center gap-1">
        ...
        <div className="relative">
          ...
          >
            ...
            <svg
              className={`w-2 h-2 transition-transform ${filterOpen ? "rotate-180" : ""}`}
              fill="currentColor"
              viewBox="0 0 12 12"
            >
              <path
      ...
      >
        ...
        {/* Gradient area fill */}
        {hasData && <path d={areaPath} fill="url(#pnl-area-gradient)" />}
        ...
        {hoverIdx !== null && hasData && (
          <>
            ...
            <text
              ...
              fontFamily="'Geist Mono', monospace"
            >
              {formatDollar(cumulative[hoverIdx])} |{" "}
              {data[hoverIdx] >= 0 ? "+" : ""}
              {formatDollar(data[hoverIdx])}
            </text>
          </>
        )}
        ...
        {!hasData && (
          <>
            <line
              x1={PAD.left}
              y1={H / 2 - VOLUME_HEIGHT / 2}
              x2={W - PAD.right}
              y2={H / 2 - VOLUME_HEIGHT / 2}
              stroke="#c79f4a"
              strokeOpacity={0.15}
              strokeDasharray="6 4"
            />
