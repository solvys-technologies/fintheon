...
function DisciplineGauge({ score }: { score: number }) {
  ...
  const color =
    pct >= 80 ? "#34D399" : pct >= 50 ? "var(--fintheon-accent)" : "#EF4444";
          ...
          <div className="space-y-1.5">
            ...
            {!agentSummary && (
              <p className="text-zinc-500 italic">
                Hermes will auto-generate a summary from today's trading
                activity.
              </p>
            )}
          </div>
        ) : (
          <p className="text-zinc-600 italic py-1">
            No session data yet. Trade to generate a summary.
          </p>
        )}
      </div>
    </div>
...
function UserNotesBlock({
  ...
  return (
    <div
      className={`bg-black/30 border rounded-lg overflow-hidden transition-colors duration-500 ${
...
export function SessionNotesPanel({
  ...
  useEffect(() => {
    if (todayEntry?.emotionalControlRating)
      setEmotionalRating(todayEntry.emotionalControlRating);
  }, [todayEntry]);

  // ER-derived score
  const erDerivedScore = er
    ? Math.max(1, Math.min(10, Math.round(5 + er.erScore * 2.5)))
    : null;
    ...
    try {
      ...
      const discScore = er
        ? Math.max(
            ...
            Math.min(
              100,
              Math.round(50 + er.erScore * 5 - er.infractionCount * 10),
            ),
          )
        : undefined;
    ...
    <div className="flex gap-3">
      ...
      <div className="flex-1 bg-[var(--fintheon-surface)] border border-[var(--fintheon-accent)]/15 rounded-lg p-3 flex flex-col gap-2.5">
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm font-mono text-[var(--fintheon-text)]">
            {todayInfractions}
          </span>
        </div>
        ...
        <div>
          <div className="flex items-center gap-1.5 mb-1">
            ...
            {erDerivedScore && (
              <span className="text-[9px] text-[var(--fintheon-accent)] font-mono ml-auto">
                {erDerivedScore}/10
              </span>
            )}
          </div>
          ...
          <div className="flex justify-between text-[9px] text-[var(--fintheon-muted)] mt-0.5">
            <span>Tilted</span>
            <span className="font-mono text-[var(--fintheon-text)]">
              {emotionalRating ?? "--"}
            </span>
            <span>Composed</span>
          </div>
