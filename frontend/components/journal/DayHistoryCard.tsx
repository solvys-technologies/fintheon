...
export function DayHistoryCard({
  ...
  const erColor =
    erScore != null
      ? erScore >= 7 ? '#34D399' : erScore >= 4 ? 'var(--fintheon-accent)' : '#EF4444'
      : 'var(--fintheon-muted)';
  ...
  return (
    <div className="bg-[var(--fintheon-surface)] border border-[var(--fintheon-accent)]/10 rounded-lg p-3 flex gap-3">
      ...
      <div className="flex flex-col items-start min-w-[80px]">
        ...
        {/* P&L */}
        <div className="text-base font-bold font-mono mt-2" style={{ color: pnlColor }}>
          {pnlStr}
        </div>

        {/* Instrument close */}
        {instrumentClose && (
          <div className="text-[10px] font-mono mt-1" style={{ color: 'var(--fintheon-accent)' }}>
            {instrumentClose.name}{' '}
            <span style={{ color: instrumentClose.points >= 0 ? '#34D399' : '#EF4444' }}>
              {instrumentClose.points >= 0 ? '+' : ''}{instrumentClose.points} pts
            </span>
            <span className="text-[var(--fintheon-muted)]">
              {' '}({instrumentClose.pctChange >= 0 ? '+' : ''}{instrumentClose.pctChange.toFixed(1)}%)
            </span>
          </div>
        )}
        ...
        {isAgentView && (
          <div className="mt-1.5 text-[9px] text-[var(--fintheon-muted)]">
            {agentName && <div className="text-[var(--fintheon-accent)]">{agentName}</div>}
            {proposalCount != null && <div>{proposalCount} proposals</div>}
            {winRate != null && (
              <div className="font-mono" style={{ color: winRate >= 50 ? '#34D399' : '#EF4444' }}>
      ...
      <div className="flex-1 flex flex-col justify-between min-w-0">
        <div className="text-[11px] text-[var(--fintheon-text)] text-right leading-relaxed line-clamp-4">
          {notes || (
            <span className="text-[var(--fintheon-muted)] italic">No notes recorded</span>
          )}
        </div>
