...
function actionIcon(type: string) {
  switch (type) {
    case 'heartbeat': return <Heart className="w-3 h-3 text-emerald-400" />
    case 'alert': return <AlertTriangle className="w-3 h-3 text-amber-400" />
    case 'error': return <X className="w-3 h-3 text-red-400" />
    default: return <Zap className="w-3 h-3 text-[var(--fintheon-accent)]" />
  }
}
...
function OpsFeedItem({ entry, onApprove, onDeny }: {
  entry: OpsEntry
    ...
    <div className="border-b border-[var(--fintheon-accent)]/8 py-2 px-3 hover:bg-[var(--fintheon-accent)]/[0.03] transition-colors">
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className={`text-[10px] font-mono ${severityColor(entry.severity)}`}>
      ...
      {entry.requiresApproval && entry.approvalStatus === 'pending' && (
        <div className="mt-2 ml-5 flex gap-2">
          ...
          <button
            onClick={(e) => { e.stopPropagation(); onDeny(entry.id) }}
...
export function HarperOpsPanel() {
  const {
    ...
    triggerHeartbeat, triggerTask, approve, deny,
  } = useHarperOps()
    ...
    <div className="h-full flex flex-col text-[var(--fintheon-text)]" style={{ fontFamily: 'var(--font-body)' }}>
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-[var(--fintheon-accent)]/8 bg-[var(--fintheon-accent)]/[0.02]">
        ...
        <div className="flex items-center gap-3 text-[9px] font-mono text-[var(--fintheon-muted)]">
          ...
          {pendingCount > 0 && (
            <span className="text-[var(--fintheon-accent)]">{pendingCount} pending</span>
          )}
        </div>
      </div>
