...
const MODELS: ModelEntry[] = [
  {
    ...
    whenToUse:
      "High IV environments with a clear slope change on the 40 DMA. Best after extended consolidation or a volatility squeeze.",
    riskProfile:
      "Moderate — tight stops at the prior swing, 2:1 minimum R:R. Sized at 1-2 contracts.",
  },
  {
    name: "Ripper",
    description:
    ...
    whenToUse:
      "Trending days with clear directional bias. Avoid chop and range-bound sessions. Best during RTH after the first 30 minutes.",
    riskProfile:
      "Aggressive — wider stops behind structure, targets 3:1+ R:R. Can pyramid on confirmation.",
  },
  {
    ...
    whenToUse:
      "VIX above 20 with steep put skew. Works best during macro fear events that overshoot. Avoid when vol is structurally elevated (e.g. earnings season).",
    riskProfile:
      "Conservative — small position, wide stop, 4:1+ R:R. Designed for infrequent, high-conviction setups.",
  },
  {
    name: "Snipe",
    description:
    ...
    whenToUse:
      "Any session with defined levels. Best when GEX walls and options flow align with a technical level. Avoid during low-volume holiday sessions.",
    riskProfile:
      "High frequency, low risk per trade — 3-5 tick stops, 1:1 to 2:1 R:R. Volume is the edge, not size.",
  },
];
        ...
        {MODELS.map((model, idx) => {
          ...
          return (
            <div key={model.name}>
              ...
              >
                <span className="text-[11px] font-semibold text-[#f0ead6]">
                  {model.name}
                </span>
                <ChevronDown
              ...
              >
                <div className="px-3 pb-2.5 space-y-2 border-t border-[#c79f4a]/10">
                  <p className="text-[11px] text-[#f0ead6]/60 leading-relaxed pt-2">
                    {model.description}
                  </p>
                  <div>
                    <div className="text-[9px] text-[#f0ead6]/30 uppercase tracking-wider">
                      When to Use
                    </div>
                    <p className="text-[10px] text-[#f0ead6]/40 leading-relaxed">
                      {model.whenToUse}
                    </p>
                  </div>
                  <div>
