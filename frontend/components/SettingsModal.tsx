...
export default function SettingsModal({ onClose }: SettingsModalProps) {
  ...
  const [provider, setProvider] = useState<
    "projectx" | "alpaca" | "interactive_brokers"
  >("projectx");
  ...
  const handleSaveConnection = async () => {
    ...
    try {
      setMessage({
        text: "Broker connection management coming soon!",
        type: "success",
      });
            ...
            <div className="bg-[#140a00] rounded-lg p-4 space-y-4">
              <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-4">
                <h4 className="text-sm font-medium text-blue-400 mb-2">
                  Configure ProjectX Credentials
                </h4>
                <p className="text-xs text-zinc-400 mb-3">
                  This terminal trades exclusively on ProjectX (TopStepX) prop
                  firm accounts. Configure your ProjectX credentials in Leap
                  Settings (sidebar) to enable trading.
                </p>
                <div className="space-y-2 text-xs text-zinc-400">
                  ...
                  <div className="ml-3">
                    •{" "}
                    <code className="text-[var(--fintheon-accent)]">
                      ProjectXUsername
                    </code>{" "}
                ...
                <p className="text-xs text-zinc-500 mt-3">
                  Once configured, use the "Establish Uplink" button in Mission
                  Control to connect your account.
                </p>
              </div>
            </div>
          </div>
