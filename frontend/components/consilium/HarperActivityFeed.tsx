  ...
  return (
    ...
    >
      <div className="flex items-start gap-2">
        ...
        <span className={`mt-1.5 h-1.5 w-1.5 flex-shrink-0 rounded-full ${SEVERITY_DOT[entry.severity] ?? SEVERITY_DOT.info}`} />
        <span className="text-xs text-[#f0ead6]/70 leading-snug line-clamp-2">{entry.title}</span>
      </div>
    ...
    <div className="flex h-full flex-col bg-[var(--fintheon-bg)]">
      ...
      <div className="flex items-center justify-between border-b border-[#c79f4a]/10 px-3 py-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-medium text-[#f0ead6]/60">Harper Activity</span>
          <span className={`h-1.5 w-1.5 rounded-full ${isAlive ? 'bg-emerald-400 animate-pulse' : 'bg-[#f0ead6]/20'}`} />
        </div>
        <div className="flex items-center gap-2">
          <span className="text-[10px] tabular-nums text-[#f0ead6]/25">{feed.length}</span>
          {onCollapse && (
            <button
              onClick={onCollapse}
      ...
      <div className="flex-1 overflow-y-auto">
        {loading && feed.length === 0 ? (
          ...
          <div className="flex items-center justify-center py-8">
            <span className="text-xs text-[#f0ead6]/20">No recent activity</span>
          </div>
        ) : (
