...
export function BlindspotsInterview({
  ...
  const canProceed = () => {
    switch (step) {
      ...
      case 2:
        return data.roadblocks.length > 0 || data.customRoadblock.trim().length > 0;
      case 3:
        return true;
      default:
        return true;
        ...
        <div className="px-5 py-5 min-h-[220px]">
          {step === 0 && (
            <div className="space-y-4">
              <div>
                <label className="text-sm font-medium text-white block mb-2">What's your name?</label>
                <input
                  type="text"
                  value={data.name}
              ...
              <div>
                <label className="text-sm font-medium text-white block mb-2">Discord username</label>
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 text-sm">@</span>
                  <input
                    type="text"
                    value={data.discord}
                    onChange={(e) => setData((d) => ({ ...d, discord: e.target.value }))}
          ...
          {step === 1 && (
            <div>
              <label className="text-sm font-medium text-white block mb-3">What do you trade?</label>
          ...
          {step === 2 && (
            <div>
              ...
              <input
                ...
                onChange={(e) => setData((d) => ({ ...d, customRoadblock: e.target.value }))}
          ...
          {step === 3 && (
            <div className="space-y-4">
              <label className="text-sm font-medium text-white block">Trading goals</label>
              <div>
                <label className="text-xs text-gray-400 block mb-1">Daily P&L target ($)</label>
                <input
                  type="text"
                  value={data.dailyTarget}
                  onChange={(e) => setData((d) => ({ ...d, dailyTarget: e.target.value }))}
                  placeholder="e.g. 500"
                  className="w-full px-3 py-2 rounded-lg bg-black/30 border border-white/10 text-white text-sm placeholder-gray-500 focus:border-[var(--fintheon-accent)]/50 focus:outline-none transition-colors"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setData((d) => ({ ...d, weeklyGoal: e.target.value }))}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setData((d) => ({ ...d, accountSize: e.target.value }))}
