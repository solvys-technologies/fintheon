...
import { X, CheckCircle, AlertTriangle, XCircle, RefreshCw, ExternalLink, Terminal } from 'lucide-react';
...
export function SetupWizard({ visible, onClose }: SetupWizardProps) {
  ...
  const runChecks = useCallback(async () => {
    const results: CheckStatus[] = [...checks].map((c) => ({ ...c, status: 'checking' as const }));
    setChecks([...results]);
    ...
    try {
      ...
      if (res.ok) {
        ...
        results[1].status = data.openRouterKey || data.hermesReady ? 'ok' : 'warn';
      } else {
    ...
    try {
      const res = await fetch('http://localhost:8080/api/market-data/iv-score', { signal: AbortSignal.timeout(3000) });
      ...
      <div className="w-[460px] bg-[var(--fintheon-surface)] border border-[var(--fintheon-accent)]/30 rounded-xl shadow-2xl overflow-hidden">
        ...
        <div className="px-5 py-4 space-y-3">
          {checks.map((check, idx) => (
            <div key={check.label} className="flex items-start gap-3 p-3 rounded-lg bg-black/20 border border-white/5">
