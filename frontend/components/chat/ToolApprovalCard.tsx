...
import { Shield, ShieldCheck, ShieldX, Terminal, FileText, Globe, FolderCog, PenLine } from 'lucide-react'
...
function getToolMeta(toolName: string) {
  return TOOL_META[toolName] ?? { icon: Shield, label: toolName, color: '#9CA3AF' }
}
...
export function ToolApprovalCard({ approval, onApprove, onDeny }: Props) {
  ...
  const statusColor = isPending ? meta.color : isApproved ? '#34D399' : '#EF4444'
  const statusLabel = isPending ? 'Permission Required' : isApproved ? 'Approved — Permanent' : 'Denied'
