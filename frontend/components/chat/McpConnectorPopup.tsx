...
const CATEGORY_ORDER: McpServerConfig["category"][] = [
  "internal",
  "trading",
  "data",
  "search",
...
function StatusDot({ server }: { server: McpServerConfig }) {
  if (!server.installed) {
    return (
      <span
        className="w-1.5 h-1.5 rounded-full bg-red-500/80 flex-shrink-0"
        title="Not installed"
      />
    );
  }
  if (server.requiresApiKey && !server.hasApiKey) {
    return (
      <span
        className="w-1.5 h-1.5 rounded-full bg-yellow-500/80 flex-shrink-0"
  ...
  return (
    <span
      className="w-1.5 h-1.5 rounded-full bg-emerald-500/80 flex-shrink-0"
      title="Ready"
    />
  );
}
...
export function McpConnectorPopup({
  open,
  servers,
  activeIds,
  onToggle,
  onClose,
}: McpConnectorPopupProps) {
  const activeCount = activeIds.length;
  const totalCount = servers.length;

  // Group by category, preserve CATEGORY_ORDER
  const grouped = CATEGORY_ORDER.reduce<Record<string, McpServerConfig[]>>(
    (acc, cat) => {
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 mb-0.5">
                        <span className="text-[12px] font-semibold text-[var(--fintheon-text)] truncate">
                          {server.name}
                        </span>
        ...
        <div className="px-3 py-2 border-t border-[var(--fintheon-accent)]/10 flex items-center justify-between">
          ...
          {activeCount === 0 && (
            <span className="text-[10px] text-yellow-500/60">
              No connectors active — tools unavailable
            </span>
          )}
        </div>
      )}
