...
export function AssistantMessagePrimitive({
  ...
  return (
    <div style={{ display: "flex", justifyContent: "flex-start" }}>
      <div style={{ maxWidth: "85%", border: "1px solid #27272a", borderRadius: 8, padding: "10px 12px" }}>
        {agentName ? <div style={{ fontSize: 11, color: "#a1a1aa", marginBottom: 6 }}>{agentName}</div> : null}
        <div style={{ whiteSpace: "pre-wrap", fontSize: 13, color: "var(--fintheon-text)" }}>
          {text || (isStreaming ? "…" : "")}
        </div>
        {citations.length > 0 ? (
        ...
        {onTakeNote && messageId && text ? (
          <button
            ...
            style={{ marginTop: 8, fontSize: 11, color: "var(--fintheon-accent)", background: "transparent", border: "none" }}
          >
            Take Note
          </button>
        ) : null}
