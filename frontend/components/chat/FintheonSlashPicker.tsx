...
export function FintheonSlashPicker({
  query,
  onSelect,
  onDismiss,
  onStop,
  disabledSkills,
}: FintheonSlashPickerProps) {
            ...
            >
              <div className="flex-shrink-0">
                {disabled ? (
                  ...
                  <Icon
                    size={14}
                    style={{ color: highlighted ? skill.color : "#6B7280" }}
                  />
                )}
              </div>
              <div className="flex-1 text-left min-w-0">
                <span
                  className={`text-[12px] font-medium ${highlighted && !disabled ? "text-white" : "text-gray-400"}`}
                >
                  /{skill.id}
                </span>
