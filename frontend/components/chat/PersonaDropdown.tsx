  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        <ChevronDown size={10} className={`text-zinc-500 transition-transform ${open ? 'rotate-180' : ''}`} />
      </button>
        ...
        >
          {agents.map((agent) => {
            ...
            return (
              <button
                key={agent.id}
                onClick={() => { setActiveAgent(agent); setOpen(false); }}
                className={[
              ...
              >
                ...
                <div className="flex flex-col leading-none min-w-0">
                  <span className={`text-[11px] font-semibold truncate ${isActive ? 'text-[var(--fintheon-accent)]' : 'text-zinc-300'}`}>
                    {agent.name}
                  </span>
                  <span className="text-[9px] text-zinc-500 mt-[2px]">{sectorLabel}</span>
                </div>
              </button>
            );
          })}
