// S38-T1: Dispatch removed — FCB is a simple chat toggle.
...
function FloatingInner({
  ...
  return (
    <div
      className="fintheon-popover-surface fixed z-[90] flex flex-col rounded-2xl border border-[var(--fintheon-accent)]/20 bg-[var(--fintheon-surface)] shadow-2xl overflow-hidden"
...
export function FintheonFloatingChat({
  ...
  /* Collapsed state — simple FCB pill */
  if (!expanded) {
    return (
      <button
        onClick={() => setExpanded(true)}
        className="fixed z-[90] flex items-center justify-center rounded-full bg-[var(--fintheon-accent)] text-black hover:bg-[#C5A030] transition-all"
