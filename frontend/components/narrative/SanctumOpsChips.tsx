...
export function SanctumOpsChips() {
  ...
  const mirosharkColor =
    status.miroshark.status === "stale"
      ? "var(--fintheon-neutral-severe)"
      : status.miroshark.status === "ok"
        ? "var(--fintheon-low)"
        : "var(--fintheon-muted)";
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <div
        ...
        title={
          status.miroshark.lastRunAt
            ? `Aquarium last ran ${new Date(status.miroshark.lastRunAt).toLocaleString()}`
            : "Aquarium has not run yet"
        }
      >
        <Activity
          size={10}
          style={{ color: mirosharkColor }}
          className={status.miroshark.status === "ok" ? "animate-pulse" : ""}
        />
        <span
        ...
        <span
          className="font-bold"
          style={{
            color: mirosharkColor,
            fontFamily: "Doto, ui-monospace, monospace",
            letterSpacing: "0.02em",
          }}
        >
          {formatAgo(status.miroshark.ageMinutes)}
        </span>
      </div>
