...
const SORT_OPTIONS: {
  value: "severity" | "date" | "health" | null;
  label: string;
}[] = [
...
export function SanctumFilterPanel({
  open,
  onClose,
  anchorRef,
}: SanctumFilterPanelProps) {
  ...
  const handleSentimentChange = useCallback(
    (sentiment: CatalystSentiment | "all") => {
  ...
  const handleSortChange = useCallback(
    (sort: "severity" | "date" | "health" | null) => {
    ...
    >
      ...
      <div className="px-3 pb-2 space-y-1.5">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-[var(--fintheon-text)]/60">
            Hub → Catalyst (structural)
          </span>
        </div>
        <div className="flex items-center gap-2">
          <RopeLegendLine width={2} color="#3B82F6" opacity={0.3} />
          <span className="text-[10px] text-[var(--fintheon-text)]/60">
            Same-narrative connection
          </span>
        </div>
        <div className="flex items-center gap-2">
          <RopeLegendLine width={2} color="#c79f4a" opacity={0.25} />
          <span className="text-[10px] text-[var(--fintheon-text)]/60">
            Cross-narrative (gold)
          </span>
        </div>
        <p className="text-[9px] text-[var(--fintheon-text)]/30 italic">
