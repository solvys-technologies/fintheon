  ...
  return (
    <div
      ...
      style={{
        ...
        opacity,
        boxShadow: isSelected ? '0 0 12px rgba(199,159,74,0.15)' : '0 2px 6px rgba(0,0,0,0.2)',
        ...staggerDelay(staggerIndex),
      }}
      onClick={onClick}
      onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.transform = `scale(${scale}) translateY(-1px)`; }}
      onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.transform = `scale(${scale})`; }}
    >
      {/* Imported indicator */}
      {isImported && (
        <span className="absolute top-1 right-1 w-1.5 h-1.5 rounded-full" style={{ backgroundColor: 'var(--fintheon-accent)' }} />
      )}

      {/* Row 1: severity dot + title */}
      <div className="flex items-start gap-1">
        <span className="mt-0.5 w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ backgroundColor: SEVERITY_BORDER[card.severity] }} />
        <p className="text-[10px] font-semibold leading-tight truncate" style={{ color: 'var(--fintheon-text)' }}>
          {card.title}
        </p>
      </div>
      ...
      <div className="flex items-center gap-1 mt-1">
        <span className="text-[9px] font-mono font-bold" style={{ color: dir.color }}>{dir.char}</span>
        <span className="text-[8px] font-mono uppercase" style={{ color: dir.color }}>
          {card.directionBias ?? card.sentiment}
        </span>
        {instrument && (
          <span className="text-[8px] font-mono" style={{ color: 'var(--fintheon-muted)' }}>{instrument}</span>
        )}
        <span className="text-[8px] ml-auto" style={{ color: 'var(--fintheon-muted)' }}>
          {timeAgo(card.date)}
        </span>
      </div>
      ...
      <div className="flex items-center gap-1 mt-1">
        {card.category && (
          <span className="text-[7px] font-mono uppercase px-1 py-0.5 rounded" style={{
            ...
            backgroundColor: 'color-mix(in srgb, var(--fintheon-muted) 12%, transparent)',
          }}>
            {card.category}
          </span>
        )}
