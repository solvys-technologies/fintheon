...
export function CategoryScoreCard({
  ...
  const deltaColor =
    delta > 0 ? "#EF4444" : delta < 0 ? "#34D399" : "var(--fintheon-muted)";
  const deltaSign = delta > 0 ? "+" : "";
  const confPct = Math.round(confidence * 100);
  const confColor =
    confPct >= 70 ? "#34D399" : confPct >= 50 ? "#F59E0B" : "#EF4444";
    ...
    >
      ...
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-center gap-2">
          ...
          />
          <span className="text-[11px] font-mono text-[var(--fintheon-text)] uppercase tracking-wider">
            {label}
          </span>
        </div>
        <span
