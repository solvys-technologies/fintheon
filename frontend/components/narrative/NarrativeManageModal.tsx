...
export function NarrativeTimelineModal({ open, onClose }: NarrativeTimelineModalProps) {
  const { state, dispatch } = useNarrative();
  const [tagInput, setTagInput] = useState<Record<string, string>>({});
  const [isClosing, setIsClosing] = useState(false);

  const sorted = useMemo(
    () => [...state.catalysts].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()),
    [state.catalysts],
  );
  ...
  const updateCatalyst = useCallback((id: string, updates: Partial<CatalystCard>) => {
        ...
        <div className="flex-1 overflow-y-auto py-4 px-4">
          <div className="relative pl-8">
            ...
            <div
              ...
              style={{ backgroundColor: 'color-mix(in srgb, var(--fintheon-accent) 30%, transparent)' }}
            />

            {sorted.length === 0 && (
              <p className="text-center text-xs py-8 ml-4" style={{ color: 'var(--fintheon-muted)' }}>
            ...
            {sorted.map(catalyst => (
              <div key={catalyst.id} className="relative mb-4">
                ...
                <div
                  ...
                  style={{
                    backgroundColor: catalyst.sentiment === 'bullish'
                      ? 'var(--fintheon-bullish)' : 'var(--fintheon-bearish)',
                ...
                >
                  ...
                  <input
                    ...
                    onChange={e => updateCatalyst(catalyst.id, { title: e.target.value })}
                  ...
                  <div className="relative">
                    <textarea
                      ...
                      onChange={e => updateCatalyst(catalyst.id, { description: e.target.value })}
                  ...
                  <div className="flex flex-wrap items-center gap-1">
                    ...
                    <input
                      ...
                      onChange={e => setTagInput(prev => ({ ...prev, [catalyst.id]: e.target.value }))}
                      onKeyDown={e => { if (e.key === 'Enter') addTag(catalyst.id); }}
                      placeholder="+ tag"
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      onChange={e => updateCatalyst(catalyst.id, { date: e.target.value })}
                    ...
                    <select
                      ...
                      onChange={e => updateCatalyst(catalyst.id, {
                        category: (e.target.value || undefined) as NarrativeCategory | undefined,
                      })}
