        ...
        <div className="border-t border-[var(--fintheon-border)]/10 px-3 py-2.5 flex flex-col gap-2">
          ...
          {item.econ_data && (
            <div className="grid grid-cols-4 gap-2">
              {item.econ_data.actual != null && (
                <div>
                  <span className="text-[7px] text-[var(--fintheon-muted)]/40 uppercase block">
                    Actual
                  </span>
                  <span className="text-[10px] font-mono text-[var(--fintheon-text)]">
                    {item.econ_data.actual}
                  </span>
                </div>
              )}
              ...
              {item.econ_data.previous != null && (
                <div>
                  ...
                  <span className="text-[10px] font-mono text-[var(--fintheon-text)]/50">
                    {item.econ_data.previous}
                  </span>
                </div>
              )}
              {item.econ_data.beatMiss && (
                <div>
                  ...
                  <span
                    ...
                    style={{
                      color:
                        item.econ_data.beatMiss === "beat"
                          ? "var(--fintheon-low)"
                          : item.econ_data.beatMiss === "miss"
                            ? "var(--fintheon-severe)"
                            : "var(--fintheon-neutral-severe)",
                    }}
                  >
                    {item.econ_data.beatMiss.toUpperCase()}
                    {item.econ_data.surprisePercent != null &&
                      ` ${item.econ_data.surprisePercent > 0 ? "+" : ""}${item.econ_data.surprisePercent.toFixed(1)}%`}
                  </span>
                </div>
              )}
            </div>
          ...
          {item.sub_scores && (
            <div>
              ...
              <div className="flex flex-wrap gap-x-3 gap-y-0.5 text-[9px] font-mono text-[var(--fintheon-muted)]/50">
                {item.sub_scores.eventWeight != null && (
                  <span>E:{fmt(item.sub_scores.eventWeight)}</span>
                )}
                {item.sub_scores.timing != null && (
                  <span>T:{fmt(item.sub_scores.timing)}</span>
                )}
                {item.sub_scores.deviation != null && (
                  <span>D:{fmt(item.sub_scores.deviation)}</span>
                )}
                {item.sub_scores.momentum != null && (
                  <span>M:{fmt(item.sub_scores.momentum)}</span>
                )}
                {item.sub_scores.vixMultiplier != null &&
                  item.sub_scores.vixMultiplier !== 1 && (
                    <span className="text-[var(--fintheon-accent)]">
                      VIX:{item.sub_scores.vixMultiplier.toFixed(2)}x
                    </span>
                  )}
                {item.sub_scores.regimeName && (
                  <span>
                    ...
                    item.sub_scores.regimeMultiplier !== 1
                      ? ` (${item.sub_scores.regimeMultiplier.toFixed(2)}x)`
                      : ""}
                  </span>
                )}
                {item.sub_scores.speaker && (
                  <span className="text-[var(--fintheon-accent)]/60">
                    {item.sub_scores.speaker}
                    {item.sub_scores.commentatorMultiplier != null &&
                    item.sub_scores.commentatorMultiplier !== 1
                      ? ` (${item.sub_scores.commentatorMultiplier.toFixed(2)}x)`
                      : ""}
                  </span>
                )}
              </div>
          ...
          {item.price_brain_score && (
            <div className="flex items-center gap-3 text-[9px] font-mono text-[var(--fintheon-muted)]/50">
              {item.price_brain_score.sentiment && (
                <span>Bias: {item.price_brain_score.sentiment}</span>
              )}
              {item.price_brain_score.classification && (
                <span>{item.price_brain_score.classification}</span>
              )}
              {item.price_brain_score.instrument && (
                <span>{item.price_brain_score.instrument}</span>
              )}
            </div>
          )}
...
export function SanctumRiskAssessment({
  riskflowItems,
  categoryScores,
}: SanctumRiskAssessmentProps) {
          ...
          {relevantScores.map((cs) => (
            ...
            >
              <div
                ...
                style={{ backgroundColor: ivHeatColor(cs.ivScore) }}
              />
              ...
              <span
                ...
                style={{ color: ivHeatColor(cs.ivScore) }}
              >
      ...
      {[...grouped.entries()].map(([type, items]) => (
        <div key={type}>
          <div className="flex items-center gap-2 mb-1.5">
            <span className="text-[8px] text-[var(--fintheon-muted)]/40 font-mono uppercase tracking-wider">
              {type}
            </span>
            <span className="text-[8px] text-[var(--fintheon-muted)]/25 font-mono">
              ({items.length})
            </span>
