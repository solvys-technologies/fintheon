...
import type { SimulationContext } from "../../types/agent-desk";
...
const INDICATORS: IndicatorDef[] = [
  {
    ...
    unit: "",
    stress: (v) =>
      v >= 30 ? "high" : v >= 20 ? "elevated" : v >= 15 ? "moderate" : "low",
  },
  {
    key: "hyOasSpread",
    label: "HY OAS",
    unit: "%",
    stress: (v) =>
      v >= 5 ? "high" : v >= 4 ? "elevated" : v >= 3 ? "moderate" : "low",
  },
  {
    key: "yieldCurve2s10s",
    label: "2s10s",
    unit: "%",
    stress: (v) =>
      v < 0 ? "high" : v < 0.3 ? "elevated" : v < 0.8 ? "moderate" : "low",
  },
  {
    key: "tedSpread",
    label: "TED",
    ...
    <div className="flex items-center gap-2 flex-wrap">
      ...
      {context.fredFetchedAt && (
        <span className="text-[8px] text-[var(--fintheon-muted)]/25 font-mono ml-1">
          FRED:{" "}
          {new Date(context.fredFetchedAt).toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
          })}
        </span>
