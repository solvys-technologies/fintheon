...
import {
  matchCandidatesToLanes,
  type MatchedCandidate,
} from "../../lib/narrative-matcher";
...
export function RiskFlowImportModal({
  open,
  onClose,
  onImport,
  lanes,
}: RiskFlowImportModalProps) {
  ...
  const handleScoreRiskflow = useCallback(async () => {
    ...
    try {
      ...
      const { scored, provider: p } =
        await baseBackend.narrative.scoreRiskflow(mapped);
  ...
  const handleScoreBrief = useCallback(async () => {
    ...
    try {
      ...
      const briefText = res.items
        ...
        .join("\n\n");
      const { scored, provider: p } =
        await baseBackend.narrative.scoreBrief(briefText);
      const matched = matchCandidatesToLanes(scored, lanes);
      setCandidates(matched);
      setSelected(new Set());
      setProvider(p);
  ...
  const selectAbove70 = useCallback(() => {
    setSelected(
      new Set(
        candidates.filter((c) => c.notabilityScore > 70).map((c) => c.sourceId),
      ),
    );
  }, [candidates]);
  ...
  const handleImport = useCallback(() => {
    const catalysts = candidates
      ...
      .map((c) => ({
        ...
        severity: c.severity,
        source: (c.sourceType === "riskflow"
          ? "riskflow"
          : "brief") as CatalystSource,
      ...
      >
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-5 py-3">
          ...
          {candidates.length === 0 && !loading && (
            <div className="flex flex-col items-center justify-center py-8 gap-3">
              ...
              <button
                onClick={
                  tab === "riskflow" ? handleScoreRiskflow : handleScoreBrief
                }
          ...
          {loading && (
            <div className="flex items-center justify-center py-12">
              ...
              <span className="ml-2 text-xs text-[var(--fintheon-muted)]">
                Scoring...
              </span>
            </div>
          )}
          ...
          {candidates.length > 0 && !loading && (
            <>
              ...
              <div className="flex items-center gap-3 mb-2">
                ...
                <button
                  onClick={selectAll}
                  className="text-[10px] text-[var(--fintheon-muted)] hover:text-[var(--fintheon-text)]"
                >
                  Select All
                </button>
                <button
              ...
              <div className="flex flex-col gap-1">
                {candidates.map((c) => {
                  const sev =
                    SEVERITY_LABELS[c.severity] || SEVERITY_LABELS.low;
                  const laneName =
                    c.matchedLaneIds.length > 0
                      ? c.matchedLaneIds
                          .map((id) => laneMap.get(id) || id)
                          .join(", ")
                      : "Unassigned";
                  ...
                  return (
                    ...
                    >
                      ...
                      <div
                        ...
                        style={{
                          borderColor: selected.has(c.sourceId)
                            ? "var(--fintheon-accent)"
                            : "var(--fintheon-border)",
                          backgroundColor: selected.has(c.sourceId)
                            ? "var(--fintheon-accent)"
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t border-[var(--fintheon-border)]/20">
          <span className="text-[10px] text-[var(--fintheon-muted)]">
            {candidates.length > 0
              ? `${candidates.length} items scored via ${provider}`
              : ""}
          </span>
          <button
