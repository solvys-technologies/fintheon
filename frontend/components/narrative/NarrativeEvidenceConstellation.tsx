...
export function NarrativeEvidenceConstellation({
  ...
  return (
    <section className="t-panel-slide flex h-full min-h-0 flex-col overflow-hidden rounded-md border border-[var(--fintheon-accent)]/15 bg-[var(--fintheon-surface)] p-3" data-open="true">
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-3">
          <div className="relative min-h-0 flex-1 overflow-hidden rounded-md border border-[var(--fintheon-accent)]/10 bg-[var(--fintheon-bg)]">
            <svg
              ...
              aria-hidden="true"
            >
              <ChartLines nodes={chartNodes} hasDirective={Boolean(editDirective)} />
            </svg>
            <div className="relative grid h-full grid-cols-[1fr_1.2fr_1fr] gap-3 p-4">
              <ChartColumn nodes={chartNodes.filter((node) => node.column === "left")} />
              <div className="flex flex-col items-center justify-center gap-4">
                <ChartNode node={chartNodes.find((node) => node.kind === "root")!} />
                {editDirective ? (
              ...
              <ChartColumn nodes={chartNodes.filter((node) => node.column === "right")} />
            </div>
          </div>
...
function buildChartNodes(hypothesis: NarrativeHypothesis | null): ChartNodeData[] {
  if (!hypothesis) return [];
  const groups = groupEvidence(hypothesis.evidence);
  const supports = groups.supports.slice(0, 3).map((item) =>
    toNode(item, "left", "Supports"),
  );
  const neutral = groups.neutral.slice(0, 3).map((item) =>
    toNode(item, "right", "Neutral"),
  );
  const contradicts = groups.contradicts.slice(0, 2).map((item) =>
    toNode(item, "right", "Contradicts"),
  );
...
function ChartLines({
  ...
  const leftLines = Array.from({ length: leftCount }, (_, index) => 110 + index * 95);
  const rightLines = Array.from({ length: rightCount }, (_, index) => 110 + index * 95);
