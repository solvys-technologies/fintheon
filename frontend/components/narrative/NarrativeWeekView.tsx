...
export default function NarrativeWeekView() {
  ...
  const [newLaneDirection, setNewLaneDirection] = useState<
    "long" | "short" | "neutral"
  >("neutral");

  const currentMonday = useMemo(
    () => getMonday(new Date(state.currentWeekStart)),
  ...
  const handleMoveCatalyst = useCallback(
  ...
  const handleDoubleClickDay = useCallback(
    (e: React.MouseEvent, date: Date) => {
      ...
      const narrativeId =
        laneEl?.getAttribute("data-lane-id") || lanesFiltered[0]?.id;
  ...
  const handlePillSubmit = useCallback(
    (title: string) => {
      if (!pillInput) return;
      dispatch({
        type: "ADD_CATALYST",
  ...
  const handleAddLane = useCallback(() => {
    ...
    dispatch({
      ...
      lane: {
        ...
        instruments: newLaneInstruments
          .split(",")
          .map((s) => s.trim())
          .filter(Boolean),
      ...
      >
        {weeks.map((monday, idx) => {
          ...
          const weekCatalysts = state.catalysts.filter((c) => {
            ...
            return (
              cDate >= weekDates[0] && cDate <= weekDates[weekDates.length - 1]
            );
          });
                  ...
                  {showAddLane ? (
                    ...
                    >
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setNewLaneDirection(
                            e.target.value as "long" | "short" | "neutral",
                          )
                        }
                    ...
                    <button
                      ...
                      style={{
                        ...
                        border:
                          "1px dashed color-mix(in srgb, var(--fintheon-accent) 30%, transparent)",
                      }}
                    >
                      + Add Narrative
                    </button>
