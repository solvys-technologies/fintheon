...
function deriveConfidence(prints: EconHistoryPrint[]): number {
  if (prints.length === 0) return 0;
  const beats = prints.filter((p) => p.direction === "beat").length;
  const misses = prints.filter((p) => p.direction === "miss").length;
  const consistency = Math.max(beats, misses) / prints.length;
  const sampleFactor = Math.min(prints.length / 6, 1);
  return Math.round(consistency * sampleFactor * 100) / 100;
}
