...
export function NarrativeCanvasChat({ pendingChips = [], onClearChip }: NarrativeCanvasChatProps) {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    const chipContext = pendingChips.length > 0
    ...
    try {
      ...
      if (scored && scored.length > 0) {
        ...
        setResponse(`Added ${scored.length} event${scored.length !== 1 ? 's' : ''} to the Observatory.`);
      } else {
  ...
  return (
    <div className="relative flex flex-col items-center">
      ...
      {responseVisible && response && (
        ...
        >
          <Sparkles className="w-3.5 h-3.5 shrink-0" style={{ color: 'var(--fintheon-accent)' }} />
          <p className="flex-1 text-[11px] leading-relaxed" style={{ color: 'var(--fintheon-text)', fontFamily: 'var(--font-body)' }}>
            {response}
          </p>
          <button
          ...
          <div className="flex items-center gap-1 shrink-0">
            {pendingChips.slice(0, 2).map(chip => (
              ...
              >
                ...
                <button onClick={() => onClearChip?.(chip.id)} className="opacity-50 hover:opacity-100">
                  <X className="w-2 h-2" />
                </button>
              </span>
            ))}
