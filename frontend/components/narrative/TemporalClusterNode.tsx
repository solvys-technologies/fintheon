...
export const TemporalClusterNode = memo(({ data }: { data: TemporalClusterNodeData }) => {
  const threadColor = THREAD_COLORS[data.narrativeSlug] ?? '#6B7280';
  const severityColor = data.maxSeverity === 'high' ? '#EF4444'
    : data.maxSeverity === 'medium' ? '#c79f4a' : 'var(--fintheon-text-muted)';
  ...
  return (
    ...
    >
      ...
      <div className="text-[10px] font-mono text-[var(--fintheon-text)] truncate" title={data.label}>
        <span className="inline-block w-2 h-2 rounded-full mr-1.5 align-middle" style={{ backgroundColor: threadColor }} />
        {data.label}
      </div>
      ...
      <div className="flex gap-0.5 mt-1">
        ...
        {data.cards.length > 12 && (
          <span className="text-[8px] text-[var(--fintheon-text-muted)]">+{data.cards.length - 12}</span>
        )}
      </div>
