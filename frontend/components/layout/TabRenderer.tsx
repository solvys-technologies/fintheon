...
export function TabRenderer({ activeTab, tabTransitioning, prevTab, showRefinement, navigateTab }: TabRendererProps) {
  ...
  return (
    <div className="flex-1 min-h-0 overflow-hidden">
      ...
      {!showRefinement && activeTab === 'dashboard' && (
        <div key="dashboard" data-tour-target="dashboard" className={`h-full w-full section-fade-corners ${animClass}`}>
          <MainDashboard onNavigateTab={(tab) => navigateTab(tab as NavTab)} />
        </div>
      )}
      {!showRefinement && activeTab === 'analysis' && (
        <div key="analysis" data-tour-target="chat" className={`h-full w-full section-fade-corners ${animClass}`}>
          <ConsiliumHub />
        </div>
      )}
      {!showRefinement && activeTab === 'riskflow' && (
        <div key="riskflow" data-tour-target="riskflow" className={`h-full w-full section-fade-corners ${animClass}`}>
          <RiskFlowMain />
        </div>
      )}
      {!showRefinement && activeTab === 'econ' && (
        <div key="econ" data-tour-target="econ" className={`h-full w-full ${animClass}`}>
      ...
      {!showRefinement && activeTab === 'narrative' && (
        <div key="narrative" data-tour-target="narrative" className={`h-full w-full ${animClass}`}>
          <NarrativeProvider>
            <NarrativeMap />
          </NarrativeProvider>
        </div>
      ...
      {!showRefinement && activeTab === 'apparatus' && (
        <div key="apparatus" data-tour-target="apparatus" className={`h-full w-full ${animClass}`}>
          <ApparatusMap />
        </div>
      )}
      {/* S14-T5: scriptorium, documents, memory tabs removed — now in ConsiliumHub dropdowns */}
      {!showRefinement && activeTab === 'proposals' && (
        <div key="proposals" data-tour-target="proposals" className={`h-full w-full ${animClass}`}>
          <ProposalWidget />
        </div>
      )}
      {!showRefinement && activeTab === 'performance' && (
        <div key="performance" data-tour-target="performance" className={`h-full w-full ${animClass}`}>
          <PerformanceJournal />
        </div>
      )}
