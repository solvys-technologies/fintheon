...
export function TimelineOverlay({ open, onClose }: TimelineOverlayProps) {
  ...
  const activeThread = NARRATIVE_THREADS.find(t => t.slug === selectedThread) ?? NARRATIVE_THREADS[0]
      ...
      >
        ...
        <div className="shrink-0 flex items-center justify-between px-4 py-3 border-b border-[var(--fintheon-accent)]/10">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold uppercase tracking-widest text-[var(--fintheon-accent)]"
              style={{ fontFamily: 'var(--font-heading)' }}>
              Timeline
            </h2>
            <span className="text-[11px] text-[var(--fintheon-muted)]/40" style={{ fontFamily: 'var(--font-mono)' }}>
              {filteredItems.length}
            </span>
          </div>
          <button onClick={onClose} className="p-1.5 rounded hover:bg-[var(--fintheon-accent)]/10 transition-colors" title="Close">
        ...
        <div className="shrink-0 px-4 py-2 border-b border-[var(--fintheon-accent)]/5 relative">
          ...
          >
            <div className="flex items-center gap-2">
              <div className="w-2.5 h-2.5 rounded-sm" style={{ backgroundColor: activeThread.color }} />
              <span className="text-[12px] font-medium text-[var(--fintheon-text)]" style={{ fontFamily: 'var(--font-body)' }}>
                {activeThread.title}
              </span>
            </div>
            <ChevronDown className={`w-3.5 h-3.5 text-[var(--fintheon-muted)]/40 transition-transform ${dropdownOpen ? 'rotate-180' : ''}`} />
          </button>
          ...
          {dropdownOpen && (
            <div className="absolute left-4 right-4 top-full mt-1 z-50 rounded-lg border border-[var(--fintheon-accent)]/15 bg-[var(--fintheon-bg)] shadow-xl overflow-hidden max-h-[300px] overflow-y-auto">
              {NARRATIVE_THREADS.map(thread => (
                ...
                >
                  <div className="w-2.5 h-2.5 rounded-sm shrink-0" style={{ backgroundColor: thread.color }} />
                  {thread.title}
                  {thread.slug !== 'all' && (
                    <span className="ml-auto text-[10px] text-[var(--fintheon-muted)]/30" style={{ fontFamily: 'var(--font-mono)' }}>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-3 py-2 space-y-3">
          ...
          {!loading && dateGroups.length === 0 && (
            <p className="text-[13px] text-[var(--fintheon-muted)]/30 text-center py-12" style={{ fontFamily: 'var(--font-body)' }}>
              No items in this timeline
            </p>
          )}

          {!loading && dateGroups.map(([date, events]) => (
            <div key={date}>
              <div className="flex items-center gap-2 mb-2">
                <span className="text-[10px] font-bold uppercase tracking-wider"
                  style={{ color: `${activeThread.color}80`, fontFamily: 'var(--font-mono)' }}>
                  {formatDate(date)}
                </span>
                <div className="flex-1 h-px" style={{ backgroundColor: `${activeThread.color}12` }} />
                <span className="text-[9px] text-[var(--fintheon-muted)]/25" style={{ fontFamily: 'var(--font-mono)' }}>
                  {events.length}
                </span>
              </div>
              ...
              <div className="space-y-1.5 ml-0.5">
                {events.map(item => {
                  const sentiment = item.priceBrainScore?.sentiment?.toLowerCase() ?? item.sentiment ?? 'bearish'
                  const isBullish = sentiment === 'bullish'
                  const sevColor = MACRO_COLOR[item.macroLevel ?? 2] ?? '#6B7280'
                  return (
                    <div
                      key={item.id}
                      className="rounded-lg px-3 py-2.5 transition-colors hover:brightness-110"
                      style={{ border: `1px solid ${sevColor}35`, backgroundColor: `${sevColor}04` }}
                    >
                        ...
                        <div className="flex flex-wrap gap-1 mt-1.5">
                          {item.tags.slice(0, 3).map(t => (
                            <span key={t} className="text-[8px] px-1.5 py-0.5 rounded"
                              style={{ color: `${activeThread.color}80`, backgroundColor: `${activeThread.color}08`, fontFamily: 'var(--font-mono)' }}>
                              #{t}
                            </span>
                          ))}
                        </div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-[9px] text-[var(--fintheon-muted)]/30" style={{ fontFamily: 'var(--font-mono)' }}>
                          {formatRelative(item.publishedAt)}
                        </span>
                        {item.source && (
                          <span className="text-[8px] text-[var(--fintheon-muted)]/20" style={{ fontFamily: 'var(--font-mono)' }}>
                            {item.source}
                          </span>
                        )}
                      </div>
