...
import { Search, Plus, Trash2, Loader2, Brain, Save, ChevronRight, User, BookOpen, Link2 } from 'lucide-react'
...
export function SharedMemoryPanel({ mode = 'standard' }: SharedMemoryPanelProps) {
      ...
      <div className="flex h-full bg-[#050402]">
        ...
        <div className="flex flex-col w-72 flex-shrink-0 border-r border-[#c79f4a]/10">
          ...
          <div className="flex items-center gap-2 px-4 py-3 border-b border-[#c79f4a]/10">
            ...
            <span className="text-[10px] text-[#f0ead6]/30 ml-auto">{entries.length} items</span>
          </div>
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {!loading && entries.map((entry) => {
              ...
              const valuePreview = typeof entry.value === 'object'
                ? (entry.value as Record<string, unknown>)?.text as string ?? JSON.stringify(entry.value).slice(0, 80)
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <ChevronRight size={10} className={`text-[#f0ead6]/20 ml-auto flex-shrink-0 transition-transform ${isActive ? 'rotate-90' : ''}`} />
                  </div>
                  <div className="text-[10px] text-[#f0ead6]/30 mt-0.5 truncate">{valuePreview}</div>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-[9px] px-1 rounded bg-[#c79f4a]/10 text-[#c79f4a]/60">{entry.category}</span>
                    {entry.agentName && (
                      <span className="text-[9px] text-[#f0ead6]/25">{entry.agentName}</span>
                    )}
                  </div>
                </button>
              )
        ...
        <div className="flex-1 overflow-y-auto">
          {!selected ? (
            ...
            <div className="p-5">
              {/* Title + category */}
              <div className="mb-4">
                <h3 className="text-sm font-bold text-[#f0ead6]">{selected.key}</h3>
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-[10px] px-1.5 py-0.5 rounded bg-[#c79f4a]/10 text-[#c79f4a]/70">{selected.category}</span>
                  <span className="text-[10px] text-[#f0ead6]/25">
                    Updated {new Date(selected.updatedAt).toLocaleString()}
                  </span>
                  {selected.ttlHours && (
                    <span className="text-[10px] text-[#f0ead6]/25">{selected.ttlHours}h TTL</span>
                  )}
                </div>
              </div>
              ...
              {selected.agentName && (
                <div className="flex items-center gap-2 mb-3 px-3 py-2 rounded-lg bg-[#0a0a08] border border-[#c79f4a]/10">
                  ...
                  <span className="text-xs text-[#f0ead6]/50">Linked Agent</span>
                  <span className="text-xs font-medium text-[#c79f4a]">{selected.agentName}</span>
                </div>
              )}
              ...
              <div className="mb-3 px-3 py-2 rounded-lg bg-[#0a0a08] border border-[#c79f4a]/10">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs text-[#f0ead6]/50">Rules of Engagement</span>
                </div>
                <p className="text-[10px] text-[#f0ead6]/30 italic">
                  {(selected.value as Record<string, unknown>)?.rules_of_engagement as string
              ...
              <div className="px-3 py-2 rounded-lg bg-[#0a0a08] border border-[#c79f4a]/10">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs text-[#f0ead6]/50">Event Citations</span>
                </div>
                {(() => {
                  const citations = (selected.value as Record<string, unknown>)?.event_citations as Array<{ headline: string; date: string }> | undefined
                  if (!citations || citations.length === 0) {
                    return <p className="text-[10px] text-[#f0ead6]/30 italic">No event citations — pending DB migration</p>
                  }
                  return (
                    <div className="space-y-1">
                      {citations.map((c, i) => (
                        <div key={i} className="flex items-center gap-2 text-[10px]">
                          ...
                          <span className="text-[#f0ead6]/60">{c.headline}</span>
                        </div>
                      ))}
                    </div>
                  )
