        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold text-[var(--fintheon-text)]">{agent.name}</span>
            </div>
            <div className="flex items-center gap-1.5">
              <span className={`w-2 h-2 rounded-full ${STATUS_DOT[agent.status]}`} />
              <span className="text-[10px] text-zinc-400">{STATUS_LABEL[agent.status]}</span>
            </div>
          </div>
          ...
          <div className="text-[10px] text-zinc-600 font-mono mt-auto">{agent.model}</div>
        </div>
      ))}
    </div>
  );
