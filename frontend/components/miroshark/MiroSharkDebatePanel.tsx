    ...
    <div className="h-full flex flex-col bg-[var(--fintheon-bg)] text-[var(--fintheon-text)]">
      ...
      <div className="flex-shrink-0 p-3 border-b border-[var(--fintheon-accent)]/10">
        ...
        <PhaseTimeline
          ...
          govSkipped={state?.govOfficialsSkipped}
        />
      </div>
        ...
        {state?.govOfficialsSkipped && phaseIndex >= 1 && (
          <div className="rounded border border-[var(--fintheon-text)]/5 p-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-[var(--fintheon-text)]/40">
                Gov Officials — Skipped (no geopolitical content)
              </span>
            </div>
          </div>
        )}
          ...
          <div key={p.short} className="flex items-center gap-1">
            <div
              className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-medium transition-colors ${
                isSkipped
                  ? "bg-[var(--fintheon-text)]/5 text-[var(--fintheon-text)]/20"
                  : isActive
                    ? "bg-[var(--fintheon-accent)] text-[var(--fintheon-bg)]"
                    : isComplete
                      ? "bg-[var(--fintheon-accent)]/30 text-[var(--fintheon-accent)]"
            ...
            <span
              className={`text-[10px] ${
                isSkipped
                  ? "text-[var(--fintheon-text)]/20 line-through"
                  : isActive
                    ? "text-[var(--fintheon-accent)]"
                    : isComplete
                      ? "text-[var(--fintheon-text)]/60"
            ...
            {i < 3 && (
              <div
                className={`w-3 h-px ${isComplete ? "bg-[var(--fintheon-accent)]/30" : "bg-[var(--fintheon-text)]/10"}`}
              />
            )}
          </div>
        );
  ...
  return (
    <div
      className={`rounded border transition-colors ${
        isActive
          ? "border-[var(--fintheon-accent)]/30"
          : isComplete
            ? "border-[var(--fintheon-accent)]/10"
    ...
    >
      ...
      >
        ...
        <span
          className={`text-xs font-medium flex-1 ${isActive ? "text-[var(--fintheon-accent)]" : "text-[var(--fintheon-text)]/70"}`}
        >
          {title}
        </span>
        {isComplete && <CheckCircle2 className="w-3 h-3 text-emerald-400/60" />}
        {isActive && (
          <Loader2 className="w-3 h-3 animate-spin text-[var(--fintheon-accent)]" />
        )}
        {expanded ? (
          <ChevronDown className="w-3 h-3 text-[var(--fintheon-text)]/40" />
        ) : (
          <ChevronRight className="w-3 h-3 text-[var(--fintheon-text)]/40" />
        )}
      </button>
...
function AnalystCard({ assessment }: { assessment: MarketAnalystAssessment }) {
  const ivColor =
    assessment.projectedIVScore >= 6
      ? "text-red-400"
      : assessment.projectedIVScore >= 4
        ? "text-amber-400"
        : "text-emerald-400";
  ...
  return (
    <div
      className={`rounded bg-[var(--fintheon-text)]/5 p-2 ${isContrarian ? "border border-amber-400/20" : ""}`}
    >
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[9px] text-[var(--fintheon-text)]/40">
            {assessment.title}
          </span>
          {isContrarian && (
        ...
        <span className={`text-xs font-medium ${ivColor}`}>
          IV {assessment.projectedIVScore.toFixed(1)}
        </span>
      </div>
      ...
      <div className="flex flex-wrap gap-1 mb-1.5">
        {assessment.subjects.map((s) => (
          <span
            key={s}
            className="text-[8px] px-1 py-0.5 rounded bg-[var(--fintheon-accent)]/10 text-[var(--fintheon-accent)]/70"
          >
            {s}
          </span>
        ))}
      ...
      <p className="text-[10px] text-[var(--fintheon-text)]/60 mb-1">
        {assessment.keyConcern}
      </p>
        ...
        {[...assessment.categoryScores]
          ...
          .map((c) => (
            <div key={c.category} className="flex items-center gap-1">
              <span className="text-[8px] text-[var(--fintheon-text)]/30 truncate max-w-[50px]">
                {c.category.replace("-", " ")}
              </span>
  ...
  return (
    <div className="rounded bg-[var(--fintheon-text)]/5 p-2">
      <div className="flex items-center justify-between mb-1">
        ...
        <span className="text-[10px] text-[var(--fintheon-text)]/40">
          {(assessment.confidence * 100).toFixed(0)}% conf
        </span>
      </div>
      <p className="text-[10px] text-[var(--fintheon-text)]/60 mb-1">
        {assessment.keyConcern}
      </p>
      <p className="text-[10px] text-[var(--fintheon-text)]/40">
...
function HermesCard({ result }: { result: HermesDeliberation }) {
  ...
  return (
    <div className="rounded bg-[var(--fintheon-text)]/5 p-2">
      <div className="flex items-center justify-between mb-1">
        <span className="text-xs font-medium text-[var(--fintheon-text)]/80">
          {result.name}
        </span>
        <span
          className={`text-[10px] font-medium px-1.5 py-0.5 rounded ${verdictColor} ${verdictBg}`}
        >
          {result.verdict.toUpperCase()}
        </span>
      </div>
      <p className="text-[10px] text-[var(--fintheon-text)]/60">
        {result.reasoning}
      </p>
    </div>
  );
...
function HarperCard({ scoring }: { scoring: HarperOpusScoring }) {
  const consensusColor =
    (scoring.consensusScore ?? 50) >= 70
      ? "text-emerald-400"
      : (scoring.consensusScore ?? 50) >= 40
        ? "text-[var(--fintheon-accent)]"
        : "text-red-400";
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="grid grid-cols-4 gap-2">
        ...
        <KPI
          label="Regime Risk"
          value={`${(scoring.regimeShiftProbability * 100).toFixed(0)}%`}
        />
        <KPI
          label="Actionability"
          value={scoring.actionabilityScore.toFixed(1)}
        />
        <KPI
          label="Consensus"
          value={
            scoring.consensusScore != null ? `${scoring.consensusScore}` : "—"
          }
      ...
      {scoring.healthyDisagreementCount != null &&
        scoring.healthyDisagreementCount > 0 && (
          <div className="flex items-center gap-1.5 text-[10px] text-[var(--fintheon-accent)]/60">
            <Zap className="w-3 h-3" />
            {scoring.healthyDisagreementCount} analyst
            {scoring.healthyDisagreementCount > 1 ? "s" : ""} diverge
            significantly — healthy tension
          </div>
        )}

      {/* Briefing */}
      <div className="rounded bg-[var(--fintheon-text)]/5 p-2">
        <p className="text-[10px] text-[var(--fintheon-text)]/70 leading-relaxed">
          {scoring.finalBriefing}
        </p>
      </div>
      ...
      {scoring.surfacedTheses.length > 0 && (
        <div>
          <span className="text-[10px] font-medium text-emerald-400/80 block mb-1">
            Surfaced
          </span>
          {scoring.surfacedTheses.map((t, i) => (
            <p
              key={i}
              className="text-[10px] text-[var(--fintheon-text)]/60 pl-2 border-l border-emerald-400/20 mb-1"
            >
              {t}
            </p>
          ))}
      ...
      {scoring.contestedTheses.length > 0 && (
        <div>
          <span className="text-[10px] font-medium text-amber-400/80 block mb-1">
            Contested
          </span>
          {scoring.contestedTheses.map((t, i) => (
            <p
              key={i}
              className="text-[10px] text-[var(--fintheon-text)]/60 pl-2 border-l border-amber-400/20 mb-1"
            >
              {t}
            </p>
          ))}
      ...
      {scoring.downgradedTheses.length > 0 && (
        <div>
          <span className="text-[10px] font-medium text-red-400/80 block mb-1">
            Downgraded
          </span>
          {scoring.downgradedTheses.map((t, i) => (
            <p
              key={i}
              className="text-[10px] text-[var(--fintheon-text)]/60 pl-2 border-l border-red-400/20 mb-1"
            >
              {t}
            </p>
          ))}
...
function KPI({
  ...
  return (
    <div className="rounded bg-[var(--fintheon-text)]/5 p-1.5 text-center">
      <div
        className={`text-xs font-medium ${valueColor ?? "text-[var(--fintheon-accent)]"}`}
      >
        {value}
      </div>
...
function getPhaseIndex(phase: DeliberationPhase): number {
  switch (phase) {
    case "market-analysts":
      return 0;
    case "gov-officials":
      return 1;
    case "hermes-deliberation":
      return 2;
    case "harper-scoring":
      return 3;
    case "complete":
