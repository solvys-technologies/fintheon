...
export function TeamMemberCard({ member, isSelf }: TeamMemberCardProps) {
  ...
  const newsfeedStale = isStale(presence.services.newsfeedPolling.lastUpdate, 15);
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="relative flex items-center gap-2 min-w-0" ref={dropdownRef}>
          ...
          <div className="relative shrink-0">
            <div className={`w-2 h-2 rounded-full ${statusDotColor(presence.userStatus)}`} />
          ...
          {dropdownOpen && isSelf && (
            <div className="absolute top-full left-0 mt-1 z-50 rounded border border-[var(--fintheon-accent)]/20 bg-[var(--fintheon-bg)] shadow-lg min-w-[140px]">
              {STATUS_OPTIONS.map((opt) => (
                <button
                  key={opt.value}
                  onClick={() => { setUserStatus(opt.value); setDropdownOpen(false); }}
                  className={`w-full flex items-center gap-2 px-3 py-1.5 text-[10px] font-mono transition-colors hover:bg-[var(--fintheon-accent)]/10 ${
                    presence.userStatus === opt.value ? 'text-[var(--fintheon-accent)]' : 'text-zinc-400'
                  }`}
                >
