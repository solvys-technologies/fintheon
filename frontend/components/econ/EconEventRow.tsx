...
function getSurpriseColor(direction: string | null, goodBeta: boolean): string {
  ...
  if (direction === "beat")
    return goodBeta ? "text-emerald-400" : "text-red-400";
  if (direction === "miss")
    return goodBeta ? "text-red-400" : "text-emerald-400";
  ...
  return (
    <div className="group">
      ...
      >
        ...
        {/* Flag */}
        <span className="text-sm shrink-0" title={event.country}>
          {flag}
        </span>

        {/* Volume bar (replaces importance dots) */}
        <div
          className="flex items-end shrink-0 w-3"
          title={`Impact: ${event.importance}/3`}
        >
        ...
        <div className="flex items-center gap-3 shrink-0 text-[10px] font-mono">
          ...
          <div
            className={`w-14 text-right font-semibold ${hasActual ? "text-[var(--fintheon-accent)]" : "text-zinc-600"}`}
            title="Actual"
          >
            {event.actual ?? "-"}
          </div>
        ...
        <div className="px-4 pb-4 pt-1 ml-6 border-l border-[var(--fintheon-accent)]/15 space-y-2">
                  ...
                  {prints.map((p) => {
                    ...
                    return (
                      <tr key={p.id} className="border-b border-zinc-800/50">
                        ...
                        <td className="py-1.5 text-right text-zinc-500 font-mono">
                          {p.previous ?? "-"}
                        </td>
                        <td className="py-1.5 text-right text-zinc-200 font-mono font-semibold">
                          {p.actual ?? "-"}
                        </td>
                        ...
                        <td
                          className={`py-1.5 text-right font-mono font-semibold ${getSurpriseColor(p.direction, p.goodBeta)}`}
                        >
                          {p.surprise != null
                            ? `${p.surprise > 0 ? "+" : ""}${p.surprise.toFixed(1)}%`
                            : "-"}
                        </td>
                        <td className="py-1.5 text-center">
                          {printBeat === "beat" && (
                            <Check
                              className="w-3 h-3 text-emerald-400 inline-block"
                              strokeWidth={3}
                            />
                          )}
                          {printBeat === "miss" && (
                            <X
                              className="w-3 h-3 text-red-400 inline-block"
                              strokeWidth={3}
                            />
                          )}
                          {printBeat === "pending" && (
          ...
          {!loadingPrints && prints.length === 0 && (
            <div className="text-[10px] text-zinc-600 italic">
              No historical prints available
            </div>
          )}
        </div>
      )}
