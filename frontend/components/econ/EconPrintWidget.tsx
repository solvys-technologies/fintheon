...
export function EconPrintWidget({
  print,
  compact = false,
}: EconPrintWidgetProps) {
    ...
    return (
      <div className="flex items-center gap-2 text-[10px] font-mono">
        ...
        <span className={`font-semibold ${surpColor}`}>
          {dirIcon}{" "}
          {print.surprise != null
            ? `${print.surprise > 0 ? "+" : ""}${print.surprise.toFixed(1)}%`
            : ""}
        </span>
      </div>
    );
  ...
  return (
    <div className="grid grid-cols-4 gap-1 text-[10px]">
      <div className="flex flex-col items-center">
        <span className="text-[8px] text-zinc-600 uppercase tracking-wider mb-0.5">
          Prev
        </span>
