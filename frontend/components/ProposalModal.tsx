...
export function ProposalModal({
  ...
  const directionCssColor = isLong
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b border-zinc-900">
          ...
          <button
            onClick={handleClose}
            className="p-1.5 hover:bg-zinc-900 rounded transition-all"
          >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              ...
              <div className="text-lg font-mono font-bold text-[var(--fintheon-accent)]">
                {proposal.instrument}
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                {isLong ? (
                  <Diff
                    className="w-8 h-8"
                    style={{ color: "var(--fintheon-bullish)" }}
                  />
                ) : isShort ? (
                  <TrendingDown
                    className="w-8 h-8"
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-[var(--fintheon-bg)] border border-zinc-900 rounded-lg p-3">
                ...
                <div className="text-lg font-mono text-green-400">
                  {proposal.takeProfit?.length
                    ? proposal.takeProfit[0].toFixed(2)
                    : "N/A"}
                </div>
              </div>
              <div className="bg-[var(--fintheon-bg)] border border-zinc-900 rounded-lg p-3">
                <div className="text-xs text-zinc-500 mb-1">R:R</div>
                <div className="text-lg font-mono text-[var(--fintheon-accent)]">
                  {proposal.riskRewardRatio.toFixed(1)}:1
                </div>
              </div>
            </div>
          )}
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              {Object.entries(proposal.analystInputs).map(([key, value]) => (
                <div
                  key={key}
                  className="bg-[var(--fintheon-bg)] border border-zinc-900/50 rounded p-2"
                >
...
export const MOCK_PROPOSAL: TradingProposal = {
  ...
  entryPrice: 19250.0,
  stopLoss: 19220.0,
  takeProfit: [19300.0, 19350.0],
  positionSize: 2,
  riskRewardRatio: 2.5,
  confidence: 78,
  rationale:
    "VIX elevated at 22.5, market showing oversold conditions with bullish divergence on 5m RSI. Morning flush setup triggered with confluence at prior session VWAP.",
  analystInputs: {
