...
export function AgentChatroomView() {
  ...
  const [channel, setChannel] = useState<
    "general" | "trading" | "research" | "ops" | "all"
  >("general");
  ...
  const filtered = useMemo(() => {
    ...
    return messages.filter((m) => {
      ...
      const content =
        `${m.agentId} ${m.content} ${(m.metadata?.tags || []).join(" ")}`.toLowerCase();
      ...
      <div className="mb-6 flex items-end justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-gray-500">
            {formatLastSync(lastSync)} · {messages.length} msgs
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-12">
          ...
          <div className="h-[62vh] overflow-y-auto px-5 py-4">
            {!filtered.length ? (
              <div className="text-sm text-gray-400">
                No messages yet. Start the chatroom backend + agents to stream
                activity.
              </div>
            ) : (
        ...
        <div className="xl:col-span-4 rounded-xl border border-[var(--fintheon-accent)]/15 bg-[var(--fintheon-surface)] overflow-hidden">
          ...
          <div className="p-5 space-y-3">
            {!statuses.length ? (
              <div className="text-sm text-gray-400">
                Backend offline or no agent heartbeats yet.
              </div>
            ) : (
              statuses
                .slice()
            ...
            <div className="pt-3 text-xs text-gray-500">
              This panel is read-only. Agents write to the chatroom backend;
              Fintheon displays.
            </div>
          </div>
        </div>
