...
function StatBox({ label, value, sub, accent }: { label: string; value: string; sub?: string; accent?: boolean }) {
  return (
    <div className={`bg-[#0a0a06] border px-2.5 py-2 ${accent ? 'border-[var(--fintheon-accent)]/40' : 'border-[var(--fintheon-accent)]/20'}`}>
      <div className="text-[9px] tracking-[0.22em] uppercase text-[var(--fintheon-accent)]/60 mb-0.5">{label}</div>
      <div className={`text-xs font-semibold ${accent ? 'text-[var(--fintheon-accent)]' : 'text-[var(--fintheon-text)]'}`}>{value}</div>
...
export default function TradeIdeaModal({ idea, onClose }: TradeIdeaModalProps) {
  ...
  const confidenceColor =
    idea.confidence === 'high' ? 'text-[var(--fintheon-accent)]' :
    idea.confidence === 'medium' ? 'text-zinc-400' :
    'text-zinc-600';

  // Derive expected print context from hermesDescription or title
  const hasExpectedPrint = idea.hermesDescription?.match(/expect|forecast|consensus|print|release/i);

  // Determine recommendation based on confidence + R/R
  const shouldAct = idea.confidence === 'high' && (idea.riskRewardRatio == null || idea.riskRewardRatio >= 1.5);
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
        ...
        >
          ...
          <div className="flex items-start justify-between px-4 py-3 border-b border-[var(--fintheon-accent)]/20 shrink-0">
            <div className="flex items-center gap-3 min-w-0">
              <div className={`flex-shrink-0 p-1.5 border ${
                isLong ? 'border-[var(--fintheon-accent)]/40 text-[var(--fintheon-accent)]' :
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <span className={`text-[10px] font-bold tracking-[0.2em] uppercase ${
                    isLong ? 'text-[var(--fintheon-accent)]' : isShort ? 'text-zinc-400' : 'text-zinc-600'
                  }`}>
          ...
          <div className="flex-1 overflow-y-auto px-4 py-3.5 space-y-4">
            ...
            {(idea.entry || idea.stopLoss || idea.takeProfit) && (
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-[9px] tracking-[0.2em] uppercase text-zinc-600">Price Levels</span>
                </div>
                <div className="grid grid-cols-3 gap-2">
                  {idea.entry != null && (
                    <StatBox label="Entry" value={formatPrice(idea.entry)} accent />
                  )}
                  {idea.stopLoss != null && (
                    <StatBox label="Stop Loss" value={formatPrice(idea.stopLoss)} sub={idea.entry != null ? `${Math.abs(idea.entry - idea.stopLoss).toFixed(1)} pts risk` : undefined} />
                  )}
                  {idea.takeProfit != null && (
                    <StatBox label="Profit Target" value={formatPrice(idea.takeProfit)} sub={idea.entry != null ? `${Math.abs(idea.takeProfit - idea.entry).toFixed(1)} pts reward` : undefined} />
                  )}
                </div>
              </div>
            )}
            ...
            {(idea.potentialRisk != null || idea.potentialProfit != null || idea.riskRewardRatio != null) && (
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-[9px] tracking-[0.2em] uppercase text-zinc-600">Risk / Reward</span>
                </div>
                <div className="grid grid-cols-3 gap-2">
                  {idea.potentialRisk != null && (
                    <StatBox label="Risk" value={`${idea.potentialRisk.toFixed(1)}%`} sub="of position" />
                  )}
                  {idea.potentialProfit != null && (
                    <StatBox label="Profit" value={`${idea.potentialProfit.toFixed(1)}%`} sub="of position" />
                  )}
                  {idea.riskRewardRatio != null && (
                    <StatBox label="R/R Ratio" value={`${idea.riskRewardRatio.toFixed(1)}:1`} accent={idea.riskRewardRatio >= 2} />
                  )}
                </div>
              </div>
            )}
            ...
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[9px] tracking-[0.2em] uppercase text-zinc-600">Trade Scenarios</span>
              </div>
              <div className="grid grid-cols-2 gap-2">
                {/* Consensus trade */}
                <div className="rounded border border-[var(--fintheon-accent)]/20 bg-[#0a0a06] p-3">
                  <div className="flex items-center gap-1.5 mb-2">
                    <div className={`w-1.5 h-1.5 rounded-full ${isLong ? 'bg-[var(--fintheon-low)]' : 'bg-[var(--fintheon-severe)]'}`} />
                    <span className="text-[8px] font-mono font-bold text-[var(--fintheon-text)]/60 uppercase tracking-wider">Consensus</span>
                  </div>
                  ...
                  {idea.entry != null && (
                    <div className="mt-1.5 text-[9px] font-mono text-[var(--fintheon-accent)]/70">
                      Entry {formatPrice(idea.entry)} → Target {idea.takeProfit != null ? formatPrice(idea.takeProfit) : 'TBD'}
                    </div>
                  )}
                </div>
                ...
                <div className="rounded border border-violet-500/20 bg-[#0a0a06] p-3">
                  <div className="flex items-center gap-1.5 mb-2">
                    <div className={`w-1.5 h-1.5 rounded-full ${isLong ? 'bg-[var(--fintheon-severe)]' : 'bg-[var(--fintheon-low)]'}`} />
                    <span className="text-[8px] font-mono font-bold text-violet-400/80 uppercase tracking-wider">Contrarian</span>
                  </div>
                  <p className="text-[10px] text-zinc-400 leading-relaxed">
                    {hasExpectedPrint
                  ...
                  {idea.stopLoss != null && idea.entry != null && (
                    <div className="mt-1.5 text-[9px] font-mono text-violet-400/70">
                      Entry near {formatPrice(idea.stopLoss)} → Target {formatPrice(idea.entry)}
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className={`rounded border p-3 ${
              ...
              <div className="flex items-center gap-2">
                ...
                <span className={`text-[10px] font-bold tracking-[0.15em] uppercase ${
                  shouldAct ? 'text-[var(--fintheon-accent)]' : 'text-zinc-500'
                }`}>
                  {shouldAct ? 'Actionable — Execute Trade' : 'Review — Discretionary'}
                </span>
              </div>
          ...
          <div className="shrink-0 flex items-center justify-between px-4 py-2.5 border-t border-zinc-800/60">
            <span className="text-[10px] text-zinc-700 tracking-wider uppercase">PIC Trade Proposals</span>
            <div className="flex items-center gap-2">
              {idea.confidence && (
                <span className={`text-[8px] font-mono font-bold px-1.5 py-0.5 rounded ${
                  idea.confidence === 'high' ? 'bg-[var(--fintheon-accent)]/15 text-[var(--fintheon-accent)]' :
