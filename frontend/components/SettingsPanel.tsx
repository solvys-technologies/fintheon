...
import { Settings, Bell, CreditCard, Cpu, Code, Terminal, Palette, Users, AlertTriangle, ArrowLeft, Globe } from 'lucide-react';
...
const AVAILABLE_SYMBOLS = [
  { symbol: 'MNQ', contractName: 'MNQ Z25', description: 'E-mini Micro Nasdaq Futures' },
  { symbol: 'ES', contractName: 'ES Z25', description: 'E-mini S&P 500 Futures' },
  { symbol: 'NQ', contractName: 'NQ Z25', description: 'E-mini Nasdaq-100 Futures' },
  { symbol: 'YM', contractName: 'YM Z25', description: 'E-mini Dow Jones Futures' },
  { symbol: 'RTY', contractName: 'RTY Z25', description: 'E-mini Russell 2000 Futures' },
];
...
export function SettingsPage() {
  ...
  const {
    apiKeys, setAPIKeys, tradingModels, setTradingModels, alertConfig, setAlertConfig,
    mockDataEnabled, setMockDataEnabled, selectedSymbol, setSelectedSymbol,
    riskSettings, setRiskSettings, developerSettings, setDeveloperSettings,
  ...
  const handleTabChange = (tab: SettingsTab) => {
    ...
    if (showLanding) {
      ...
      setActiveTab(tab);
      setTimeout(() => { setShowLanding(false); setLandingExiting(false); }, 200);
      return;
    }
    ...
    setTimeout(() => {
      setActiveTab(tab);
      setTimeout(() => { setTabTransitioning(false); setPrevTab(null); }, 50);
    }, 300);
  };

  const handleBackToLanding = () => {
    setLandingTransition(true);
    setTimeout(() => { setShowLanding(true); setLandingTransition(false); }, 250);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (isAuthenticated) {
        ...
        if (apiKeys.topstepxUsername || apiKeys.topstepxApiKey) {
          try {
            ...
            setTimeout(() => { addToast('Settings saved. ProjectX credentials failed — check API key.', 'info'); setIsSaving(false); }, remaining);
            return;
          }
        }
      }
      const elapsed = Date.now() - startTime;
      const remaining = Math.max(0, 1200 - elapsed);
      setTimeout(() => { addToast('Settings saved successfully', 'success'); setIsSaving(false); }, remaining);
    } catch (error) {
      console.error('Failed to save settings:', error);
      const elapsed = Date.now() - startTime;
      const remaining = Math.max(0, 1200 - elapsed);
      setTimeout(() => { addToast('Settings saved locally. Backend sync unavailable.', 'info'); setIsSaving(false); }, remaining);
    }
  };
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        if (account.contractsPerTrade) setContractsPerTrade(account.contractsPerTrade);
        if (account.projectxUsername) {
          setAPIKeys((prev: APIKeys) => ({ ...prev, topstepxUsername: account.projectxUsername || '' }));
        }
      } catch (error) { console.warn('Failed to load settings data:', error); }
    }
    fetchData();
  }, [backend, isAuthenticated, setAPIKeys]);

  const animClass = tabTransitioning && prevTab ? 'animate-fade-out-tab' : 'animate-fade-in-tab';

  const renderTabContent = () => {
    switch (activeTab) {
      case 'notifications': return <div key="notifications" className={animClass}><NotificationsTab alertConfig={alertConfig} setAlertConfig={setAlertConfig} voiceMemory={voiceMemory} /></div>;
      ...
      case 'danger': return <div key="danger" className={animClass}><DangerTab /></div>;
      case 'developer': return <div key="developer" className={animClass}><DeveloperTab devAuthenticated={devAuthenticated} onAuthenticated={() => setDevAuthenticated(true)} tier={tier} setTier={setTier} mockDataEnabled={mockDataEnabled} setMockDataEnabled={setMockDataEnabled} developerSettings={developerSettings} setDeveloperSettings={setDeveloperSettings} /></div>;
      default: return null;
    }
  };
  ...
  return (
    <div className="h-full w-full flex relative">
      <div className="flex-1 flex flex-col min-h-0">
                    ...
                    <React.Fragment key={tab.id}>
                      ...
                      >
                        <div className="flex items-start gap-3">
                          <div className={`mt-0.5 w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${isDanger ? 'bg-red-500/10 text-red-400 group-hover:bg-red-500/20' : 'fintheon-settings-icon'} transition-colors`}>
                            <Icon className="w-4 h-4" />
                          </div>
                          <div className="min-w-0">
                            <div className={`text-[13px] font-semibold ${isDanger ? 'text-red-400' : 'text-white'}`}>{tab.label}</div>
                            <div className="text-[11px] text-gray-500 mt-0.5 leading-relaxed">{tab.description}</div>
                          </div>
                        </div>
                      </button>
                    </React.Fragment>
        ...
        <div className={`flex-1 flex flex-col min-h-0 transition-all duration-200 ${landingTransition ? 'opacity-0 translate-y-2' : 'opacity-100 translate-y-0'}`}>
          <div className="shrink-0 flex items-center gap-3 px-8 pt-5 pb-3">
            <button onClick={handleBackToLanding} className="flex items-center justify-center w-7 h-7 rounded-md text-gray-400 hover:text-[var(--fintheon-accent)] hover:bg-[var(--fintheon-accent)]/10 transition-colors" title="Back to Settings">
            ...
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-[14px] font-semibold text-white tracking-tight">{TABS.find(t => t.id === activeTab)?.label}</h2>
            </div>
          </div>
      ...
      {!showLanding && (
      <div className="absolute right-0 top-0 bottom-0 z-30" onMouseEnter={() => setSidebarHovered(true)} onMouseLeave={() => setSidebarHovered(false)}>
        {!sidebarHovered && (
          <div className="absolute right-0 top-0 bottom-0 w-3 bg-transparent cursor-pointer">
            <div className="absolute right-1 top-1/2 -translate-y-1/2 flex flex-col gap-1.5">
              {TABS.map((_, i) => (
                <div key={i} className={`w-1 rounded-full transition-all ${TABS[i].id === activeTab ? 'h-4 bg-[var(--fintheon-accent)]' : 'h-1.5 bg-[var(--fintheon-accent)]/25'}`} />
              ))}
            </div>
          </div>
        )}
        <div className={`h-full bg-[var(--fintheon-bg)] border-l border-[var(--fintheon-accent)]/15 flex flex-col py-5 transition-all duration-200 ease-out overflow-hidden ${sidebarHovered ? 'w-52 opacity-100' : 'w-0 opacity-0'}`}>
          ...
          <div className="flex-1 space-y-0.5 px-2">
            {TABS.map(tab => {
              ...
              return (
                <button key={tab.id} onClick={() => handleTabChange(tab.id)} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-md transition-colors ${isActive ? 'bg-[var(--fintheon-accent)]/15 text-[var(--fintheon-accent)]' : 'text-gray-400 hover:bg-[var(--fintheon-accent)]/8 hover:text-gray-200'}`}>
