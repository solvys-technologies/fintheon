...
function SatValCanvas({
  hue, sat, val, onChange, width = 220, height = 160,
}: {
  ...
  return (
    <div className="relative h-3 rounded-full overflow-hidden cursor-pointer"
      ...
      onClick={(e) => {
        ...
        onChange(Math.max(0, Math.min(359, ((e.clientX - rect.left) / rect.width) * 360)));
      }}
    >
      <div
...
export function ColorPickerPopover({ color, onChange, label, onClose }: ColorPickerProps) {
    ...
    >
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1 flex items-center gap-1 bg-[var(--fintheon-bg)]/60 rounded-md border border-[var(--fintheon-border)]/15 px-2 py-1.5">
          <span className="text-[10px] text-[var(--fintheon-muted)]/40 font-mono">#</span>
          <input
            type="text"
...
export function ColorSwatchInput({ color, onChange, label }: ColorSwatchInputProps) {
    ...
    <div ref={wrapperRef} className="relative">
      <div className="flex items-center gap-3 cursor-pointer" onClick={() => setOpen(!open)}>
        ...
        <div className="flex-1">
          <span className="text-[11px] text-[var(--fintheon-muted)]/60 uppercase tracking-wider block">{label}</span>
          <span className="text-[12px] font-mono text-[var(--fintheon-text)]/80">{color}</span>
        </div>
      </div>
