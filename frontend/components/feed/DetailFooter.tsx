...
function computeDeviation(alert: RiskFlowAlert): { text: string; color: string } | null {
  const ed = alert.econData;
  if (!ed || ed.actual == null || ed.forecast == null || ed.forecast === 0) return null;
  const dev = ((ed.actual - ed.forecast) / Math.abs(ed.forecast)) * 100;
  const sign = dev >= 0 ? '+' : '';
  return {
    text: `${sign}${dev.toFixed(2)}%`,
    color: dev > 0 ? 'text-emerald-400' : dev < 0 ? 'text-red-400' : 'text-zinc-500',
  };
}

/** Derive beat/miss from econData */
function deriveBeatMiss(alert: RiskFlowAlert): { label: string; color: string } | null {
  const ed = alert.econData;
  if (ed?.beatMiss) {
    if (ed.beatMiss === 'beat') return { label: 'BEAT', color: 'text-emerald-400' };
    if (ed.beatMiss === 'miss') return { label: 'MISS', color: 'text-red-400' };
    if (ed.beatMiss === 'inline') return { label: 'IN LINE', color: 'text-zinc-500' };
  }
...
function getCyclicalLabel(alert: RiskFlowAlert): { label: string; color: string } | null {
  const cyc = alert.cyclical;
  if (!cyc || cyc === 'Neutral') return null;
  if (cyc === 'Cyclical') return { label: 'CYC', color: 'text-[var(--fintheon-accent)]' };
...
export function DetailFooter({ alert }: DetailFooterProps) {
  ...
  const ivDisplay = sub
    ? Math.min(10, Math.max(0,
        (sub.eventWeight + sub.timing + sub.deviation + sub.momentum + sub.vixContext)
        * (sub.vixMultiplier ?? 1) * (sub.regimeMultiplier ?? 1) * (sub.commentatorMultiplier ?? 1) / 2.8
      )).toFixed(1)
    : null;

  const hasRow1 = ivDisplay || deviation || beatMiss || cyclical;
  const hasRow2 = !!sub;
  const hasSpeaker = sub?.speaker && sub?.commentatorMultiplier != null && sub.commentatorMultiplier !== 0.8;
    ...
    <div className="border-t border-zinc-800/40 bg-zinc-900/50 px-3 py-1.5 space-y-0.5">
        ...
        <div className="flex items-center gap-3 font-mono text-[9px] tabular-nums">
          {ivDisplay && (
            <span className="text-zinc-400">
              IV: <span className="text-[var(--fintheon-accent)] font-semibold">{ivDisplay}</span>
            </span>
          )}
          {deviation && (
            <span className="text-zinc-500">
              Deviation: <span className={deviation.color}>{deviation.text}</span>
            </span>
          )}
          {beatMiss && (
            <span className="text-zinc-500">
              Beat/Miss: <span className={`font-semibold ${beatMiss.color}`}>{beatMiss.label}</span>
            </span>
          )}
          {cyclical && (
            <span className={`font-semibold ${cyclical.color}`}>{cyclical.label}</span>
          )}
        </div>
      )}
      ...
      {hasRow3 && (
        <div className="font-mono text-[9px] text-zinc-500 tabular-nums">
          ...
          {hasSpeaker && hasRegime && <span>{'  '}·{'  '}</span>}
          {hasRegime && (
            <span className="text-cyan-400/70">
              Regime: {sub!.regimeName}
            </span>
          )}
        </div>
      )}
