...
export function ProposalCard({ proposal, timestamp }: ProposalCardProps) {
  ...
  const reward = proposal.takeProfit.length > 0
    ...
    <div className="border border-[#D4AF37]/20 rounded-lg bg-[#050402] p-3 space-y-2">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[#f0ead6] font-mono font-semibold text-sm">{proposal.ticker}</span>
        </div>
      ...
      <div className="flex items-center gap-3 text-[11px] font-mono">
        ...
        {proposal.takeProfit.map((tp, i) => (
          <span key={i} className="text-[#3b82f6]">T{i + 1} {tp.toFixed(1)}</span>
        ))}
      ...
      <div className="flex items-center justify-between pt-1">
        <span className="text-zinc-600 text-[9px] tracking-wider uppercase">via Hermes</span>
        {status === 'pending' ? (
          ...
          <span className={`text-[10px] font-medium tracking-wider uppercase ${
            status === 'approved' ? 'text-[#22c55e]' : status === 'rejected' ? 'text-[#ef4444]' : 'text-zinc-500'
          }`}>
            {status}
          </span>
        )}
