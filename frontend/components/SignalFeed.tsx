...
function Countdown({ expiresAt }: { expiresAt: string }) {
  ...
  return (
    <span
      className={`text-[10px] font-mono ${isExpired ? "text-red-400" : "text-[#c79f4a]"}`}
    >
      {remaining}
    </span>
  );
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-xs font-medium text-[#f0ead6] uppercase tracking-wider">
          Signal Feed
        </span>
        {signals.length > 0 && (
          <span className="text-[10px] text-[#f0ead680] ml-auto">
        ...
        >
          <div className="flex items-center justify-between mb-1.5">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-[#c79f4a]">
                {proposal.instrument}
              </span>
          ...
          <div className="flex items-center justify-between">
            <span className="text-[10px] text-[#f0ead680]">
              {proposal.strategyName.replace(/_/g, " ")}
            </span>
            <span
              className={`text-xs font-bold font-mono ${proposal.confidenceScore >= 80 ? "text-[#c79f4a]" : "text-[#f0ead680]"}`}
            >
              {proposal.confidenceScore}%
            </span>
          </div>
          {proposal.rationale && (
            <div className="mt-1.5 text-[10px] text-[#f0ead650] truncate">
              {proposal.rationale}
            </div>
          )}
        </button>
        ...
        return (
          ...
          >
            ...
            <div className="flex items-center gap-2 mb-1.5">
              ...
              <span
                className={
                  isLong ? "text-green-400 text-sm" : "text-red-400 text-sm"
                }
              >
                {isLong ? "\u2191" : "\u2193"}
              </span>
              <span className="text-xs font-bold text-[#f0ead6]">
                {signal.instrument}
              </span>
              <span
                className={`text-xs font-mono ml-auto ${signal.confidence >= 80 ? "font-bold text-[#c79f4a]" : "text-[#f0ead680]"}`}
              >
                {signal.confidence}%
              </span>
