...
if [[ -f "$BACKEND_ENV" ]]; then
  ...
  grep -q "^FINANCIALJUICE_INTERVAL_MS=" "$BACKEND_ENV" 2>/dev/null || echo "FINANCIALJUICE_INTERVAL_MS=30000" >> "$BACKEND_ENV"
  grep -q "^FINANCIALJUICE_BACKOFF_MS=" "$BACKEND_ENV" 2>/dev/null || echo "FINANCIALJUICE_BACKOFF_MS=120000" >> "$BACKEND_ENV"
