...
if [[ ! -d "$FINTHEON_ROOT/.git" ]]; then
  ...
  echo '    /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/solvys-technologies/fintheon/v7.0.8/scripts/fintheon-setup.sh)"'
  exit 1
fi
...
if [[ -f "$BACKEND_ENV" ]]; then
  ...
  grep -q "^FINANCIALJUICE_INTERVAL_MS=" "$BACKEND_ENV" 2>/dev/null || echo "FINANCIALJUICE_INTERVAL_MS=120000" >> "$BACKEND_ENV"
  grep -q "^FINANCIALJUICE_BACKOFF_MS=" "$BACKEND_ENV" 2>/dev/null || echo "FINANCIALJUICE_BACKOFF_MS=300000" >> "$BACKEND_ENV"
