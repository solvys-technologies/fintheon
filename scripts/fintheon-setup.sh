...
#   8. Connects Anthropic subscription via VProxy OAuth
#   9. Builds the backend + frontend + DMG
#  10. Installs the `fintheon` CLI command globally
#  11. Starts the backend and launches Fintheon
#
...
# Harper uses Anthropic via local VProxy by default.
...
if [[ -f "$BACKEND_ENV" ]]; then
  ...
  grep -q "^DATABASE_URL=" "$BACKEND_ENV" || echo "DATABASE_URL=$SUPABASE_DATABASE_URL" >> "$BACKEND_ENV"
  ...
  grep -q "^AI_PRIMARY_PROVIDER=" "$BACKEND_ENV" || echo "AI_PRIMARY_PROVIDER=anthropic-vproxy" >> "$BACKEND_ENV"
...
# Harper uses Anthropic via local VProxy (OAuth subscription flow).
# ============================================================================
...
# Deployed Supabase Postgres pooler URL (same connection used on Fly)
DATABASE_URL=$SUPABASE_DATABASE_URL

# --- Harper Anthropic via VProxy ---
...
# ── Step 8: VProxy Anthropic OAuth ──────────────────────────────────────────

step "Connecting Anthropic subscription via VProxy..."

if [[ -f "$FINTHEON_DIR/scripts/vproxy-anthropic-oauth.sh" ]]; then
  if bash "$FINTHEON_DIR/scripts/vproxy-anthropic-oauth.sh"; then
    ok "VProxy Anthropic OAuth complete"
  else
    warn "VProxy OAuth step failed — you can retry later with: fintheon oauth"
  fi
else
  warn "vproxy-anthropic-oauth.sh not found — run OAuth later with VibeProxy manually"
fi
...
# ── Step 10: Install Fintheon CLI + DMG ─────────────────────────────────────
...
# ── Step 11: Start backend and launch ───────────────────────────────────────
...
echo "  ║  Logs:     tail -f /tmp/fintheon-backend.log         ║"
echo "  ║  CLI:      fintheon update | start | stop | oauth    ║"
echo "  ║                                                      ║"
echo "  ║  If Anthropic is not connected yet, run:             ║"
echo "  ║    fintheon oauth                                    ║"
echo "  ║  Then restart:                                       ║"
echo "  ║    fintheon stop && fintheon start                   ║"
echo "  ║                                                      ║"
echo "  ╚══════════════════════════════════════════════════════╝"
echo ""
