...
async function installHermes() {
  ...
  if (hermesExists) {
    ...
    p.log.success(`${pc.green('✓')} Hermes already installed ${pc.dim(version ?? '')}`);
...
async function collectApiKeys() {
  p.log.info(pc.dim('API keys are stored in backend-hono/.env (never committed)'));
  ...
  if (existing.OPENAI_API_KEY) {
    ...
    const openAiKey = await p.text({
      message: 'OpenAI API key (optional — for voice features, press Enter to skip):',
      placeholder: 'sk-...',
      validate: (v) => {
...
async function detectPort() {
  ...
  if (alreadyRunning) {
    p.log.success(`${pc.green('✓')} Backend already running on :${DEFAULT_PORT}`);
...
async function buildAndStartBackend() {
  ...
  const buildResult = await runCommand('bun', ['run', 'build'], { cwd: BACKEND_DIR });
  if (!buildResult.ok) {
  ...
  const child = spawnProcess('node', [join(BACKEND_DIR, 'dist', 'index.js')], {
    cwd: BACKEND_DIR,
    env: { ...process.env, NODE_ENV: 'production', DOTENV_CONFIG_PATH: envPath, PORT: String(ctx.port) },
    stdio: 'ignore',
    detached: true,
  });
  child.unref();
  ...
  // Wait for health
  const health = await waitForHealth(`http://localhost:${ctx.port}/health`, 10, 2000);
  if (health) {
...
async function verifyHealth() {
  ...
  const health = await waitForHealth(`http://localhost:${ctx.port}/health`, 3, 1000);
  ...
  if (health.components) {
    for (const [name, info] of Object.entries(health.components)) {
      const icon = info.status === 'ok' ? pc.green('✓') : info.status === 'degraded' ? pc.yellow('~') : pc.red('✗');
...
async function verifyHermes() {
  ...
  try {
    ...
    if (hermes?.status === 'ok') {
      ...
      await fetch(`http://localhost:${ctx.port}/api/diagnostics/hermes/restart`, {
        method: 'POST',
...
async function triggerWelcome() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (data.message) {
        const truncated = data.message.length > 500
...
function showSummary() {
  ...
  console.log(`    3. Desktop app:     ${pc.cyan('bun run desktop:build')} (for DMG)`);
  console.log('');
