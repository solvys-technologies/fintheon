...
export function createSkillsRoutes(): Hono {
  ...
  app.post('/trade-plan', async (c) => {
    ...
    if (!isComputerUseAvailable()) {
      return c.json({ plan: null, reason: 'Computer Use not enabled (set ENABLE_COMPUTER_USE=true)' })
    }

    const plan = await generateTradePlan(body.instrument, body.direction, body.context)
    return c.json({ plan })
  })
  ...
  app.post('/trade-plan/enrich', async (c) => {
    ...
    const direction = proposal.direction === 'flat' ? 'long' : proposal.direction
