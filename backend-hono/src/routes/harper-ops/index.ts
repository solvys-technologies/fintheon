...
export function createHarperOpsRoutes() {
  ...
  app.get('/journal/latest-synthesis', async (c) => {
    ...
    let entry = n
    if (s && (!n || (s.createdAt && n.createdAt && s.createdAt > n.createdAt))) entry = s
    return c.json({ entry })
  })
  ...
  app.post('/feed', async (c) => {
    ...
    const { writeOpsEntry } = await import('../../services/harper-autonomous/ops-store.js')
  ...
  app.post('/journal', async (c) => {
    const body = await c.req.json<{ entryType?: string; content?: string; tags?: string[]; context?: Record<string, unknown> }>().catch(() => null)
    if (!body || !body.content) return c.json({ error: 'content required' }, 400)
    const { writeJournalEntry } = await import('../../services/harper-autonomous/journal-store.js')
  ...
  app.post('/trigger', async (c) => {
    const body = await c.req.json<{ type?: string; message?: string }>().catch(() => ({ type: undefined, message: undefined })) as { type?: string; message?: string }
    ...
    const validTypes = ['scoring-qa', 'narrative-synthesis', 'brief-review', 'regime-memo', 'manual']
    if (!validTypes.includes(body.type)) {
      return c.json({ error: `Invalid task type. Valid: ${validTypes.join(', ')}` }, 400)
    }

    enqueueTask({
      type: body.type as HarperTask['type'],
      payload: { message: body.message, manual: true, timestamp: new Date().toISOString() },
      priority: 'normal',
    })
