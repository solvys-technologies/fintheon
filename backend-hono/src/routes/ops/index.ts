...
import { getLatestReport } from "../../services/miroshark/miroshark-service.js";
...
const MIROSHARK_FRESH_HOURS = 6; // expected cadence
const MIROSHARK_STALE_HOURS = 12;
...
export function createOpsRoutes() {
  ...
  app.get("/schedule-status", async (c) => {
    ...
    // Aquarium / MiroShark ──────────────────────────────────
    let mirosharkLastRunAt: string | null = null;
    try {
      ...
      const generatedAt =
        ...
        null;
      mirosharkLastRunAt = generatedAt ?? null;
    } catch (err) {
      log.warn("getLatestReport failed", err);
    }
    const mirosharkAgeMin = ageMinutesFrom(mirosharkLastRunAt, now);
    const mirosharkStatus: MirosharkStatus = {
      lastRunAt: mirosharkLastRunAt,
      ageMinutes: mirosharkAgeMin,
      status:
        mirosharkAgeMin == null
          ? "unknown"
          : mirosharkAgeMin < MIROSHARK_FRESH_HOURS * 60
            ? "ok"
            : mirosharkAgeMin < MIROSHARK_STALE_HOURS * 60
              ? "ok"
              : "stale",
    };
