...
const KNOWN_SERVERS: Record<string, Partial<McpServerEntry>> = {
  ...
  "close-crm": {
    name: "Close CRM",
    description: "CRM contacts, leads, and pipeline",
    category: "productivity",
    toolCount: 10,
    requiresApiKey: true,
  ...
  "qc-mcp": {
    name: "QuantConnect",
    description: "Algorithmic trading backtests and live deployment",
    category: "trading",
    toolCount: 12,
    requiresApiKey: true,
    apiKeyEnvVar: "QUANTCONNECT_API_TOKEN",
  },
  tradingview: {
    name: "TradingView",
    description: "Stock screener, technical analysis, chart data",
    category: "trading",
    toolCount: 6,
  ...
  figma: {
    name: "Figma",
    description: "Design system, components, and mockups",
    category: "productivity",
    toolCount: 18,
  },
};
...
function claudeServerToEntry(
  ...
  const envApiKey = known.apiKeyEnvVar
    ? server.env?.[known.apiKeyEnvVar]
    : undefined;
...
export function createMcpRoutes() {
  ...
  app.patch("/:id/toggle", async (c) => {
    ...
    const exists =
      id in claudeConfig.mcpServers || id in projectConfig.mcpServers;
    if (!exists) {
    ...
    log.info(
      `MCP server toggled: ${id} → ${body.enabled ? "enabled" : "disabled"}`,
    );
    return c.json({ ok: true, id, enabled: body.enabled });
  });
  ...
  app.put("/:id", async (c) => {
    ...
    if (!(id in config.mcpServers)) {
      return c.json(
        { error: `Server '${id}' not found in Claude config` },
        404,
      );
    }
