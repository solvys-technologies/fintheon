...
import { getProposalHistory, type StoredProposal } from '../../services/autopilot/proposal-service.js'
import { readTradeIdeas, type TradeIdeaRecord } from '../../services/supabase-service.js'
...
function deduplicate(cards: TradeIdeaCard[]): TradeIdeaCard[] {
  ...
  for (const card of cards) {
    ...
    const diff = Math.abs(
      new Date(card.createdAt).getTime() - new Date(existing.createdAt).getTime()
    )
...
export async function handleGetTradeIdeas(c: Context) {
  ...
  if (proposalsResult.status === 'fulfilled') {
    ...
    console.error('[TradeIdeas] proposals fetch failed:', proposalsResult.reason)
  }
