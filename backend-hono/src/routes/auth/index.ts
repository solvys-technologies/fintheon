...
export function createAuthRoutes(): Hono {
  ...
  router.post('/login', async (c) => {
    ...
    if (!client) {
      return c.json(
        { error: 'Supabase auth is not configured (SUPABASE_URL + key required)' },
        503,
      )
    }
    ...
    if (!body.password) {
      return c.json({ error: 'password is required when magicLink=false' }, 400)
    }
  ...
  router.get('/me', async (c) => {
    ...
    return c.json({
      user: {
        id: auth.userId,
        email: auth.email,
        displayName: profile?.displayName ?? auth.email.split('@')[0] ?? 'Peer User',
  ...
  router.post('/admin/add', async (c) => {
    ...
    const targetUserId =
      body?.userId === 'self' ? auth.userId : body?.userId
    ...
    return c.json({ user, bootstrap: bootstrapAllowed && requester?.role !== 'admin' })
  })
