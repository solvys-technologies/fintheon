// [claude-code 2026-04-20] S21-T1/T2: Omi integration routes.
...
//
// Mounted at /api/omi in routes/index.ts. Webhook paths are NOT gated by
...
import {
  ...
  endSession,
  getActiveSession,
  resolveUserIdForOmiUid,
  setPrimaryAgent,
  startSession,
} from "../services/omi/session-manager.js";
import type {
  OmiMemoryWebhookBody,
  OmiTranscriptWebhookBody,
  OmiTrigger,
} from "../services/omi/types.js";
import { routeIntent } from "../services/omi/router.js";
import {
  computeFeatures,
  persistSample,
...
const VALID_TRIGGERS: readonly OmiTrigger[] = [
  "psych_assist",
  "voice_assistant",
  "performance_chat",
];

export function createOmiRoutes() {
  ...
  router.post("/webhook/transcript", async (c: Context) => {
    ...
    const body = (await c.req
      .json()
      .catch(() => null)) as OmiTranscriptWebhookBody | null;
  ...
  router.post("/webhook/memory", async (c: Context) => {
    ...
    const body = (await c.req
      .json()
      .catch(() => null)) as OmiMemoryWebhookBody | null;
    if (!body) return c.json({ error: "bad body" }, 400);
  ...
  router.post("/session/start", async (c: Context) => {
    ...
    const trigger = body.trigger as OmiTrigger;
