...
async function handleSummarize(action: SidebarAction): Promise<SidebarAction> {
  ...
  if (!text.trim()) {
    return { ...action, result: { content: 'Document is empty — nothing to summarize.' } }
  }

  // Use Claude CLI if available, otherwise basic extractive summary
  try {
    const { getSessionManager } = await import('../claude-sdk/session-manager.js')
    ...
    log.warn('Claude CLI unavailable for summarize, using excerpt', { error: String(err) })
...
async function handleAnalyze(action: SidebarAction): Promise<SidebarAction> {
  ...
  try {
    ...
    if (regimeEntries.length > 0) {
      memoryContext = '\n\n**Shared Team Memory (Regime):**\n' +
        regimeEntries.slice(0, 5).map((e) => `- ${e.key}: ${JSON.stringify(e.value).slice(0, 200)}`).join('\n')
    }
  } catch { /* non-fatal */ }
  ...
  try {
    ...
    return { ...action, result: { content: `Analysis unavailable — Claude CLI not ready: ${String(err)}` } }
  }
}
...
async function handleFetchChart(action: SidebarAction): Promise<SidebarAction> {
  if (!isComputerUseAvailable()) {
    return {
      ...action,
      result: { content: 'Computer Use not configured. Set ENABLE_COMPUTER_USE=true to enable chart fetching.' },
    }
  }
  ...
  try {
    ...
    if (base64Match) {
      return { ...action, result: { imageBase64: base64Match[1], content: `Chart for ${action.prompt}` } }
    }
    return { ...action, result: { content: result } }
  } catch (err) {
    return { ...action, result: { content: `Failed to fetch chart: ${String(err)}` } }
  }
}
...
async function handleFetchData(action: SidebarAction): Promise<SidebarAction> {
  try {
    ...
    return { ...action, result: { content: `Data fetch unavailable: ${String(err)}` } }
  }
}

async function handleInsertImage(action: SidebarAction): Promise<SidebarAction> {
  if (!isComputerUseAvailable()) {
    return {
      ...action,
      result: { content: 'Computer Use not configured. Set ENABLE_COMPUTER_USE=true to enable image capture.' },
    }
  }
  ...
  try {
    ...
    const base64Match = result.match(/data:image\/[^;]+;base64,([A-Za-z0-9+/=]+)/) ||
      result.match(/([A-Za-z0-9+/=]{100,})/)
    if (base64Match) {
      return { ...action, result: { imageBase64: base64Match[1], content: `Image: ${action.prompt}` } }
    }
    return { ...action, result: { content: result } }
  } catch (err) {
    return { ...action, result: { content: `Image capture failed: ${String(err)}` } }
  }
}
...
export async function executeSidebarAction(action: SidebarAction): Promise<SidebarAction> {
  log.info(`Executing sidebar action: ${action.type}`, { documentId: action.documentId })

  switch (action.type) {
    case 'summarize': return handleSummarize(action)
    case 'analyze': return handleAnalyze(action)
    case 'fetch-chart': return handleFetchChart(action)
    case 'fetch-data': return handleFetchData(action)
    case 'insert-image': return handleInsertImage(action)
    default:
      return { ...action, result: { content: `Unknown action type: ${(action as any).type}` } }
  }
}
