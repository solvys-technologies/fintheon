...
export interface PMAReport {
  predictionMarkets: {
    ...
    politicalRisk: { level: 'low' | 'moderate' | 'elevated' | 'high'; drivers: string[] }
  }
  macroAssessment: {
...
export async function analyzePMAMerged(
  ...
  if (cached && (cached.reportData as Record<string, unknown>)?.predictionMarkets) {
    return cached
  }
...
function buildPMAPrompt(data: PMAInput): string {
  const sections: string[] = ['Synthesize the following data into a unified prediction market + macro assessment:']

  if (data.polymarketData) {
    sections.push('\n=== PREDICTION MARKETS ===')
    if (data.polymarketData.fedCutProb != null)
      sections.push(`Fed rate cut probability: ${(data.polymarketData.fedCutProb * 100).toFixed(1)}%`)
    if (data.polymarketData.recessionProb != null)
      sections.push(`Recession probability: ${(data.polymarketData.recessionProb * 100).toFixed(1)}%`)
    if (data.polymarketData.electionOdds) {
      sections.push('Election odds:')
      for (const [candidate, odds] of Object.entries(data.polymarketData.electionOdds)) {
  ...
  if (data.macroData) {
    sections.push('\n=== MACRO DATA ===')
    if (data.macroData.latestCpi != null) sections.push(`Latest CPI: ${data.macroData.latestCpi}%`)
    if (data.macroData.fedFundsRate != null) sections.push(`Fed Funds Rate: ${data.macroData.fedFundsRate}%`)
    if (data.macroData.gdpGrowth != null) sections.push(`GDP Growth: ${data.macroData.gdpGrowth}%`)
    if (data.macroData.unemploymentRate != null) sections.push(`Unemployment: ${data.macroData.unemploymentRate}%`)
    if (data.macroData.nfpDelta != null) sections.push(`NFP Delta: ${data.macroData.nfpDelta}k`)
  }
