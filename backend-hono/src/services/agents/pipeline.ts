...
export async function runAgentPipeline(
  ...
  const [marketDataReport, technicalReport, sentimentReport] =
    await Promise.all([
  ...
  const sentiment =
    sentimentReport.reportData as unknown as NewsSentimentReport;
  ...
  const currentPrice =
    options.currentPrice ?? technical.emaAnalysis.ema20 ?? 19000;
...
function createQuickConsensus(
  ...
  const bullScore =
    bullish.conviction *
    (bullish.keyArguments.reduce((s, a) => s + a.strength, 0) /
      Math.max(1, bullish.keyArguments.length * 10));
  const bearScore =
    bearish.conviction *
    (bearish.keyArguments.reduce((s, a) => s + a.strength, 0) /
      Math.max(1, bearish.keyArguments.length * 10));
  ...
  return {
    ...
    finalAssessment: {
      ...
      keyRisks: [
        ...bullish.riskFactors.slice(0, 2),
        ...bearish.riskFactors.slice(0, 2),
      ],
    },
    createdAt: new Date().toISOString(),
  };
...
export async function runAnalystsOnly(userId: string): Promise<{
  ...
  const [marketDataReport, sentimentReport, technicalReport] =
    await Promise.all([
      analyzeMarketData(userId),
      analyzeHeraldSentiment(userId),
      analyzeTechnicals(userId),
