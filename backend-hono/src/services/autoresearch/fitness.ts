...
import type { ScoringObservation, FitnessResult, FitnessReport } from './types.js';

/**
 * Evaluate the fitness of a single observation.
 * Requires the observation to have both predictedMove and actualMove filled in.
 */
export function evaluateObservation(obs: ScoringObservation): FitnessResult | null {
  if (obs.actualMove == null || obs.predictedMove == null || obs.predictedMove === 0) {
    return null;
  }
  ...
  const magnitudeError = Math.abs(Math.abs(obs.actualMove) - Math.abs(obs.predictedMove));
  const magnitudeErrorPct = (magnitudeError / Math.abs(obs.predictedMove)) * 100;
 ...
 */
export function generateFitnessReport(observations: ScoringObservation[]): FitnessReport {
  ...
  // Aggregate stats
  const directionCorrectCount = results.filter(r => r.fit.directionCorrect).length;
  const sumMagError = results.reduce((s, r) => s + r.fit.magnitudeError, 0);
  const sumMagErrorPct = results.reduce((s, r) => s + r.fit.magnitudeErrorPct, 0);
  ...
  for (const { obs, fit } of results) {
    ...
    if (!byEventType[et]) {
      byEventType[et] = { count: 0, directionAccuracy: 0, meanMagnitudeError: 0, meanBias: 0 };
    }
  ...
  for (const et of Object.keys(byEventType)) {
    ...
    byEventType[et].directionAccuracy = Number((byEventType[et].directionAccuracy / n * 100).toFixed(1));
    byEventType[et].meanMagnitudeError = Number((byEventType[et].meanMagnitudeError / n).toFixed(2));
    byEventType[et].meanBias = Number((byEventType[et].meanBias / n).toFixed(2));
  }
  ...
  for (const { obs, fit } of results) {
    ...
    if (!bySession[sess]) {
      bySession[sess] = { count: 0, directionAccuracy: 0, meanMagnitudeError: 0 };
    }
  ...
  for (const sess of Object.keys(bySession)) {
    ...
    bySession[sess].directionAccuracy = Number((bySession[sess].directionAccuracy / n * 100).toFixed(1));
    bySession[sess].meanMagnitudeError = Number((bySession[sess].meanMagnitudeError / n).toFixed(2));
  }

  return {
    totalObservations: observations.length,
    evaluatedObservations: evaluated,
    directionAccuracy: Number((directionCorrectCount / evaluated * 100).toFixed(1)),
