...
export function parseBulkText(rawText: string): BulkParseResult {
  ...
  for (const chunk of chunks) {
    ...
    try {
      const { parsed: parsedHeadline } = parseHeadline(headline, {
      ...
      errors.push(
        `Failed to parse: "${headline.slice(0, 80)}...": ${(err as Error).message}`,
      );
      skipped++;
    }
  }
...
function classifyEventType(text: string): string {
  ...
  if (/\bSANCTION|\bWAR\b|\bTENSION|\bMISSILE|\bATTACK\b/.test(upper))
    return "geopolitical";
  if (/\bCHINA\b.*\bTRADE\b|\bTRADE\b.*\bCHINA\b/.test(upper))
    return "chinaTrade";
  ...
  if (/\bPOLITICAL\b|\bCONGRESS\b|\bSENATE\b|\bTRUMP\b|\bBIDEN\b/.test(upper))
