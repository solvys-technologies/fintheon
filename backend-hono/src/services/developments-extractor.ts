...
export async function extractDevelopments(
  ...
  for (const msg of messages) {
    ...
    if (content.includes("[TRADE IDEA]")) {
      ...
      /regime/i.test(content) &&
      /shift|change|transition/i.test(content)
    ) {
      ...
      content.includes("[PRE-MARKET BRIEF]") ||
      content.includes("[POST-MARKET BRIEF]")
    ) {
      category = "briefing";
      severity = "info";
