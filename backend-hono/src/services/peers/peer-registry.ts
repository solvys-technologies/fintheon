...
function mapPeerRow(row: any): ClaudePeer {
  ...
  return {
    ...
    hermesAvailable: Boolean(
      row.hermes_available ?? row.hermesAvailable ?? false,
    ),
    createdAt: toIso(row.created_at ?? row.createdAt),
    user,
  };
...
export async function getUserById(userId: string): Promise<User | null> {
  ...
  try {
    const rows =
      await sql`SELECT * FROM users WHERE id::text = ${userId} LIMIT 1`;
    if (rows.length === 0) return null;
    return mapUserRow(rows[0]);
  } catch (error) {
    if (shouldFallbackToMemory(error)) {
      forceMemoryFallback = true;
      log.warn("users table unavailable, falling back to memory", {
        error: String(error),
      });
      return memoryUsers.get(userId) ?? null;
    }
...
async function getOrCreateUser(
  ...
  try {
    ...
    if (shouldFallbackToMemory(error)) {
      forceMemoryFallback = true;
      log.warn("user upsert failed, switching to memory peer registry", {
        error: String(error),
      });
      return ensureMemoryUser(userId, seed);
    }
...
export async function upsertUserRole(
  ...
  try {
    ...
    if (shouldFallbackToMemory(error)) {
      forceMemoryFallback = true;
      log.warn("user role upsert failed, using memory fallback", {
        error: String(error),
      });
      const user = ensureMemoryUser(userId, seed);
      const updated = { ...user, role };
...
export async function hasAdminUsers(): Promise<boolean> {
  ...
  try {
    const rows =
      await sql`SELECT COUNT(*)::int AS count FROM users WHERE role = 'admin'`;
...
export async function registerPeer(
  ...
  if (!canUseDb() || !isUuid(userId)) {
    const existing = Array.from(memoryPeers.values()).find(
      (peer) =>
        peer.userId === userId && peer.deviceName === registration.deviceName,
    );
    ...
    if (existing) {
      const updated: ClaudePeer = {
        ...existing,
        platform:
          registration.platform ?? existing.platform ?? process.platform,
        capabilities: registration.capabilities ?? existing.capabilities ?? [],
        deskId: registration.deskId ?? existing.deskId ?? null,
        assignedAgents:
          registration.assignedAgents ?? existing.assignedAgents ?? [],
  ...
  try {
    ...
    if (shouldFallbackToMemory(error)) {
      forceMemoryFallback = true;
      log.warn("claude_peers insert failed, using memory fallback", {
        error: String(error),
      });
      const existing = Array.from(memoryPeers.values()).find(
        (peer) =>
...
export function startHeartbeatMonitor(): void {
  ...
  heartbeatMonitor = setInterval(async () => {
    ...
    try {
      ...
      if (shouldFallbackToMemory(error)) {
        forceMemoryFallback = true;
        log.warn("heartbeat monitor switched to memory mode", {
          error: String(error),
        });
      } else {
