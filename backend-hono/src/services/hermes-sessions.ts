...
import {
  ...
  addBoardroomMessage,
} from "./boardroom-store.js";

const HERMES_SESSIONS_DIR = join(
  process.env.HOME ?? "",
  ".hermes/agents/main/sessions",
);
const HERMES_SEND_URL =
  process.env.HERMES_SESSIONS_SEND_URL ??
  "http://localhost:47832/api/sessions/send";
...
const inferAgent = (
  content: string,
): { agent: BoardroomAgent; emoji: string } => {
...
async function findSessionFilesByLabel(
  sessionLabel: string,
): Promise<string[]> {
...
export async function getBoardroomMessages(
  ...
  if (filter?.until) {
    ...
    messages = messages.filter(
      (m) => new Date(m.timestamp).getTime() <= untilMs,
    );
  }
...
export async function getInterventionMessages(
  sessionLabel = "pic-intervention",
...
export async function appendToBoardroom(
  ...
  try {
    ...
    const msg = await addBoardroomMessage(session.id, {
      agent,
      role,
      content,
      metadata,
    ...
    console.error(
      "[Boardroom] Supabase write failed, falling back to JSONL:",
      err,
    );
  }
...
async function appendToSession(
  ...
  const targetFile =
    files[0] ?? join(HERMES_SESSIONS_DIR, `${sessionLabel}.jsonl`);
...
export async function sendToIntervention(
  message: string,
  sessionKey = "pic-intervention",
): Promise<void> {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to send intervention message: ${response.status} ${text}`,
    );
  }
  ...
  await appendToBoardroom(
    `Human Executive (Intervention): ${message.trim()}`,
    "user",
  );
}
...
export async function checkBoardroomStatus(): Promise<{
