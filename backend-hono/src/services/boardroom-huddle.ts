...
export async function triggerHuddle(event: FeedItem): Promise<void> {
  ...
  for (const agent of AGENTS) {
    ...
    try {
      const response = await handleHermesChat({ message: prompt, agentOverride: agent.role });
      await appendToBoardroom(`**${agent.name}** (Huddle):\n${response.content}`, 'assistant');
    } catch (err) {
      console.error(`[Huddle] Agent ${agent.name} failed:`, err);
      await appendToBoardroom(`**${agent.name}** (Huddle): [Error — agent unavailable]`, 'assistant');
    }
  }
