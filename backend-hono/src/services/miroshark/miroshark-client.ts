 ...
 */
async function fetchHeadlinesForAnalyst(analyst: MarketAnalystAgent): Promise<string[]> {
  const sb = getSupabaseClient();
  if (!sb) return [];
  ...
  const { data, error } = await sb
    .from('scored_riskflow_items')
    .select('headline, body, tags, created_at, macro_level, iv_score, sentiment')
    .gte('created_at', cutoff)
    .order('created_at', { ascending: false })
    .limit(200);
  ...
  for (const row of data) {
    ...
    const hasSubjectMatch = tags.some(t =>
      t.startsWith('subj:') && analystSubjects.has(t.slice(5))
    );
    if (hasSubjectMatch) {
      matching.push(row);
    } else {
  ...
  return [...primary, ...crossDomain].map(row => {
    ...
    const tier = level >= 4 ? 'CRITICAL' : level >= 3 ? 'HIGH' : level >= 2 ? 'MED' : 'LOW';
    const sent = row.sentiment ? ` (${row.sentiment})` : '';
    return `[${tier}] ${row.headline}${sent}`;
  });
}

function buildAnalystSystemPrompt(persona: string, headlines: string[]): string {
  let prompt = `${persona}
 ...
 */
export async function runMarketAnalystDebate(seed: MiroSharkSeed): Promise<MiroSharkReport> {
  const context = formatSeedContext(seed);
  const startTime = Date.now();
  ...
  const results = await Promise.allSettled(
    MARKET_ANALYSTS.map((analyst, i) => runAnalystAgent(analyst, context, headlinesByAnalyst[i])),
  );
  ...
  console.log(`[MiroShark] Analyst debate completed: ${responses.length}/${MARKET_ANALYSTS.length} analysts responded in ${Date.now() - startTime}ms`);
