...
class SessionManager {
  ...
  constructor() {
    this.refreshHourET = Number(process.env.CLAUDE_SESSION_REFRESH_HOUR_ET ?? '18')
  }
   ...
   */
  sendPrompt(prompt: string, options?: Partial<ClaudeSDKConfig>): AsyncGenerator<ClaudeStreamEvent> {
    if (!this.alive) {
    ...
    return (async function* () {
      // Wait for queue slot
      const stream = await new Promise<AsyncGenerator<ClaudeStreamEvent>>((resolve, reject) => {
   ...
   * Queued — only one prompt processes at a time.
   */
  async sendPromptSync(prompt: string, options?: Partial<ClaudeSDKConfig>): Promise<string> {
    if (!this.alive) {
      throw new Error('Session not alive — use fallback')
    }
  ...
  private async processStreamItem(item: StreamQueueItem): Promise<void> {
    ...
    log.info(`Session processing stream request (queue: ${this.queue.length} remaining)`)
  ...
  private async processSyncItem(item: SyncQueueItem): Promise<void> {
    ...
    log.info(`Session processing sync request (queue: ${this.queue.length} remaining)`)
  ...
  private async *createStreamGenerator(
    ...
    if (!stdout) {
      yield { type: 'error', error: { message: 'No stdout from Claude process' } }
      return
    }
  ...
  private async refresh(): Promise<void> {
    log.info(`Session refresh triggered (${this.refreshHourET}:00 ET) — waiting for in-flight request...`)
    ...
    log.info(`Session refreshed (served ${prevCount} requests in previous window)`)
  }
}
