...
import { NEWS_WORKER_CONTRACT } from "../../workers/riskflow-worker/contract.js";
import {
...
const X_LIST_URL = process.env.X_BURNER_LIST_URL ?? "https://x.com/i/lists/0"; // TP supplies the real list ID via env
...
function extractTweetsFromUserTweets(
  payload: UserTweetsXhrPayload,
): TwitterTweet[] {
...
async function attachInterceptor(page: Page): Promise<void> {
  page.on("response", async (res) => {
    try {
      const url = res.url();
      if (
        !/\/i\/api\/graphql\/[^/]+\/(UserTweets|HomeLatestTimeline)/.test(url)
      ) {
        return;
      }
      ...
      const json = (await res
        .json()
        .catch(() => null)) as UserTweetsXhrPayload | null;
      if (!json) return;
