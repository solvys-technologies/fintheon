...
import { drainNewTweets, getStreamingState } from "./streaming-watcher.js";
...
function tweetToCollectedItem(
  ...
  return {
    ...
    source: `twitter:${t.username}`,
    source_domain: "x.com",
    headline: t.text,
    body: t.text,
    url: t.url,
    image_url: (t as { imageUrl?: string | null }).imageUrl ?? null,
    tier,
...
export async function collectFromTwitterPipelineInternal(
  ...
  if (mode === "streaming-only") {
    ...
    if (state === "healthy") {
      ...
      source =
        fromStream.length >= fromRettiwt.length ? "streaming" : "rettiwt";
    }
  }
