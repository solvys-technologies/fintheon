...
const PROJECT_ROOT = process.env.FINTHEON_PROJECT_ROOT ?? join(import.meta.dirname, '..', '..', '..', '..')
...
async function getSoulFile(): Promise<string> {
  ...
  try {
    ...
    log.error('Failed to read HARPER-SOUL.md', { error: err instanceof Error ? err.message : String(err) })
...
async function getCodebaseManifest(): Promise<string> {
  ...
  try {
    ...
    manifestCache = entries.map((e) => `- \`${e.path}\` — ${e.purpose}`).join('\n')
    manifestCacheTime = Date.now()
    return manifestCache
  } catch {
    // Manifest doesn't exist yet — generate a minimal one
    try {
      const tree = execFileSync('find', [
        'backend-hono/src/services', '-name', '*.ts',
...
function formatJournalEntries(entries: JournalEntry[]): string {
  ...
  return entries
    .map((e) => {
      const time = e.createdAt ? new Date(e.createdAt).toLocaleTimeString('en-US', { timeZone: 'America/New_York', hour: '2-digit', minute: '2-digit' }) : '??:??'
...
async function getRiskFlowContext(): Promise<string> {
  try {
    ...
    const data = await res.json() as { items?: Array<{ headline: string; macroLevel: number; sentiment: string; source: string }> }
    if (!data.items?.length) return '(no recent RiskFlow items)'

    return data.items
      .map((item) => {
        const level = item.macroLevel >= 4 ? 'CRITICAL' : item.macroLevel >= 3 ? 'HIGH' : item.macroLevel >= 2 ? 'MED' : 'LOW'
...
export interface HarperTask {
  type: 'heartbeat' | 'level4-item' | 'vix-spike' | 'pipeline-stall' |
    'scoring-qa' | 'narrative-synthesis' | 'brief-review' | 'regime-memo' |
    'consilium-intervention' | 'manual'
...
export async function buildAutonomousContext(task: HarperTask): Promise<{ systemPrompt: string; taskPrompt: string }> {
  const [soulFile, journalEntries, manifest, gitDiff, riskFlow] = await Promise.all([
    getSoulFile(),
    getRecentEntries(20),
    getCodebaseManifest(),
