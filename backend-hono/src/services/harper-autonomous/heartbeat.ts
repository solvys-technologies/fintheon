...
export function startHeartbeat(): void {
  ...
  log.info(
    "Heartbeat scheduler started (5min market hours, 15min off-hours, 30min weekends)",
  );
}
...
function getHeartbeatTask(count: number): {
  ...
  if (count % 6 === 0) {
    ...
    return {
      type: "heartbeat",
      payload: {
        heartbeatNumber: count,
        includesScoringQA: true,
        includesNarrativeSynthesis: true,
      },
      priority: "normal",
    };
  }
