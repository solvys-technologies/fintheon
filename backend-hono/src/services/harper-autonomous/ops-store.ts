...
export async function writeOpsEntry(entry: OpsEntry): Promise<OpsEntry> {
  ...
  try {
    ...
    log.error("Failed to write ops entry", {
...
export async function getOpsFeed(
  limit = 50,
  offset = 0,
): Promise<{ entries: OpsEntry[]; total: number }> {
  if (!isDatabaseAvailable()) {
    return {
  ...
  try {
    ...
    log.error("Failed to read ops feed", {
      error: err instanceof Error ? err.message : String(err),
    });
    return {
      entries: memoryFeed.slice(offset, offset + limit),
...
export async function getPendingApprovals(): Promise<OpsEntry[]> {
  ...
  try {
    ...
    log.error("Failed to read pending approvals", {
      error: err instanceof Error ? err.message : String(err),
    });
    return [];
  }
...
export async function getOpsStatus(): Promise<{
  ...
  try {
    ...
    log.error("Failed to get ops status", {
      error: err instanceof Error ? err.message : String(err),
    });
    return {
      alive: false,
