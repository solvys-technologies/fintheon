...
import { startLoop, stopLoop, isAlive, getStatus, enqueueTask, triggerHeartbeat } from './loop-manager.js'
import { startHeartbeat, stopHeartbeat } from './heartbeat.js'
import { writeJournalEntry, getRecentEntries, searchJournal, getEntriesByTags } from './journal-store.js'
import { writeOpsEntry, getOpsFeed, getPendingApprovals, updateApproval, getOpsStatus, opsEmitter } from './ops-store.js'
...
export {
  // Loop
  startLoop, stopLoop, isAlive, getStatus, enqueueTask, triggerHeartbeat,
  // Heartbeat
  startHeartbeat, stopHeartbeat,
  // Journal
  writeJournalEntry, getRecentEntries, searchJournal, getEntriesByTags,
  // Ops
  writeOpsEntry, getOpsFeed, getPendingApprovals, updateApproval, getOpsStatus, opsEmitter,
  // Context
...
export async function bootHarperAutonomous(): Promise<void> {
  ...
  if (!enabled) {
    log.info('Harper autonomous loop disabled (set HARPER_AUTONOMOUS_ENABLED=true to enable)')
    return
  }
