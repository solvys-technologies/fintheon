...
// [claude-code 2026-04-26] services/boardroom-spawner.ts wasn't checked in on
// s455-cleanup; megacap-analyst was orphaned from that tree. Until the spawner
// lands, stub spawnSectorDispatch with a logger-only no-op so the build clears
// and the cron schedulers boot. Earnings ingestion + persistence still work;
// only the boardroom dispatch hand-off is silenced.
async function spawnSectorDispatch(
  args: Record<string, unknown>,
): Promise<void> {
  log.warn("spawnSectorDispatch stubbed (boardroom-spawner missing)", args);
}
