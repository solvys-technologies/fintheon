...
export function getCommandmentGates(context: {
  ...
  if (context.morningRoutineDone === false) {
    gates.push('[HARD BLOCK C14] Morning routine not completed. No trading until routine is verified.')
  }
  ...
  if (context.timeEST) {
    ...
    if (h > 11 || (h === 11 && m >= 30)) {
      gates.push('[HARD BLOCK] 11:30 AM EST circuit breaker active. No new trades.')
    }
  }

  // HARD BLOCK: Commandment 7 — no doubling down
  if (context.holdingLosingPosition) {
    gates.push('[HARD BLOCK C7] Holding a losing position. Do NOT add to this position. Cut and reassess.')
  }

  // SOFT: Commandment 6 — anti-revenge after losses
  if (context.consecutiveLosses && context.consecutiveLosses >= 2) {
    gates.push(`[WARNING C6] ${context.consecutiveLosses} consecutive losses detected. "You never need to make back losses the same way you lost them." Consider a different instrument or direction.`)
  }

  // SOFT: Hot hand — 48h overconfidence flag
  if (context.lastBigWinWithin48h) {
    gates.push('[WARNING C4] Hot hand flag active (48h window after $5K+ win). Verify thesis quality. Enforce standard sizing. Remember the BOJ pattern: $20K win, gave back half next day.')
  }

  // SOFT: Loss streak lockout awareness
  if (context.consecutiveLosses && context.consecutiveLosses >= 3) {
    gates.push('[ESCALATION] 3+ consecutive losses. Soft lockout should be triggered. Recommend stopping and running debrief before next trade.')
  }
