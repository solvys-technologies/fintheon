...
function looksScheduled(result: ExaSearchResult): boolean {
  ...
  const hasScheduleSignal = SCHEDULED_KEYWORDS.some((kw) => haystack.includes(kw));
...
function referencesUpcomingWindow(result: ExaSearchResult): boolean {
  const haystack = normalizeText(`${result.title} ${result.text}`);
  if (haystack.includes('next 4 hours') || haystack.includes('within 4 hours')) return true;
  return TIME_HINTS.some((kw) => haystack.includes(kw));
}

function overlapsEconCalendar(result: ExaSearchResult, existingEventNames: string[]): boolean {
...
export async function checkForScheduledEvents(): Promise<void> {
  ...
  const econEvents = await fetchEconCalendar({ from: today, to: today }).catch((err) => {
    log.warn('[ExaMonitor] Econ calendar fetch failed (non-fatal)', { error: String(err) });
    return [];
  });
  const normalizedEventNames = econEvents.map((event) => normalizeText(event.name));

  const queryResults = await Promise.all(
    SCHEDULED_EVENT_QUERIES.map((query) =>
      exaSearch(query, { type: 'neural', numResults: 5, useAutoprompt: true })
        .catch((err) => {
          log.warn('[ExaMonitor] Exa query failed', { query, error: String(err) });
          return [];
        })
    )
  );
  ...
  const candidateResults = Array.from(dedupByTitle.values()).filter((result) => {
  ...
  log.info(`[ExaMonitor] Ingested ${written} scheduled events from Exa (${freshRows.length} candidates)`);
}
...
export function startExaScheduledMonitor(): void {
  ...
  const scheduledCheck = async (): Promise<void> => {
    ...
    log.info(`[ExaMonitor] Next check in ${Math.round(interval / 60_000)}m (hotHours=${isHotHours})`);
