...
import { fetchBookmarks, isTwitterCliInstalled } from '../twitter-cli/twitter-cli-service.js';
...
const NARRATIVE_KEYWORDS: Record<string, RegExp> = {
  ...
  'us-china-relations': /\b(us.?china|china.*sanction|china.*tariff|taiwan|xi jinping|chip ban|export control|tiktok ban|fentanyl|rare earth|china.*retaliat)\b/i,
  'rate-cut-cycle': /\b(rate cut|fed cut|fomc|fed funds|powell|dot plot|terminal rate|neutral rate|easing cycle|pivot)\b/i,
  'trump-presidency': /\b(trump|executive order|white house|truth social|maga|bessent|doge|elon.*gov|vivek)\b/i,
  'price-stability': /\b(cpi|ppi|pce|inflation|deflation|disinflation|core price|shelter cost|sticky inflation|supercore)\b/i,
  'maximum-employment': /\b(nfp|payroll|unemployment|jobless claim|jolts|hiring|layoff|ADP|labor market|wage growth|quit rate)\b/i,
};
...
const SEARCH_GROUPS: SearchGroup[] = [
  ...
  {
    name: 'FJ-Wire',
    query: 'breaking market news Fed rate decision economic data CPI NFP earnings guidance tariff',
    source: 'FinancialJuice',
    idPrefix: 'exa-fj',
    numResults: 12,
    includeDomains: [
      'financialjuice.com', 'features.financialjuice.com',
      'zerohedge.com', 'reuters.com', 'bloomberg.com',
    ],
  },
  ...
  {
    name: 'Iran-POI',
    query: 'Araghchi OR Khamenei OR Pezeshkian OR "Hossein Salami" OR "Kamal Kharrazi" OR Shamkhani OR IRGC OR "Iran foreign minister" OR "Iran UN mission" statement nuclear sanctions retaliation enrichment Hormuz',
    source: 'OSINTSources',
    idPrefix: 'exa-iran',
    numResults: 10,
  },
  ...
  // ── Broader OSINT / Geopolitical ──────────────────────────────
  {
    ...
    query: 'breaking geopolitical military strike missile Iran Israel Houthi Hezbollah conflict ceasefire IRGC sanctions Yemen Lebanon',
    source: 'OSINTSources',
    idPrefix: 'exa-osint',
    numResults: 10,
  },
  ...
  {
    name: 'GOV-POI',
    query: '"Federal Reserve" statement OR "Treasury Secretary Bessent" OR "White House" economic OR ECB rate OR "Bank of Japan" OR Netanyahu OR tariff announcement',
    source: 'OSINTSources',
    idPrefix: 'exa-gov',
    numResults: 8,
  },
  ...
  {
    name: 'Mag7-AI',
    query: 'NVIDIA earnings OR Apple AI OR Microsoft Azure AI OR Google Gemini OR Meta AI OR Amazon AWS AI OR Tesla autopilot OR "data center" capex OR GPU shortage OR AI spending',
    source: 'FinancialJuice',
    idPrefix: 'exa-mag7',
    numResults: 8,
  },
  ...
  {
    name: 'Macro-Watchers',
    query: '"Nick Timiraos" OR "WSJ Fed" OR "FOMC minutes" OR "rate cut" OR "rate hold" OR "inflation outlook" OR "Powell press conference"',
    source: 'FinancialJuice',
    idPrefix: 'exa-macro',
    numResults: 8,
  },
...
function extractTags(text: string): string[] {
  ...
  if (/\b(iran|israel|irgc|houthi|hezbollah|military|missile|strike|araghchi|khamenei)\b/i.test(text)) tags.push('GEOPOLITICAL');
...
function exaToRawItem(result: ExaSearchResult, group: SearchGroup): RawRiskFlowItem | null {
...
async function pollBookmarks(): Promise<number> {
  ...
  if (items.length === 0) {
    log.info(`Bookmarks: ${tweets.length} fetched, 0 matched active narratives`);
    return 0;
  }

  const written = await writeRawItems(items);
  for (const item of items) submittedIds.add(item.tweet_id);
  log.info(`Bookmarks: ${written} catalysts ingested from ${tweets.length} bookmarks`);
  return written;
}
...
export function startCommentaryScraper(): void {
  if (!isExaAvailable()) {
    log.warn('EXA_API_KEY not set — Exa scrape disabled, bookmark polling may still work');
  }
  ...
  const scheduledPoll = async (): Promise<void> => {
    ...
    log.info(`Next catalyst scrape in ${Math.round(interval / 60_000)}m (hotHours=${isHotHours})`);
