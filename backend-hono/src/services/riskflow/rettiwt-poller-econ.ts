...
export async function processActualsFromTweets(
  ...
  for (const tweet of fjTweets) {
    // Keyword-first gate — cheap skip before running the event matcher.
    if (!ECON_KEYWORD_RE.test(tweet.text)) continue;

    const event = matchTweetToEvent(tweet.text, activeEvents);
    if (!event) continue;

    // Numeric extraction is best-effort enrichment. Keyword-only matches (no
    // numeric) are handled by the econ-keyword-trigger sweep against
    // raw_riskflow_items; this rettiwt path still requires a numeric actual
    // because it writes the event's print record to the econ tables.
