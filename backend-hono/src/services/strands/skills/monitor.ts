...
export const solvysMonitorTool = tool({
  name: "solvys_monitor",
  description:
    "Monitor parallel track execution during orchestrated sprints. Shows git activity, running processes, and build status across tracks.",
  inputSchema: z.object({
    trackIds: z
      .array(z.string())
      .optional()
      .describe("Specific track IDs to monitor (default: all)"),
  }),
  callback: async (input: { trackIds?: string[] }) => {
    const results: string[] = ["=== Track Monitor ==="];

    // Recent git activity
    const gitLog = await runShell(
      'git log --oneline --all -15 --format="%h %s (%an, %ar)"',
    );
    results.push("\n[Recent Git Activity]", gitLog);

    // Active branches
    const branches = await runShell(
    ...
    const procs = await runShell(
      'ps aux | grep -E "bun|node|claude" | grep -v grep | head -10',
    );
