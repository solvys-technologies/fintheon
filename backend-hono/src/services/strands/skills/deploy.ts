...
export const solvysDeployTool = tool({
  name: "solvys_deploy",
  description:
    "Deploy to production across targets: Electron DMG, Vercel, and/or Cloudflare Workers. Creates a GitHub release after successful deploy.",
  inputSchema: z.object({
    targets: z
      .array(z.enum(["dmg", "vercel", "workers"]))
      .describe("Which deployment targets to hit"),
    version: z
      .string()
      .optional()
      .describe("Version tag (auto-detected from git if omitted)"),
    dryRun: z
      .boolean()
      .optional()
      .describe("Preview what would happen without deploying"),
  }),
  callback: async (input: {
    ...
    const results: string[] = [
      `=== Solvys Deploy (${input.dryRun ? "DRY RUN" : "LIVE"}) ===`,
    ];
    const version =
      input.version ?? (await runShell("git describe --tags --always")).trim();
    ...
    if (!input.dryRun) {
      ...
      results.push(
        await runShell(
          `gh release create "${version}" --generate-notes 2>&1 | tail -3`,
        ),
      );
    }
