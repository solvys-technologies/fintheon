...
export const solvysInformTool = tool({
  name: "solvys_inform",
  description:
    "Generate handoff prompts for external AI agents that depend on recent work. Summarizes changes and provides context for other Claude Code/Cursor instances.",
  inputSchema: z.object({
    agents: z
      .array(z.string())
      .describe('Agent names to inform (e.g., "harper", "cursor", "codex")'),
    summary: z.string().describe("What was changed and why"),
    filesChanged: z
      .array(z.string())
      .optional()
      .describe("Key files that were modified"),
  }),
