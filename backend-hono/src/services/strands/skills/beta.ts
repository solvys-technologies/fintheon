...
export const solvysBetaTool = tool({
  name: 'solvys_beta',
  description: 'Build and test locally without committing or pushing. Verifies changes work before shipping. Optionally builds a local DMG for testing.',
  inputSchema: z.object({
    buildDmg: z.boolean().optional().describe('Also build a local DMG for testing (default false)'),
    startDev: z.boolean().optional().describe('Start the dev server after build (default false)'),
  }),
