...
export const solvysShipTool = tool({
  name: "solvys_ship",
  description:
    "Build the project, fix any errors, commit changes, push to remote, and optionally rebuild the Electron DMG. Runs the full ship workflow.",
  inputSchema: z.object({
    ...
    skipDmg: z
      .boolean()
      .optional()
      .describe("Skip DMG rebuild (default false)"),
  ...
  callback: async (input: {
    ...
    const commitMsg =
      input.message ??
      `[ship] auto-commit ${new Date().toISOString().slice(0, 16)}`;
    ...
    if (!input.skipDmg) {
      ...
      const dmgResult = await runShell(
        "cd electron && bun run build:dmg 2>&1 | tail -5",
      );
