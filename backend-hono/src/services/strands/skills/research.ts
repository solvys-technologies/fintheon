...
export const solvysResearchTool = tool({
  ...
  description: 'Conduct deep research on a topic using web search, Twitter/X, Notion docs, and browser. Synthesizes human testimony and experiments into actionable findings.',
  inputSchema: z.object({
    query: z.string().describe('The research question or topic'),
    sources: z.array(z.enum(['web', 'twitter', 'notion', 'docs'])).optional().describe('Which sources to search (defaults to all)'),
    maxResults: z.number().optional().describe('Max results per source (default 5)'),
  }),
  callback: async (input: { query: string; sources?: string[]; maxResults?: number }) => {
