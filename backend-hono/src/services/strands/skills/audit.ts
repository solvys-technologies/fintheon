...
export const solvysAuditTool = tool({
  name: 'solvys_audit',
  description: 'Run a full-stack health check: TypeScript build, file size enforcement (300-line max), orphan detection, dead code scan, and sprint file archival.',
  inputSchema: z.object({
    fix: z.boolean().optional().describe('Attempt to auto-fix issues found (default false)'),
  }),
  callback: async (input: { fix?: boolean }) => {
    ...
    const bigFiles = await runShell(`find frontend/components backend-hono/src -name "*.ts" -o -name "*.tsx" | xargs wc -l 2>/dev/null | sort -rn | head -20`)
    ...
    // Dead exports check
    const deadExports = await runShell(`grep -rn "export " backend-hono/src/services/ --include="*.ts" | wc -l`)
