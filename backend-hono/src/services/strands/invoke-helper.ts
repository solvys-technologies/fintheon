...
import { createAgent, NOUS_MODELS, type HarperProvider } from './agent-factory.js'
...
export async function invokeAgent(options: InvokeAgentOptions): Promise<{ text: string }> {
  ...
  for (const provider of FALLBACK_CHAIN) {
    try {
      ...
      log.warn(`Provider ${provider} failed, trying next`, { error: lastError.message })
    }
  }
...
async function invokeWithProvider(options: InvokeAgentOptions, provider: HarperProvider): Promise<{ text: string }> {
...
async function invokeWithNousFallback(options: InvokeAgentOptions): Promise<{ text: string }> {
  let lastError: Error | null = null
  for (const modelId of NOUS_MODELS) {
    try {
      ...
      lastError = err instanceof Error ? err : new Error(String(err))
      log.warn(`Nous model ${modelId} failed, trying next`, { error: lastError.message })
    }
  }
