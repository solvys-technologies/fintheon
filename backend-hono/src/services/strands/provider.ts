...
let healthCache: { available: boolean; checkedAt: number; error: string | null } | null = null
...
function resolveModel(modelOverride?: string): string {
  ...
  if (model === 'opus') model = process.env.VPROXY_ANTHROPIC_MODEL || DEFAULT_MODEL
...
export async function checkVProxyHealth(force = false): Promise<{ available: boolean; error: string | null }> {
