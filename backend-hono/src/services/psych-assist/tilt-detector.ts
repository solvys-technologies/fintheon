...
export function detectTiltSignals(context: TiltDetectorContext): TiltSignal[] {
  ...
  if (!context.morningRoutineDone) {
    signals.push({
      ...
      severity: 'critical',
      message: 'Morning routine not completed. The routine IS the edge. No routine, no trades.',
      commandmentRef: 14,
    })
  }
  ...
  if (context.evalBehavior && context.fundedBehavior) {
    const sizeRatio = context.fundedBehavior.avgPositionSize / (context.evalBehavior.avgPositionSize || 1)
    const freqRatio = context.fundedBehavior.entryFrequency / (context.evalBehavior.entryFrequency || 1)
    ...
    if (sizeRatio > 0.8 && freqRatio > 0.8) {
      signals.push({
        type: 'funded_creep',
        severity: 'warning',
        message: 'Funded creep detected: position sizing and entry frequency match eval-mode aggression. Scale back to funded-account discipline.',
        commandmentRef: 7,
      })
    }
  }
