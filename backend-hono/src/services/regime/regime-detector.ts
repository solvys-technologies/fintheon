...
const GEO_TAGS = ['geopolitical', 'tariffs', 'conflict', 'sanctions', 'war', 'china', 'trade-war', 'nato'];
const MACRO_TAGS = ['fed', 'fomc', 'cpi', 'ppi', 'nfp', 'gdp', 'jobless', 'pce', 'rates', 'inflation'];
const EARNINGS_TAGS = ['earnings', 'revenue', 'guidance', 'beat', 'miss', 'eps'];
const LIQUIDITY_TAGS = ['liquidity', 'repo', 'funding', 'bank-stress', 'credit-spread', 'default'];
...
export function detectRegimeFromFeed(
  ...
  const geoItems = recent.filter(i => hasTag(i, GEO_TAGS) && (i.macroLevel ?? 1) >= 3);
  ...
  const macroItems = recent.filter(i => hasTag(i, MACRO_TAGS) && (i.macroLevel ?? 1) >= 3);
  if (macroItems.length >= 3) {
    signals.push({
      ...
      reasoning: [
        ...
        `Includes: ${macroItems.slice(0, 3).map(i => i.headline.slice(0, 60)).join('; ')}`,
      ],
  ...
  if (earningsItems.length >= 5 || (earningsItems.length >= 3 && isEarningsMonth)) {
    signals.push({
      ...
      reasoning: [
        ...
        isEarningsMonth ? 'Calendar confirms earnings month' : 'High earnings volume outside typical season',
      ],
  ...
  if (bearishHighMacro.length >= 2 && liquidityItems.length >= 1) {
    signals.push({
      ...
      confidence: Math.min(0.5 + bearishHighMacro.length * 0.1 + liquidityItems.length * 0.1, 0.9),
      reasoning: [
