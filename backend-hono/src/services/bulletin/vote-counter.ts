...
export async function castVote(
  ...
  if (isDatabaseAvailable()) {
    ...
    `;
    const oldType =
      prev.length > 0 ? (String(prev[0].vote_type) as VoteType) : null;
    ...
    if (oldType) {
      ...
      if (oldType !== voteType) {
        ...
        if (oldType === "up")
          await sql`UPDATE peer_bulletin SET vote_up = GREATEST(vote_up - 1, 0) WHERE id = ${bulletinId}`;
        else if (oldType === "down")
          await sql`UPDATE peer_bulletin SET vote_down = GREATEST(vote_down - 1, 0) WHERE id = ${bulletinId}`;
        else if (oldType === "check")
          await sql`UPDATE peer_bulletin SET vote_check = GREATEST(vote_check - 1, 0) WHERE id = ${bulletinId}`;
        else if (oldType === "x")
          await sql`UPDATE peer_bulletin SET vote_x = GREATEST(vote_x - 1, 0) WHERE id = ${bulletinId}`;
        // Increment new vote column
        if (voteType === "up")
          await sql`UPDATE peer_bulletin SET vote_up = vote_up + 1 WHERE id = ${bulletinId}`;
        else if (voteType === "down")
          await sql`UPDATE peer_bulletin SET vote_down = vote_down + 1 WHERE id = ${bulletinId}`;
        else if (voteType === "check")
          await sql`UPDATE peer_bulletin SET vote_check = vote_check + 1 WHERE id = ${bulletinId}`;
        else if (voteType === "x")
          await sql`UPDATE peer_bulletin SET vote_x = vote_x + 1 WHERE id = ${bulletinId}`;
      }
    } else {
      // Insert new vote
      await sql`
  ...
  if (post) {
    if (existing && existing.voteType !== voteType) {
      ...
      if (existing.voteType === "up")
        post.voteUp = Math.max(0, post.voteUp - 1);
      else if (existing.voteType === "down")
        post.voteDown = Math.max(0, post.voteDown - 1);
      else if (existing.voteType === "check")
        post.voteCheck = Math.max(0, post.voteCheck - 1);
      else if (existing.voteType === "x")
        post.voteX = Math.max(0, post.voteX - 1);
    }
    if (!existing || existing.voteType !== voteType) {
...
export async function checkAndPromote(bulletinId: string): Promise<boolean> {
  ...
  try {
    const { createProposalFromBulletin } =
      await import("../autopilot/proposal-service.js");
    ...
    } = await import("../skills/tradingview-trade-plan.js");
    if (isComputerUseAvailable() && proposal.instrument !== "UNKNOWN") {
      const direction =
        proposal.direction === "flat" ? ("long" as const) : proposal.direction;
      generateTradePlan(proposal.instrument, direction, post.content)
        ...
        .catch((err) =>
          console.warn(
            "[Bulletin] Trade plan generation failed (non-fatal)",
            String(err),
          ),
        );
    }
    ...
    console.log(
      `[Bulletin] Post ${bulletinId} promoted to proposal (${post.voteCheck} check votes)`,
    );
    return true;
  } catch (err) {
