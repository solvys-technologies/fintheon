...
async function callSidecarOptimize(
  ...
  samples: Sample[],
): Promise<{ candidate_body: string; projected_delta: Record<string, number>; projected_risk: string; run_id: string } | null> {
  try {
    const { isSidecarEnabled, sidecarClient } = await import(
      "../ai/sidecar-client.js"
    );
...
export async function runOnce(
  ...
  for (const agent of targets) {
    ...
    const dropped = baseline_7d > 0 && baseline_7d - accuracy > ACCURACY_DROP_THRESHOLD;
    ...
    if (pausedActive) {
      ...
      if (!proposal) {
        ...
        if (!withinSizeCap(current, proposal.candidate_body)) {
          ...
          pr_url = pr.pr_url;
          if (!pr.pr_opened) skip_reason = `pr not opened: ${pr.reason ?? "dry-run"}`;
        }
      }
    }
...
export async function loadGepaDiagnostics(): Promise<GepaDiagnostics> {
  ...
  try {
    ...
    for (const [agent, m] of Object.entries(byAgent)) {
      ...
      deltas[agent] = {
        accuracy: accs.length > 1
          ? `${(((accs[accs.length - 1] ?? 0) - (accs[0] ?? 0)) * 100).toFixed(2)}%`
          : "—",
        latency: lats.length > 1
