...
function heatInterpretation(score: number): string {
  if (score >= 9) return 'Extreme heat — capital preservation mode, hedge or sit flat';
  if (score >= 7) return 'High heat — reduce size, widen stops, favor reactive over predictive';
  if (score >= 5) return 'Elevated heat — expect wider ranges and faster reversals';
...
export function generateBriefing(
  ...
  for (const cs of sortedCats.slice(0, 3)) {
    ...
    keyFindings.push(`${label} heat ${cs.ivScore.toFixed(1)} and ${dir} — ${implication || 'monitor for developments'}. ${(cs.confidence * 100).toFixed(0)}% signal strength.`);
  }

  const topScenario = report.scenarios[0];
  if (topScenario) {
    keyFindings.push(`Top scenario: "${topScenario.label}" — ${(topScenario.probability * 100).toFixed(0)}% probability, projected heat ${topScenario.projectedIVScore.toFixed(1)}.`);
  }
  ...
  for (const cs of report.categoryScores) {
    if (cs.ivScore >= 7) {
      const label = CATEGORY_LABELS[cs.category] ?? cs.category;
      const implication = CATEGORY_TRADING_IMPLICATIONS[cs.category] ?? 'reduce exposure';
      riskAlerts.push(`${label} heat ${cs.ivScore.toFixed(1)} — elevated. ${implication}.`);
    }
  }

  if (regime >= 0.3) {
    riskAlerts.push(`Regime risk ${(regime * 100).toFixed(0)}% — market may be transitioning. Trend models unreliable until confirmed.`);
  }
  ...
  if (total > 0 && highVol / total > 0.6) {
    consensusInterpretation = 'Consensus leans volatile — favor mean-reversion setups over trend continuation.';
  } else if (total > 0 && lowVol / total > 0.6) {
    consensusInterpretation = 'Consensus leans calm — trend continuation and breakout setups preferred.';
  } else {
    consensusInterpretation = 'No strong consensus — reduce position sizing until direction clarifies.';
  }
