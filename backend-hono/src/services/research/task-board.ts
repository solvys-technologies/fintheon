...
const VALID_STATUSES: ResearchTaskStatus[] = ['pending', 'active', 'deep-dive', 'complete']
...
export async function createTask(input: ResearchTaskInput): Promise<ResearchTask> {
...
export async function listTasks(filter?: {
  ...
  if (!where) {
    const rows = await sql`SELECT * FROM research_tasks ORDER BY created_at DESC`
