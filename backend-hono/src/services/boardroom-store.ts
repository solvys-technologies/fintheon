...
export async function getSessionByDate(
  date: string,
): Promise<BoardroomSession | null> {
  if (!isDatabaseAvailable() || !sql) {
    return (
      Array.from(memoryStore.sessions.values()).find(
        (s) => s.sessionDate === date,
      ) ?? null
    );
  }
...
export async function addBoardroomMessage(
  ...
  if (!isDatabaseAvailable() || !sql) {
    ...
    if (list.length > MEMORY_MESSAGES_MAX)
      list.splice(0, list.length - MEMORY_MESSAGES_MAX);
...
export async function getSessionMessages(
  ...
  if (!isDatabaseAvailable() || !sql) {
    ...
    if (filter?.messageType)
      msgs = msgs.filter((m) => m.messageType === filter.messageType);
