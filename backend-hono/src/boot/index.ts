...
export function validateEnv(): ValidationResult {
  ...
  for (const spec of REQUIRED_VARS) {
    ...
    if (!value || value.trim() === "") {
      warnings.push(
        `[WARNING] ${spec.name} is not set — dependent features will degrade`,
      );
    }
  }
  ...
  if (isProd && !isDesktop) {
    for (const { name, reason } of DANGEROUS_DEV_VARS) {
      if (process.env[name]) {
        criticalErrors.push(
          `[CRITICAL] ${name} is set in production: ${reason}`,
        );
      }
    }
  }
  ...
  if (livekitMissing.length > 0 && livekitMissing.length < 3) {
    ...
    warnings.push(
      "[INFO] LiveKit vars not set — voice/video features will be disabled (graceful degradation)",
    );
  }
  ...
  if (!ok) {
    ...
    console.error(
      "[boot] Copy backend/.env.template to backend/.env and fill in all [CRITICAL] vars.\n",
    );
    process.exit(1);
  }
...
export const env = {
  ...
  RISKFLOW_ALLOW_MOCK_FALLBACK:
    process.env.RISKFLOW_ALLOW_MOCK_FALLBACK === "true",
  ...
  CLAUDE_SDK_EFFORT: (process.env.CLAUDE_SDK_EFFORT ?? "high") as
    | "low"
    | "medium"
    | "high",
  CLAUDE_SDK_MAX_TURNS: parseInt(process.env.CLAUDE_SDK_MAX_TURNS ?? "3", 10),
  CLAUDE_SDK_TIMEOUT_MS: parseInt(
    process.env.CLAUDE_SDK_TIMEOUT_MS ?? "300000",
    10,
  ),
  CLAUDE_SDK_CWD: process.env.CLAUDE_SDK_CWD,
  CLAUDE_SDK_SYSTEM_PROMPT: process.env.CLAUDE_SDK_SYSTEM_PROMPT,
  CLAUDE_SDK_SKIP_PERMISSIONS:
    process.env.CLAUDE_SDK_SKIP_PERMISSIONS === "true",
  CLAUDE_SDK_MAX_CONCURRENT: parseInt(
    process.env.CLAUDE_SDK_MAX_CONCURRENT ?? "2",
    10,
  ),
  ...
  HERMES_STREAM_CHUNK_DELAY_MS: parseInt(
    process.env.HERMES_STREAM_CHUNK_DELAY_MS ?? "18",
    10,
  ),
  HERMES_REASONING_CHUNK_DELAY_MS: parseInt(
    process.env.HERMES_REASONING_CHUNK_DELAY_MS ?? "14",
    10,
  ),
  ...
  DB_CONNECT_TIMEOUT_MS: parseInt(
    process.env.DB_CONNECT_TIMEOUT_MS ?? "5000",
    10,
  ),
  PRIMARY_BROKER: (process.env.PRIMARY_BROKER ?? "rithmic") as
    | "rithmic"
    | "projectx"
    | "hyperliquid",
