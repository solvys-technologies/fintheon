...
describe('validateEnv', () => {
  ...
  it('warns but does not exit when OPTIONAL vars are missing', () => {
    withEnv({ ...FULL_VALID_ENV, EXA_API_KEY: undefined, FRED_API_KEY: undefined }, () => {
      const result = validateEnv();
      expect(result.ok).toBe(true);
      expect(result.warnings.some((w) => w.includes('EXA_API_KEY'))).toBe(true);
    });
  });

  it('warns on partial LiveKit config', () => {
    withEnv({ ...FULL_VALID_ENV, LIVEKIT_API_KEY: 'key', LIVEKIT_API_SECRET: undefined, LIVEKIT_URL: undefined }, () => {
      const result = validateEnv();
      expect(result.warnings.some((w) => w.includes('Partial LiveKit'))).toBe(true);
    });
  });
});
