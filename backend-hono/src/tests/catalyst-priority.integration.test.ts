...
import { addClient, broadcastLevel4, removeClient } from '../services/riskflow/sse-broadcaster.js';
...
test('Level 4 path: assignment + gates + SSE broadcast', () => {
  ...
  let enqueueCount = 0;
  const controller = { enqueue: () => { enqueueCount += 1; } } as unknown as ReadableStreamDefaultController;
...
test('narrativePressureCap hook', () => {
  assert.equal(shouldUncapNarrativePressure({ macroLevel: 4, riskType: 'Macro' }), true);
  assert.equal(shouldUncapNarrativePressure({ macroLevel: 3, riskType: 'Macro' }), false);
  assert.equal(shouldUncapNarrativePressure({ macroLevel: 4, riskType: 'Earnings' }), false);
});
