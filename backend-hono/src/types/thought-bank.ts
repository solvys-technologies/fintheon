...
export type ThoughtCategory = 'standup' | 'news-response' | 'broadcast' | 'mention' | 'scheduled' | 'spontaneous'
...
function toCategory(raw: string): ThoughtCategory {
  return VALID_CATEGORIES.includes(raw as ThoughtCategory) ? (raw as ThoughtCategory) : 'spontaneous'
}
