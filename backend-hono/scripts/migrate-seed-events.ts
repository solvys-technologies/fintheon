...
async function migrate() {
  ...
  const knownCols = new Set(probe && probe.length > 0 ? Object.keys(probe[0]) : []);
  ...
  const scoredItems = (seedEvents as any[]).map((e: any) => {
    const row: Record<string, any> = {
      ...
      iv_score: e.severity === 'high' ? 7.0 : e.severity === 'medium' ? 4.0 : 2.0,
  ...
  const links: Array<{ card_id: string; thread_slug: string; confidence: number }> = [];
  for (const e of seedEvents as any[]) {
    const threads: string[] = e.narrativeThreads ?? (e.narrative ? [e.narrative] : []);
