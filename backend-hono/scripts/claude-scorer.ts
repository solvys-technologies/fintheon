...
if (!sbUrl || !sbKey) { log('FATAL: No Supabase credentials'); process.exit(1); }
...
async function fetchUnscored(limit: number): Promise<any[]> {
  ...
  const { data: raw, error } = await supabase
    ...
    .select('tweet_id, headline, body, source, symbols, tags, is_breaking, urgency, published_at')
...
Return ONLY a valid JSON array. No markdown, no explanation.`;
  ...
  try {
    ...
    if (!jsonMatch) { log('No JSON in Claude response'); return []; }
...
async function writeScored(items: any[], rawItems: any[]): Promise<number> {
  ...
  for (const scored of items) {
    ...
    const row: Record<string, unknown> = {
      ...
      source: raw.source ?? '',
      symbols: raw.symbols ?? [],
      tags: [...(raw.tags ?? []), scored.risk_type ?? 'Commentary'].filter(Boolean),
...
// ── Main loop ──
async function processBatch(): Promise<{ fetched: number; scored: number; written: number }> {
...
async function main() {
  log(`Claude Scorer starting (mode: ${ONCE ? 'once' : 'continuous'}, batch: ${BATCH_SIZE})`);

  if (ONCE) {
    const result = await processBatch();
    log(`Done: ${result.fetched} fetched, ${result.scored} scored, ${result.written} written`);
    return;
  }
...
main().catch(e => { log(`FATAL: ${e instanceof Error ? e.message : 'unknown'}`); process.exit(1); });
