...
# [claude-code 2026-04-20] Previous impl tried to `bun run dev` a duplicate
# backend when lsof reported nothing listening. The launchd-managed
# io.solvys.fintheon-backend is authoritative — spawning a second process
# raced port 8080 binding and failed with EADDRINUSE (actual 6:30am failure
# on 2026-04-20). Now we just wait up to 30s for the running backend to
# answer /api/diagnostics, then bail loudly if it doesn't.
...
if ! curl -sf --max-time 2 "$BACKEND_URL/api/diagnostics" >/dev/null 2>&1; then
  ...
  send_imessage "⚠️ MDB dispatch failed — backend unreachable (launchd backend not running?)"
  exit 1
fi
