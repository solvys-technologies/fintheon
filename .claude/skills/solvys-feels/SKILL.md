---
name: solvys-feels
description: Visual architecture for Solvys applications. Combines impeccable.style critique discipline, Jakub interface details, and Solvys industrial-luxe materials: warm near-black canvas, Solvys Gold accent, precise typography, and frosted-glass surfaces over Kanban cards. Use for any UI work -- new components, styling changes, theme adjustments, visual reviews, or frontend code.
version: 0.1.0
---
...
## Reference Intake Doctrine

External UI references are thinking sources, not defaults to copy. When TP approves a source such as Impeccable, devl.dev, Jakub interface details, Evil Charts, dotmatrix, detail.design, MV---Design, or cult-ui, extract only the architectural lesson: hierarchy, rhythm, state model, component boundary, chart composition, loading cadence, motion restraint, or review checklist. Do not import third-party code, packages, assets, services, or upstream visual language unless TP explicitly approves that implementation.

Always translate external references into Solvys-native output: warm near-black canvas, Solvys Gold accent, precise typography, thin separators, restrained motion, and Fintheon banned-ornament rules. If upstream guidance conflicts with Solvys rules, Solvys wins.

Never use TP-vetoed references as design inputs: `Xquik-dev/x-twitter-scraper`, `EveryInc/compound-engineering-plugin`, `jamiepine/voicebox`, `elder-plinius/CL4R1T4S`, or `Bitterbot-AI/bitterbot-desktop`.

Load `reference/design-guidelines.md` before any substantial UI design, redesign, or visual review.
...
5. **Material beats cards.** Use frosted Solvys glass for meaningful surfaces; use rulers, spacing, and type for density. Never default to generic gray cards.
...
Use for panels, sheets, drawers, modals, and important grouped surfaces:
